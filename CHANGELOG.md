...
- 🎯 **出题依据回归简历**：此前把本地项目源码档案（9000 字符，含核心源码预览）整段灌给面试官，导致它围着文件名/目录层级/函数实现刨根问底——真实面试官看不到候选人电脑里的代码。现在简历是**项目拷打的唯一依据**，源码档案降级为"核对简历没吹牛"的精简简报（技术栈 + 结构前 12 行 + README，≤1200 字符，剥离源码预览）；`search_project_archive` 工具描述与 prompt 同步改口径；历史学习产出只用于挑高频考点方向
