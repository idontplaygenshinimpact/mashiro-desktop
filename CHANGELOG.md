...
- 🔀 **明确放弃就换题**：候选人说"忘了/不会/没做过"（短回答命中放弃词）→ 服务端强制换一道同类型新题，不再硬追同一题（此前低分一律强制 followup，配合卡题空转导致同一题被连续拷打 7 轮）；换题兜底文案也不再重复原题
- 💻 **手写轮给代码编辑器**：`answerMode=code` 轮次（手写/场景题）作答区自动切代码形态——行号槽 + 等宽字体 + Tab 缩进（多行选区整体缩进/Shift+Tab 反缩进）+ Enter 自动缩进（`{([` 后多一级）+ 隐藏语音按钮；原生面板与 React 版同款（用户原话："手写没有编辑器用的太难受了，别用文本框"）
- 🐛 **卡题空转修复**（现场事故）：模型连续只调工具、始终不出 JSON → 出题轮 raw 为空 → 本轮 0 分 + 兜底"请继续"，面试官连续 7 轮卡在同一题、评分全 0（`trace_llm` 里每轮 4 连击 tool_calls 可复现）。现在工具轮上限 2 轮后**强制收口**（`tool_choice:"none"` 必须出 JSON）+ 无工具兜底一次；彻底拿不到 JSON 时返回可重试错误、**不写 0 分空转轮**
