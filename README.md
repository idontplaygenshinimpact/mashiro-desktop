...
| **三态渲染层** | **9 个 Tab × 原生 / React / Vue 三态并行**（含 2026-09-16 新增的「专项练习」：三态统一 CodeMirror 6 判题编辑器）（同一 preload IPC 桥 + 同一业务层零改动）；三态对比卡（dist 实测 2026-09-16，含三态 CodeMirror 判题编辑器：原生 376.8KB / React 748.6KB / Vue 669.2KB）；**两侧全 Tab 渲染护栏**（`tests/react-tabs.render.test.mjs` + `tests/vue-tabs.render.test.mjs`：按 tab 挂载 → 数据等价 → 特色标注 → UI 不变量 → 对称卸载） | ✅ 矩阵 8×2 满格 |
...
#### 功能增强：判题编辑器与 ACM 模式（2026-09-16）

| # | 内容 | 说明与护栏 |
|---|---|---|
| 26 | **三态统一 CodeMirror 6 判题编辑器 + 新增「✍️ 专项练习」Tab** | 原先题库只有原生面板有、且"编辑器"是手搓 textarea（Tab/Enter 缩进靠 60 行手工实现，无语法高亮/括号匹配）；现三态都是 CodeMirror 6（`desktop/renderer/practice-editor.ts` esbuild IIFE 产物，原生按需注入 + 缺产物回退 textarea；React/Vue 各新增 `Practice.jsx`/`Practice.vue`）。题库从「校招」Tab 独立成一级 Tab（三态矩阵 8×2 → 9×2）。护栏：`tests/practice-editor.test.mjs`（jsdom 真挂载产物，空壳产物反证必红）、`tests/react-practice-tab.test.mjs`、`tests/vue-practice-tab.test.mjs`、`tests/panel-challenges.test.mjs`（改契约 + 补回退路径） |
| 27 | **面试「手写轮」接入同一编辑器** | `panel-study.js` 的 `setIvAnswerMode()` 收口 mount/destroy；CodeMirror 的 `onChange` 把内容同步回 `#iv-answer`，`submitAnswer()` 照旧读它 → 文本/语音/代码三种作答共用同一提交链路（零改动）；`Mod-Enter` 提交。护栏 `tests/interview-codemirror.test.mjs`（4 项：挂载/提交/切走销毁/无产物兜底） |
| 28 | **ACM 模式（标准输入输出判题）** | 原先只有 LeetCode 核心代码模式，而秋招笔试（牛客/赛码）绝大多数是 ACM 模式——"自己读输入/自己输出/多组用例/EOF 处理"是独立考点。沙箱新增 `mode:"acm"`：vm 内注入 `readline()`（逐行读、耗尽返回 `null`）与 `print()`，按 `io_cases` 逐组比对（行尾空白/末尾空行按 OJ 口径忽略，行内差异不忽略），失败给**逐用例 diff**（输入/期望/实际）；`challenges` 迁移 v5 加 `mode`/`io_cases` 列（存量题保持 `core`，零破坏）；内置 **15 道 ACM 题**（`lib/acm-bank.ts` + `scripts/import-acm-bank.mjs`，多组 A+B / EOF / 前缀和 / DP / 括号匹配 / 取模递推等）。护栏 `tests/acm-mode.test.mjs`（9 项：正解通过 / 错解 diff / **用例间输入隔离** / 归一化口径 / 运行时报错归因 / 缺用例明确报错 / 迁移默认值 / 路由 mode 过滤与回流 / 15 题参考解逐题通过——题库与判题器一起验） |
...
| 30 | **题面解析缺失 + 身份靠标题（用户追问："ACM 题面复杂，只看题目能正确匹配吗？"）** | 查证：**不能保证** —— 实测 `isSimilarTopicForArchive("n 个数求和（单组）","n 个数求和")` = **true**，而清单/复习卡/讲解存档原先都以 topic 字符串为身份，标题只差一个括号就会串题；`markChallengeWrong` 还写死 `手写题·` 前缀（ACM 题答错也标成"手写题"）。修法：① 新增 `lib/acm-statement.ts` 题面解析（按段头切出**输入格式/输出格式/数据范围**，并把多组「样例输入→样例输出」配成**可判题用例**；样例段在下一个段头处收尾，否则"数据范围"会被并进期望输出——实测踩到）；② 迁移 v7 给 `review_cards` 加 `challenge_id`，`addCard`/`addChallengeToPlan` **按题目 id 认亲**（不同 id 绝不合并；无 id 的纯知识点卡保留相似合并防"漂移卡分裂"，有回归断言）；③ 答错建卡前缀走 `challengeTopicFor`（笔试题·/算法题·/手写题·）并落 `challenge_id`；④ 新增「➕ 录入笔试题」：**把牛客/赛码上做过的题整段粘进来** → 解析样例 → 立刻可做题/可讲/可加清单（解析不出样例时如实警告，不假装能判题）。护栏 `tests/acm-statement.test.mjs`（5 项，含**两次反证**：去掉清单 id 判重 / 去掉卡片 id 认亲，都必须转红——第一版护栏因"挑了不会合并的标题"假绿，已修正） |

| 31 | **视觉复核（用户追问"你不是有多模态吗，UI 效果检查了吗"）→ 抓到 2 个真 UI 缺陷 + 3 处工具缺陷** | 此前所有 UI 结论来自 jsdom + 源码断言，**从没真渲染过**。补齐两级：① `scripts/shot-practice.mjs`（真 Chromium + `file://` + mock 桥——不启 Electron、不连运行中的 widget、不写库）出 9 张改动面截图 + 可见性/对比度/点击目标/溢出指标；② `scripts/ui-vision-review.mjs` **把截图交给视觉模型看**（视觉路由取证：本机 ollama 中转的 `deepseek-v4.1-flash:cloud` 虽 `capabilities` 含 vision，但实测原生 `/api/chat` 与 OpenAI 兼容 `/v1` 两种形状**图都到不了模型**（无 vision 的模型则直接 400），DeepSeek 官方 API 也**没有** `deepseek-v4-flash-vision-exp` 这个 id → 改用 OpenRouter）。**视觉复核抓到的真缺陷**：<br>· **面试手写轮 CodeMirror 被压成 39px 窄条**：`.iv-editor` 是 flex 行（为"行号槽+文本域"设计），CM 作为 flex 子项按 min-content 收缩；JS 加了 `.iv-cm-host` 标记类却**没配 CSS**。几何取证（CM 39×240 vs 宿主 1142×240）→ 修 `panel.css`，现 1142×240。护栏 `tests/ui-render-geometry.test.mjs`（真浏览器量宽高；反证：删掉那条 CSS 必红）<br>· **「➕ 录入笔试题」输入框太矮**（`rows=4`，粘整段题面只能看到零星提示）→ `__askText` 加可选 `rows`/`width`（默认值不变，不影响其他调用方），录入处用 `rows:16, width:720`<br>**同一轮修掉我自己的 3 处工具/检查缺陷**：截图脚本没关录入浮层 → 06~09 全被白弹窗遮住（视觉模型报"中央巨大白色弹窗遮挡"才发现）；选择器选到页面里静态 hidden 的浮层/按钮栏子元素 → "元素不可见"误报；对比度检查把渐变背景按钮误判为低对比度（改为跳过 `background-image` 元素，练习/清单面 lowContrast 归 0）。另有 1 条经 DOM 取证**否定**的疑点：React 版"孤立数字 0"其实是 `已完成 0（0%）`（JSX 文本节点拆分所致），非缺陷 |

| 32 | **全量 UI 视觉复核：27 格 Tab×三态逐屏看图**（承接 #31——"所有 UI 效果"不能只查改动面） | 新增 `scripts/shot-all-tabs.mjs`：9 个业务 Tab × 原生/React/Vue = **27 格**逐格真渲染截图 + 机器审计（高度/文本量/元素数/小字/横向溢出/无标签控件/**窄 CodeMirror**）。首轮结果：**容器缺失 0、高度 <60px 0、窄 CM 0、控制台错误 0**（#31 修的 CM 塌陷与 TAB_LABELS 类问题在其他格不复现）。27 张全部交视觉模型逐屏复核后**修掉 2 个真问题**：<br>· **爬取页渲染出字面量 `undefined`**（巡更配置字段缺失时显示 `每 undefined 分钟`，下拉里还有 `undefined 分钟` 选项）→ `loadPatrolConfig`/`renderPatrolStatus` 加防御（非法值给占位符「—」，且不写进 `option`）；护栏 `tests/panel-no-dirty-literals.test.mjs`（"字段残缺不得出现 undefined/NaN/[object Object]" + "正常值必须如实显示"正对照；反证：恢复旧拼接必红）<br>· **筛选行两个同名「全部」**（状态组「📋 全部」与分类组「全部」同形，视觉模型报"「全部」重复、分组难辨"）→ 原生/React 改「全部分类」，React/Vue 难度组改「全部难度」、Vue 状态组改「全部状态」；同护栏盯住"不得有两个同名「全部」"（反证：改回「全部」必红）<br>另 1 条经 DOM 取证**否定**：React"列表空白+孤立数字 0"的 0 其实是 `已完成 0（0%）`（JSX 文本节点拆分），非缺陷<br>**仍未量化的一条**：视觉模型在多个 Tab（kb/review/crawl/dashboard 的框架版等）报"背景壁纸透出、文字对比度偏低"——我的对比度检查按"第一个不透明背景"算，**测不出壁纸透出的情况**（返回 0 条），故只作观察上报；改法（内容区加半透明底板/毛玻璃）属全局外观变更，需产品决策，未擅自改 |

| 33 | **像素级对比度评估 → 修掉"正文压在壁纸上"**（承接 #32 里"测不出壁纸透出"的那条） | 补上能测的方法并据此决策：新增 `scripts/ui-contrast-pixels.mjs`（真 Chromium 截图 → 页面内 canvas 解码 → 按**文本行紧贴矩形**采样：背景取矩形**外环 6px 亮度众数**（不受字形占比影响）、字色取矩形内背离背景的 2% 分位极值，按 WCAG AA 阈值判定）。**踩坑记录**：第一版按"元素外框 + 分位数"测，小元素里字形占比不足 5% → 分位数取到背景、比值假性 ≈1.0，还得出"加底板反而更差"的错误结论；改成紧贴文本行 + 外环背景后才可信（自检时 `渲染层：` 2.53 等低值也符合肉眼判断）。**评估结论**：`.tab-panel` 原先没有任何背景，正文直接压在 body 动漫壁纸上 → 103 个文字元素里 **46 个低于 AA**（中位 4.2~5.5，最差 1.5）——这是**可读性缺陷而非审美偏好**。逐档实测选型：0.60+blur6 → 14 个不达标 / 0.72+blur4 → 6 / **0.82+blur3 → 5** / 0.90 全不透明 → 0（但壁纸被盖住）。取 **0.82+blur3**（与面板既有表面 `.rf-card` 等的半透明白+毛玻璃同一视觉语言，只加背景/圆角不加 padding 以免动布局）→ 落地复测 **1/136 不达标**，14 屏全面审计 **8/223（4%）**，残留全是 emoji/角标临界值（`🔥🔥🔥` 3.1、`✍️手写` 4.1、`简单` 4.25）。视觉复核复看 6 个原被点名的屏：**"对比度低/文字看不清"的投诉全部消失**（只剩 mock 空数据导致的留白） |

| 34 | **对比度整改收尾：残余不达标清零 + 回归护栏**（"全修复完"） | 承接 #33，把剩下的不达标项逐个修到过线：① 绿色系文字（分类/难度角标 `#2f7d4e`/`#2f7a4a`、回退编辑器语法高亮）在浅底上仅 4.1~4.3 → 统一加深为 `#1f6b3f`；② 琥珀色（中等难度/输出标签/tip）`#9a5b00` → `#7d4a00`；③ 复习阶段徽标 `#5d48b8` → `#4a35a0`（实测 4.43）；④ 复习队列 chip 蓝 `#2563eb` → `#1d4ed8`、紫 `#7c3aed` → `#6d28d9`；⑤ 面试历史「🗑 删除」原先**没有任何样式**（默认按钮色实测 2.04）→ 补 `.iv-hist-del` 样式（红字+浅红底）；⑥ 筛选 chips 行间距 4px → 6px（视觉模型报"拥挤"，实测无溢出，属观感微调）。**同时给审计工具加了一条明确口径**：纯 emoji 样本不计入文字对比度——WCAG 的对比度要求针对**文字**，emoji 是字形图（`🔥🔥🔥` 3.1、`🗑` 2.0 都是字形自带配色、CSS 改不动），含文字+emoji 的样本（如「🆕 首次」）照常测量。**最终像素实测：14 屏 219 个文字样本 0 个不达标（0%）**（整改前 46/103）。新增护栏 `tests/ui-contrast.test.mjs`（真 Chromium；① 底板必须存在且不透明度 ≥0.7 带毛玻璃 ② 6 个关键屏像素实测不达标数 ≤2；**反证：删掉底板两条都转红**）。另有 3 条经 DOM 取证**否定**的视觉模型疑点：React"列表只显示数字 0"（实为 `已完成 0（0%）`，列表正常渲染）、面试页"说明文字对比度低"（实测字色 `rgb(45,42,69)` 深色）、"筛选标签溢出"（实测 `flex-wrap: wrap` 且无溢出）。同轮修掉一处**测试假红**：`panel-challenges` 里钉死了旧绿色 `#2f7d4e`，随改色同步更新为 `#1f6b3f`（并在注释里写明这是对比度整改的结果） |

| 35 | **167/448 道题"点判题恒失败"**（用户点得到的功能缺陷）+ **复习卡去做题入口** | ① 查证：core 448 题里 **167 题 `test_code` 为空**（链表/树题 74、示例参数不可解析 62、无示例 12）。**P1 原记的"`scripts/gen-challenge-tests.mjs` 可补"经实测不成立**——对这批题**可生成 0 条**（该脚本只能处理"示例为 JSON 字面量 + 非链表/树"的题），所以"自动补测试"这条路走不通。改为**诚实修法**：`lib/ai-career.ts` 列表/详情新增 `judgeable`（core 看 `test_code`、acm 看 `io_cases`）；`/api/challenges/run` 对不可判题的题**不跑沙箱**、如实返回"本题暂无自动判题用例（缺 test_code；多为链表/树题或示例参数无法自动解析）"——**不再给"测试未执行（可能骨架函数名与测试不匹配）"这种误导性归因**；三态面板在 `judgeable === false` 时不渲染「▶ 运行判题」、改为明确说明（原生列表另加「🧩 仅手动」徽标），保留「✅ 已会 / ❌ 不会」记录。② 复习卡（答错自动建卡）已带 `challenge_id` 但复习 Tab 没有回题入口 → 卡片头加「✍️ 去做题」，并把 `gotoChallenge` 改成**模式未知时 core→acm 回退查找**（复习卡只存题目 id）；顺带修 `scrollIntoView` 在无该 API 环境抛错的隐患。护栏 `tests/challenge-judgeable.test.mjs`（3 项，含"可判题的题必须仍走正常沙箱"正对照）与 `tests/review-go-challenge.test.mjs`（2 项，行为级跑通跨模式回退 + 编辑器自动展开）；**反证各 2 次全红**。同轮修掉 1 条**旧断言**（`study-acm-explain` 钉死了重构前的 `gotoChallenge` 实现串） |

| 36 | **框架版面试「代码作答」三态对齐**（承接 #31 的三态 CodeMirror） | 查证后发现真差距**不在**"后端没传 answerMode"（我此前的判断是错的）：React 版 `panel.jsx` **已识别** `answerMode="code"` 但仍是 textarea（手搓行号列 + 手写 Tab/缩进），Vue 版 `Interview.vue` **完全不识别** `answerMode`（一直是纯 textarea）。修法：两态都改为**复用原生同一个 CodeMirror 产物**（调用 `panel-rest.js` 暴露的全局 `loadPracticeEditor`，不各自打包 CM——避免体积翻倍与版本漂移）；`answer` 仍是唯一数据源（CM 的 `onChange` 写回，提交链路零改动）；**CM 挂载失败（产物缺失/初始化异常）回退 textarea**，绝不留空白作答区（React 用 `cmFailed` state、Vue 用 `cmFailed` ref）；轮次/模式切换销毁旧实例（React `useEffect` cleanup、Vue `watch([isCode, round])` + `onBeforeUnmount`）；**删掉手搓行号列**（否则与 CM 自带行号槽叠成两条）。护栏 `tests/interview-framework-codemirror.test.mjs`（4 项：三态共用同一产物 / React / Vue / 无手搓行号；**反证 2 次全红**）。另记两条**测试环境**观察：① 浏览器类护栏（几何/对比度/tab 登记）并行跑会因 Chromium 争用把本地全套拖长甚至卡住 → 本地建议 `--test-concurrency=1`（CI 无浏览器二进制，这些用例自动跳过，CI 一直绿）；② `lane.test.mjs` 的"慢任务不阻塞队列进度"是**时序敏感**用例，并行重负载下会偶发假红（单独跑全绿，实测确认） |
...
- **面板**（**10 个 Tab**：🎤 面试 / 🔁 复习 / 📋 学习清单 / 💬 对话 / 🔍 爬取 / 🏢 校招 / ✍️ 专项练习 / 📊 驾驶舱 / 🧠 知识库 + 由右上角 ⚙️ 进入的设置 Tab）——**9 个业务 Tab** 右上角可切换**原生 / React / Vue 三态**渲染层（含包体积对比卡，数据来自 dist 实测）；⚙️ 设置 Tab 仅原生实现（不参与三态切换）。其中「✍️ 专项练习」为 2026-09-16 新增：题库从「校招」Tab 独立出来，三态统一 **CodeMirror 6** 判题编辑器（原生按需注入 `practice-editor.bundle.js`，缺失时回退 textarea）
...
│   └── renderer/                   $110$29 个业务 Tab（原生/React/Vue 三态）+ ⚙️ 设置（仅原生）
├── plugins/                        # ── 插件目录 ──
...
|---|---|---|---|---|
| 原生 | 原生 JS + **esbuild** 单入口 | 全部 10 Tab（对照基线；设置 Tab 只有原生） | **376.8KB**（gzip 115.4KB / 6 文件；判题编辑器是 494KB 的按需产物，不进首屏） | file:// 加载不需 dev server/HMR；零依赖启动快 |
| React 版 | **Vite 子项目**（`panel-react/`，vite 7） | 9/9（面试·驾驶舱·知识库·学习·爬取·校招·专项练习·对话·复习） | **748.6KB**（gzip 248.6KB，含 React 运行时 + CodeMirror 6） | 交互密集：**useReducer Phase 状态机** + useMemo 派生缓存 + useDeferredValue 搜索 |
$19/9（同上） | **669.2KB**（gzip 229.6KB，含 Vue 运行时 + CodeMirror 6） | 数据可视化：**响应式 computed 曲线缓存** + watch 动画 + Transition 切卡 |
...
- **渲染层三态并行**：同一业务层 + 同一 IPC 桥上的三套实现（原生 / React / Vue），覆盖 9 个 Tab×2 框架；`gen:sizes` 从 dist 实测包体积（原生 376.8KB / React 748.6KB / Vue 669.2KB——**2026-09-16 三态统一 CodeMirror 6 判题编辑器**后 React/Vue 明显变大，代价换来真编辑器：语法高亮/括号匹配/自动缩进/多光标/搜索/补全；框架包按 M9 按需加载，不进首屏），注册表一致性 + 两侧全 Tab 渲染测试 + 体积数据新鲜度三重护栏
