...
│   （全量 TS 迁移已完成：`lib` 122 `.ts` / 66 `.mjs`（其中 **63 个是同名一行桶**，另 3 个是设计上的 re-export 桶 `interview/study/tools/impl`）；
│    加上 `plugins` 14 `.ts` / 14 桶、`desktop` 11 `.ts` / 7 桶 —— **实现文件全在 `.ts`，`.mjs` 只剩 `export *` 一行桶**，
│    因此按 `.mjs` 路径加载的调用方（插件协议入口、Electron `main`、`#lib/*` 子路径、Vite 子项目）全部零改动）
...
|---|---|---|---|---|
| 原生 | 原生 JS + **esbuild** 单入口 | 全部 9 Tab（对照基线） | **349KB**（gzip 107KB / 6 文件） | file:// 加载不需 dev server/HMR；零依赖启动快 |
...
- **渲染层三态并行**：同一业务层 + 同一 IPC 桥上的三套实现（原生 / React / Vue），覆盖 8 个 Tab×2 框架；`gen:sizes` 从 dist 实测包体积（原生 349KB / React 242KB / Vue 158KB），注册表一致性 + 渲染测试 + 体积新鲜度三重护栏
