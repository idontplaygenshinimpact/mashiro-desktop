...
export async function prepareProjectInterview({ project, force = false } = {}) {
  ...
  try {
    ...
    // 4. LLM 生成完整面试准备文档（真实面试方向：背诵路线图/定位/决策 + 核心文件详细 + 八股详细可背 + 拷打全覆盖）
    const doc = await runSubagent({
      name: "面试准备文档生成",
      task: `基于以下项目源码要点与八股清单，生成一份**完整的项目面试准备文档**（真实面试方向——质量标准：能通过模拟面试——内容详细、可直接背诵、拷打全覆盖）。格式：
# 项目·${name} 面试准备
## 背诵路线图（时间紧 1 天/3 天/1 周——分别背什么——面试前 1 小时过什么）
## 一句话定位（30 秒开场——项目是什么/解决什么问题/技术栈）
## 关键设计决策速览（5-8 条——面试开场可用——为什么选 X 不用 Y）
## 项目概览（技术栈/架构/目录结构/核心模块/数据流）
## 源码要点（**核心文件详细**——每个 200-400 字（核心职责/关键实现/可能的坑——函数名/算法细节）；
   **其他文件速览**（一行）——真实面试拷打核心模块，不是每个文件）
## 涉及的全部八股（每个知识点**详细可背**）：
  ### <知识点>
