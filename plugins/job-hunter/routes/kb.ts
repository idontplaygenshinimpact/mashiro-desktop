...
// 全量 TS 升级工单阶段 4（插件）：实现迁至 kb.ts，kb.mjs 保留同名薄桶（插件按路径加载 → 入口与调用方零改动）
...
export function registerKbRoutes(router: Router): void {
  // ---------- 方向画像 ----------
  router.route("/api/career/profile", "GET", (req: IncomingMessage, res: ServerResponse) => {
    res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
    res.end(JSON.stringify({ ok: true, profile: getCareerProfile() }));
  });
  router.route("/api/career/profile", "POST", (req: IncomingMessage, res: ServerResponse) => {
  ...
  // ---------- 可配置知识树 ----------
  router.route("/api/knowledge/tree", "GET", (req: IncomingMessage, res: ServerResponse) => {
    res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
    res.end(JSON.stringify({ ok: true, tree: knowledgeApi.getKnowledgeTree(), isDefault: knowledgeApi.isDefaultTree() }));
  });
  router.route("/api/knowledge/tree", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, (body: string) => {
      try {
        const j = JSON.parse(body || "{}");
  ...
  router.route("/api/knowledge/templates", "GET", (req: IncomingMessage, res: ServerResponse) => {
    try {
      res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
  ...
  router.route("/api/knowledge/load-template", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, (body: string) => {
      try {
  ...
  // ---------- 薄弱点 / 掌握度 ----------
  router.route("/api/weak-points", (req: IncomingMessage, res: ServerResponse) => {
    try {
  ...
  router.route("/api/weak-points/to-plan", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, (body: string) => {
      try {
  ...
  router.route("/api/iv-focus-sources", async (req: IncomingMessage, res: ServerResponse) => {
  ...
  router.route("/api/mastery", (req: IncomingMessage, res: ServerResponse) => {
    try {
  ...
  // ---------- 知识库检索/问答/统计/重建 ----------
  router.route("/api/knowledge/ask", "POST", (req: IncomingMessage, res: ServerResponse) => {
  ...
  router.route("/api/knowledge/search", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, async (body: string) => {
      try {
  ...
  router.route("/api/knowledge/stats", (req: IncomingMessage, res: ServerResponse) => {
    try {
      res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
  ...
  router.route("/api/knowledge/paragraphs/search", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, async (body: string) => {
      try {
        const { query, topK } = JSON.parse(body || "{}");
  ...
  router.route("/api/knowledge/paragraphs/stats", (req: IncomingMessage, res: ServerResponse) => {
    try {
      res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
  ...
  router.route("/api/knowledge/paragraphs/search-reranked", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, async (body: string) => {
      try {
        const { query, topK } = JSON.parse(body || "{}");
        try { kbApi.indexStudyNotes(); } catch { /* 索引失败不影响检索 */ }
  ...
  router.route("/api/knowledge/followups-to-cards", "POST", (req: IncomingMessage, res: ServerResponse) => {
  ...
  router.route("/api/knowledge/rebuild", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, async (_body: string) => {
      try {
  ...
  // ---------- 官方文档版本检测 ----------
  router.route("/api/learning", (req: IncomingMessage, res: ServerResponse) => {
    try {
      res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
  ...
  router.route("/api/learning/check", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, async (body: string) => {
      try {
  ...
  router.route("/api/learning/project", "GET", (req: IncomingMessage, res: ServerResponse) => {
    try {
  ...
  router.route("/api/learning/project", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, (body: string) => {
      try {
  ...
  router.route("/api/self-assess/status", "GET", (req: IncomingMessage, res: ServerResponse) => {
  ...
  router.route("/api/self-assess", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, (body: string) => {
      try {
