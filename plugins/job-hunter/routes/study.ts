 ...
 */
function findSimilarArchive(topic: string, { excludeId, items }: ArchiveLookupOpts = {}) {
 ...
 */
function findEarlierArchive(topic: string, ownBirth: number, { excludeId, items }: ArchiveLookupOpts = {}) {
  const list = items || (studyApi.getPlan().items || []);
  let best = null;
  for (const other of list) {
    if (!other || other.id === excludeId) continue;
...
function findSourceFile(source: unknown): string | null {
...
function archiveHeader(item: StudyItemLike, note = ""): string {
...
// 全量 TS 升级工单阶段 4（插件）：实现迁至 study.ts，study.mjs 保留同名薄桶（插件按路径加载 → 入口与调用方零改动）
...
/** registerStudyRoutes 选项（宿主注入取源回调与 lane 队列；缺省 * 通配 + 直通） */
export interface StudyRoutesOpts {
  getCorsOrigin?: (req: IncomingMessage) => string;
  laneSubmit?: (fn: () => any) => any;
}

/** 讲解复用查找的入参（排除自身 + 可选条目列表覆盖） */
interface ArchiveLookupOpts {
  excludeId?: string;
  items?: Array<{ id: string; topic: string }>;
}

/** 清单条目最小形状（讲解/追问路径只读这几个字段；与 lib/study-store.ts 的 PlanItem 结构兼容） */
interface StudyItemLike {
  id?: string;
  topic?: string;
  why?: string | null;
  source?: string | null;
...
export function registerStudyRoutes(router: Router, { getCorsOrigin = (_req: IncomingMessage) => "*", laneSubmit = (fn: () => any) => fn() }: StudyRoutesOpts = {}): void {
  ...
  const makePush = (res: ServerResponse) => createSSEPush(res, { eventSchema: StudyStreamEvent }).push;
  ...
  router.route("/api/study-note/reset", "POST", (req: IncomingMessage, res: ServerResponse) => {
  ...
  router.route("/api/study-detail-stream", (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (filePath) {
      ...
      try {
        ...
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "读取讲解失败: " + eMsg(e) }));
        return;
      }
    }
    ...
    import("#lib/ai.ts").then(async ({ solveQuestionStream, withLLMTimeout }) => {
      ...
      full = await ((withLLMTimeout as (...args: any[]) => Promise<string>)(
        solveQuestionStream({
          title: ep.title,
      ...
      push({ type: "error", error: eMsg(e) });
      res.end();
    });
  });

  router.route("/api/study-append-stream", (req: IncomingMessage, res: ServerResponse) => {
      ...
      try {
        full = await ((withLLMTimeout as (...args: any[]) => Promise<string>)(
          solveAppendStream({
            ...
            question,
            ref: ref ?? undefined, // B2①：引用段落注入【你引用的内容】段（tail 区——前缀稳定不破坏）
          }, (delta) => {
            full += delta;
        ...
        push({ type: "error", error: eMsg(e).slice(0, 120) });
        res.end();
        return;
      }
  ...
  router.route("/api/study-consolidate-stream", (req: IncomingMessage, res: ServerResponse) => {
  ...
  router.route("/api/study-cluster-stream", (req: IncomingMessage, res: ServerResponse) => {
    ...
    readBody(req, res, (body: string) => {
      try {
        ...
        const topics: Array<{ topic: string; content: string }> = [];
  ...
  router.route("/api/study/note", (req: IncomingMessage, res: ServerResponse) => {
    try {
      const u = new URL(String(req.url), `http://127.0.0.1:${PORT}`);
  ...
  router.route("/api/study-detail", (req: IncomingMessage, res: ServerResponse) => {
    ...
    import("#lib/ai.ts").then(async ({ solveQuestion }) => {
      ...
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: "生成讲解失败: " + eMsg(e) }));
    });
  });

  router.route("/api/study-generate", (req: IncomingMessage, res: ServerResponse) => {
    // 从产出生成学习清单
  ...
  router.route("/api/study-review", (req: IncomingMessage, res: ServerResponse) => {
    // 复盘：出验证题
    studyApi
      .startReview()
      .then((r) => {
  ...
  router.route("/api/study-answer", (req: IncomingMessage, res: ServerResponse) => {
    // 复盘：提交答案判分
  ...
  router.route("/api/study-notes", async (req: IncomingMessage, res: ServerResponse) => {
    try {
  ...
  router.route("/api/study-notes/to-plan", "POST", (req: IncomingMessage, res: ServerResponse) => {
    readBody(req, res, async (body: string) => {
      try {
        ...
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
        // learnOneNote/learnAllNotes 恒带 ok 字段（返回类型 required）→ 原 ok: true 必被覆盖，等价
        res.end(JSON.stringify({ ...r }));
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: eMsg(e) }));
