...
// 全量 TS 升级工单阶段 3：lib/knowledge-base.mjs → .ts（plugins/job-hunter/routes/kb.mjs 与 tests 按 .mjs 路径加载 → 保留同名一行桶）
...
/** 段落（主文段 / 追问段） */
export interface StudyParagraph { section: string; kind: "main" | "followup"; content: string }
/** 检索命中（段落级） */
export interface ParagraphHit { id: string; docId: string; section: string; kind: string; content: string; score: number; rerankScore?: number }
/** 段落统计（知识库 Tab） */
export interface ParagraphStats { total: number; followups: number; docs: number; withVectors: number; missingVectors: number }
/** 向量回填结果 */
export interface VectorBackfillResult { embedded: number; remaining: number; modelAvailable: boolean }
/** transformers.js 模型可调用面（向量腿/精排腿各自的入参形状） */
type EmbedderFn = (text: string, opts: { pooling: string; normalize: boolean }) => Promise<{ data: ArrayLike<number> }>;
type RerankerFn = (pairs: string[][], opts: { topk: number }) => Promise<Array<Array<{ score?: unknown }>>>;
type ModelFn = EmbedderFn | RerankerFn;
 ...
 * @returns 生效后的配置
 */
export function applyTransformersEnv(env: Record<string, unknown>, envVars: Record<string, string | undefined> = process.env): { cacheDir: string; remoteHost: string } {
  ...
  return { cacheDir: String(env.cacheDir), remoteHost: String(env.remoteHost || "https://huggingface.co/") };
}

/** 加载 transformers 模型（先应用 env 配置——向量与精排共用） */
async function loadModel(task: string, model: string, opts: Record<string, unknown>): Promise<ModelFn> {
  const mod = await import("@xenova/transformers") as unknown as { pipeline: (...args: unknown[]) => Promise<unknown>; env: Record<string, unknown> };
  const { pipeline, env } = mod;
  applyTransformersEnv(env);
  return await pipeline(task, model, opts) as ModelFn;
}
...
export function splitStudyNote(text: unknown): StudyParagraph[] {
  const out: StudyParagraph[] = [];
  const t = String(text || "");
  // 1) 追问段（先切——主文切分时排除）
  const followups: StudyParagraph[] = [];
  const mainParts: string[] = [];
  let last = 0;
  for (const m of t.matchAll(FOLLOWUP_RE)) {
    const idx = m.index ?? 0;
    mainParts.push(t.slice(last, idx));
...
function getIndexedMtimes(): Record<string, { m?: number; s?: number }> {
  try {
    const row = db.prepare("SELECT value FROM settings WHERE key=?").get(MTIMES_KEY) as { value?: unknown } | undefined;
    return row ? JSON.parse(String(row.value)) as Record<string, { m?: number; s?: number }> : {};
  } catch { return {}; }
}

/** 增量索引 study_notes 文档（mtime + 段落数变化才重刷） */
export function indexStudyNotes(): { added: number; removed: number } {
...
let embedder: ModelFn | null = null; // 缓存常驻（用户主动用知识库，反复检索是预期行为）
let embedderLoading: Promise<ModelFn | null> | null = null;
async function getEmbedder(): Promise<ModelFn | null> {
...
async function embedText(text: unknown): Promise<Float32Array | null> {
  try {
    const model = await getEmbedder();
    if (!model) return null;
    const out = await (model as EmbedderFn)(String(text || "").slice(0, 500), { pooling: "mean", normalize: true });
...
let vectorBackfillRunning: Promise<{ embedded: number; remaining: number }> | null = null;

/** 还没向量化的段数（KB 检索质量的可观测量） */
export function countMissingVectors(): number {
  try {
    const row = db.prepare("SELECT COUNT(*) AS n FROM knowledge_paragraphs WHERE vector IS NULL").get() as { n?: unknown } | undefined;
    return Number(row?.n || 0);
  } catch { return 0; }
}

/** 向量回填（有界：单次最多 limit 段；模型不可用 → 立即返回且不写空向量） */
export async function backfillVectors({ limit = 64, embed = embedText }: { limit?: number; embed?: (text: string) => Promise<Float32Array | null> } = {}): Promise<VectorBackfillResult> {
  let rows: Array<Record<string, unknown>>;
  try {
    rows = db.prepare("SELECT id, content FROM knowledge_paragraphs WHERE vector IS NULL ORDER BY doc_id, seq LIMIT ?").all(limit) as Array<Record<string, unknown>>;
 ...
 *  @returns 已有任务在跑 → null */
export function scheduleVectorBackfill(): Promise<{ embedded: number; remaining: number }> | null {
...
function bm25Search(query: unknown, topK: number): Array<{ id: string; score: number }> {
  try {
    ...
    ).all(match, topK * 3) as Array<Record<string, unknown>>;
    for (const r of rows) out.push({ id: String(r.id), score: -Number(r.score) }); // bm25 负分，取反
    // 2 字词 LIKE 兜底（trigram 3 字窗口不匹配 2 字词）
    for (const w of words) {
      if (w.length > 2) continue;
      const likeRows = db.prepare("SELECT id FROM knowledge_paragraphs WHERE content LIKE ? LIMIT ?").all(`%${w}%`, topK * 2) as Array<Record<string, unknown>>;
...
async function vectorSearch(query: unknown, topK: number): Promise<Array<{ id: string; score: number }>> {
  const qv = await embedText(query);
  if (!qv) return [];
  try {
    const rows = db.prepare("SELECT id, vector FROM knowledge_paragraphs WHERE vector IS NOT NULL").all() as Array<Record<string, unknown>>;
...
function rrfMerge(lists: Array<Array<{ id: string; score: number }>>, k = 60): Array<{ id: string; score: number }> {
 ...
 */
export async function searchParagraphs(query: unknown, topK = 8): Promise<ParagraphHit[]> {
  ...
  const followupIds = new Set(
    (db.prepare("SELECT id FROM knowledge_paragraphs WHERE kind='followup'").all() as Array<Record<string, unknown>>).map((r) => String(r.id))
  );
  ...
  return top
    ...
    .filter((h): h is ParagraphHit => !!h);
}

/** 段落统计（知识库 Tab 展示） */
export function getParagraphStats(): ParagraphStats {
  try {
    ...
    const n = (sql: string): number => Number((db.prepare(sql).get() as { n?: unknown } | undefined)?.n || 0);
...
let reranker: ModelFn | null = null;
let rerankerLoading: Promise<ModelFn | null> | null = null;
async function getReranker(): Promise<ModelFn | null> {
 ...
 */
export async function rerankParagraphs(query: unknown, candidates: Array<{ id: string; content: string }>, topK = 5): Promise<Array<{ id: string; score: number }>> {
  ...
  try {
    ...
    const out = await (model as RerankerFn)(pairs, { topk: 1 });
...
export async function searchParagraphsReranked(query: unknown, topK = 5): Promise<ParagraphHit[]> {
...
export async function followupsToReviewCards(): Promise<{ added: string[] }> {
  const dir = studyNotesDir();
  const added: string[] = [];
  try {
