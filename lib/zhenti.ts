...
// 登录态：常驻 headed Edge 会话（首次扫码，见 ensureSession）；也支持手工 Cookie（data/nowcoder-cookie.json）
// 全量 TS 升级工单阶段 3：lib/zhenti.mjs → .ts（job-collect/插件路由/tests 按 .mjs 路径加载 → 保留同名一行桶）
import { readFileSync, writeFileSync, existsSync } from "node:fs";
import path from "node:path";
import { db, ensureColumn } from "./db.mjs";
import { clean } from "./text-utils.ts"; // 技术债 L3：收敛单点
import type { BrowserContext, Page } from "playwright";

// 打包版：MIANSHI_DATA_DIR 由主进程注入（asar 只读，登录态/Edge profile 需可写目录）
const DATA_DIR = (): string => process.env.MIANSHI_DATA_DIR || path.join(import.meta.dirname, "..", "data");
const COOKIE_FILE = (): string => path.join(DATA_DIR(), "nowcoder-cookie.json");
...
`);
// 迁移：旧表补 kind 列（必须先于 kind 索引创建；架构 P0-2：统一 ensureColumn 幂等补列）
ensureColumn("exam_papers", "kind", "TEXT DEFAULT 'real'");
try { db.exec("CREATE INDEX IF NOT EXISTS idx_exam_kind ON exam_papers(kind)"); } catch { /* ignore */ }

/** Cookie 对 */
export interface CookiePair { name: string; value: string }
/** 试卷清单行（面板展示用） */
export interface ZhentiPaper {
  id: string;
  test_id: string;
  kind: string;
  company: string;
...
/** 抓到的题目（提交给面板的形态，不含答案） */
export interface ZhentiQuestion { type: string; title: string }
/** 抓题结果 */
export interface FetchPaperResult { ok: boolean; questions: ZhentiQuestion[]; saved?: number; error?: string }
/** 牛客 API 返回（宽松：code/data/msg 动态） */
interface NowcoderApiResponse { code?: number; msg?: unknown; error?: unknown; data?: { testId?: unknown; paperQuestionDetails?: Array<Record<string, unknown>> } }

/** 保存牛客 Cookie（用户浏览器复制 Cookie 头字符串，本地落盘不入 git） */
export function saveNowcoderCookie(cookieStr: unknown): { ok: boolean; count?: number; error?: string } {
...
export function getNowcoderCookie(): CookiePair[] | null {
  try {
    if (!existsSync(COOKIE_FILE())) return null;
    const j = JSON.parse(readFileSync(COOKIE_FILE(), "utf8")) as { pairs?: CookiePair[] };
    return j?.pairs?.length ? j.pairs : null;
  } catch { return null; }
}

// 抓题互斥：同一时刻只允许一个抓题任务（不同试卷并发会竞争同一 Edge profile 目录锁，且 DB 写入需串行）
let fetchLock: Promise<unknown> = Promise.resolve();
/** 串行化抓题任务（后续调用排队，互不并发） */
function withFetchLock<T>(fn: () => Promise<T>): Promise<T> {
...
export function fetchPaperQuestions(paperTestId: unknown): Promise<FetchPaperResult> {
...
const SESSION_PROFILE = (): string => path.join(DATA_DIR(), "nowcoder-edge-profile");
let sessionCtx: BrowserContext | null = null;
...
async function sessionFetch(page: Page, apiPath: string, body: unknown): Promise<NowcoderApiResponse> {
  return page.evaluate(async ([p, b]: [string, unknown]) => {
    ...
    return r.json();
  }, [apiPath, body] as [string, unknown]) as Promise<NowcoderApiResponse>;
}

/** 题目 HTML → 纯文本（公式图取 alt LaTeX，保留换行） */
export function cleanQuestionHtml(html: unknown): string {
  return String(html || "")
    .replace(/<img[^>]*alt="([^"]*)"[^>]*>/g, " $1 ")
    .replace(/<br\s*\/?>/gi, "\n")
    .replace(/<[^>]+>/g, "")
...
async function fetchPaperQuestionsInner(paperTestId: unknown): Promise<FetchPaperResult> {
  ...
  if (r.code !== 0) return { ok: false, questions: [], error: `begin 失败: ${String(r.msg || r.error || r.code)}` };
  ...
  const questions = details.map((q) => {
    ...
    const coding = (q.codingProblem || {}) as { inputDesc?: unknown; outputDesc?: unknown; timeLimit?: unknown; memoryLimit?: unknown };
    const extra = coding.inputDesc
      ? `\n\n【输入描述】${cleanQuestionHtml(coding.inputDesc)}\n【输出描述】${cleanQuestionHtml(coding.outputDesc || "")}\n【限制】时间 ${String(coding.timeLimit)}s / 内存 ${String(coding.memoryLimit)}MB`
      : "";
    return { type: qType, title: (cleanQuestionHtml(q.content || "") + extra).slice(0, 3000), qId: String(q.id) };
  });
  // 入库（事务：先删后插，防重复行）
  db.exec("BEGIN");
  try {
    db.prepare("DELETE FROM exam_questions WHERE paper_test_id=?").run(String(paperTestId));
    const ins = db.prepare("INSERT INTO exam_questions (id, paper_test_id, q_index, q_type, title, created_at) VALUES (?,?,?,?,?,?)");
    questions.forEach((q, i) => {
      ins.run(`eq_${now.toString(36)}_${i}`, String(paperTestId), i + 1, q.type, q.title, now);
    });
    db.exec("COMMIT");
...
// clean 从 text-utils.ts 导入（技术债 L3：收敛单点）
...
/** 清单搜集结果 */
export interface CollectZhentiListResult { total: number; added: number; dup: number; skipped: number; papers: Array<{ kind: string; company: string; title: string }> }

/** 收集真题/模拟卷清单：抓公司真题页 → 双轨收录（大厂技术真题 + 平台模拟卷）→ 入库（去重：testId） */
export async function collectZhentiList(): Promise<CollectZhentiListResult> {
  ...
  const out: Array<{ kind: string; company: string; title: string }> = [];
...
/** 真题清单（可按公司/类型过滤） */
export function getZhentiList({ company, kind, limit = 200 }: { company?: string; kind?: string; limit?: number } = {}): ZhentiPaper[] {
  const conds: string[] = [];
  const args: string[] = [];
  if (company) { conds.push("company=?"); args.push(company); }
  if (kind) { conds.push("kind=?"); args.push(kind); }
  const where = conds.length ? "WHERE " + conds.join(" AND ") : "";
  const rows = db.prepare(`SELECT * FROM exam_papers ${where} ORDER BY updated_at DESC LIMIT ?`).all(...args, limit) as Array<Record<string, unknown>>;
  return rows.map((r) => ({
    id: String(r.id),
...
export function getZhentiStats(): { total: number; byKind: Array<Record<string, unknown>>; byCompany: Array<Record<string, unknown>> } {
  const total = Number((db.prepare("SELECT COUNT(*) n FROM exam_papers").get() as { n?: unknown } | undefined)?.n) || 0;
  const byKind = db.prepare("SELECT kind, COUNT(*) n FROM exam_papers GROUP BY kind").all() as Array<Record<string, unknown>>;
  const byCompany = db.prepare("SELECT company, COUNT(*) n FROM exam_papers WHERE kind='real' GROUP BY company ORDER BY n DESC").all() as Array<Record<string, unknown>>;
 ...
 */
export async function addWrongQuestion({ paperId, company, paperTitle, question, answer }: { paperId?: unknown; company?: unknown; paperTitle?: unknown; question?: unknown; answer?: unknown }): Promise<{ ok: boolean; added: number; topic: string }> {
  const { addPlanItems } = await import("./study.mjs");
  const { review } = await import("./review.mjs");
  const title = clean(paperTitle, 40);
  const topic = `真题错题·${String(company || "笔试")}` + (title ? `·${title.slice(0, 16)}` : "");
  // 1) 学习清单（必会，验证题=题干，学习闭环优先覆盖）
  const r = addPlanItems([{
    topic,
    why: `牛客官方真题练习错题（试卷 ${String(paperId || "?")}），需优先补强`,
    source: "牛客真题",
    verify_question: clean(question, 200) || `请完整回答并讲清原理：${topic}`,
    level: "必会",
  }]) as { added?: number } | null | undefined;
...
export async function addPaperToPlan(testId: unknown): Promise<{ ok: boolean; added?: number; topic?: string; error?: string }> {
  const row = db.prepare("SELECT kind, company, title, question_count, url FROM exam_papers WHERE test_id=?").get(String(testId)) as { kind?: unknown; company?: unknown; title?: unknown; question_count?: unknown; url?: unknown } | undefined;
  if (!row) return { ok: false, error: "试卷不存在（先搜集真题）" };
  const { addPlanItems } = await import("./study.mjs");
  const label = row.kind === "simulate" ? "模拟卷" : "真题";
  const topic = `真题·${String(row.company || "平台")}·${clean(row.title, 26)}`;
  const r = addPlanItems([{
    topic,
    why: `牛客官方${label}（${row.question_count ? "共 " + String(row.question_count) + " 题" : "题型待抓"}），练完用「记错题」回流错题`,
    source: "牛客真题",
    verify_question: `去牛客练习：${String(row.url)}（打开即练）\n练习要求：限时完成 → 做错的题用面板「❌ 记错题」回流学习清单 + 复习卡`,
    level: "必会",
  }]) as { added?: number } | null | undefined;
  return { ok: true, added: r?.added || 0, topic };
}
...
export async function collectZhentiByCompany(company: unknown): Promise<{ ok: boolean; added?: number; dup?: number; papers?: Array<{ title: string }>; error?: string }> {
  ...
  try {
    ...
    const links = await page.evaluate(() =>
      Array.from(document.querySelectorAll("a[href*='/exam/test/']")).map((a) => ({ t: (a.textContent || "").trim(), h: (a as HTMLAnchorElement).href }))
    );
    const now = Date.now();
    let added = 0, dup = 0;
    const papers: Array<{ title: string }> = [];
...
export async function collectZhentiDetails(limit = 20): Promise<Array<Record<string, unknown>>> {
  const { fetchPage } = await import("./fetch-page.mjs");
  const rows = db.prepare("SELECT id, url FROM exam_papers WHERE question_count IS NULL ORDER BY found_at DESC LIMIT ?").all(limit) as Array<Record<string, unknown>>;
  const results: Array<Record<string, unknown>> = [];
  await Promise.all(
    rows.map(async (r) => {
      try {
        const page = await fetchPage(String(r.url), { maxTextChars: 2000, waitMs: 3000 }) as { text?: string };
        ...
        results.push({ id: String(r.id), error: (e instanceof Error ? e.message : String(e)).slice(0, 60) });
      }
    })
  );
  return results;
