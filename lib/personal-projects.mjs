...
/**
 * 档案 → 面试用简报（用户反馈 2026-09：面试官拿着本地源码刨到文件名/函数实现，真实面试不会这么问）
 * 剥掉【核心源码预览】（实现细节），【源码结构】只留前 12 行——保留"项目是否真实 + 技术栈是否对得上"的核对能力。
 * 出题依据以简历为准，源码仅用于校验简历没吹牛。
 */
export function archiveBrief(content, maxLen = 1200) {
  let text = String(content || "").split("【核心源码预览】")[0];
  text = text.replace(/(【源码结构】\n)([\s\S]*?)(?=\n【|$)/, (_m, head, body) => {
    const lines = String(body).trim().split("\n").filter(Boolean);
    const kept = lines.slice(0, 12).join("\n");
    return head + kept + (lines.length > 12 ? `\n…（其余 ${lines.length - 12} 个文件略）` : "") + "\n";
  });
...
export async function getProjectArchiveContext(topic, source = "", force = false, scene = "learn") {
  try {
    const projects = getPersonalProjects();
