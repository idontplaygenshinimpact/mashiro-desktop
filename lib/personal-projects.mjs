        ...
        try {
          ...
          try {
            const ba = await Promise.race([
              runSubagent({
                name: "八股提取",
                task: "基于以下项目源码要点，提取项目涉及的全部技术知识点（八股）——包括但不限于：状态管理（Zustand/Redux）、SSE 流式输出、有限状态机、iframe 沙箱、防抖节流、Next.js App Router、Web Worker、闭包/事件循环等。每个知识点一行：【八股】<知识点>：项目里怎么用的（真实文件/代码）+ 面试官可能追问的问题。至少 10 个，覆盖核心业务（不只是配置文件）。",
                context: fullSummary.slice(0, 8000),
              }),
          ...
          try {
            const qa = await Promise.race([
              runSubagent({
                ...
                context: `${fullSummary.slice(0, 6000)}\n\n${baSection.slice(0, 3000)}`,
              }),
