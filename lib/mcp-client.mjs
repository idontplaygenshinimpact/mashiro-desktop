...
export async function initMcpClients() {
  ...
  for (const cfg of configs) {
    ...
    try {
      ...
      const client = new Client({ name: "mashiro-desktop-client", version: "0.1.0" });
      ...
      console.log(`[mcp-client] 连接 ${cfg.name} 失败: ${e.message.slice(0, 80)}（已隔离，不影响主 agent）`);
    }
  }
  return getMcpTools();
}

/** 已发现的 MCP 工具（OpenAI function calling 格式） */
...
/** 调用 MCP 工具（返回文本结果；未连接/失败/超时返回可读错误） */
export async function callMcpTool(fullName, args) {
  ...
  try {
    const result = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error(`MCP 调用超时(${MCP_CALL_TIMEOUT_MS / 1000}s)`)), MCP_CALL_TIMEOUT_MS)),
    ]);
    // 结果转文本
