...
// hooks 命名空间：skill 的 hooks 自动注册到 lib/hooks.ts（监听器抛错由 hooks 层隔离）
// 全量 TS 升级工单阶段 3：lib/skills.mjs → .ts（widget/agent/插件路由/tests 按 .mjs 路径加载 → 保留同名一行桶）
...
/** SKILL.md 解析结果 */
export interface ParsedSkillMd { name: string; description: string; body: string }
/** 注入 agent system prompt 的技能提示 */
export interface SkillHint { name: string; description: string; system: string }
/** skill 工具 schema（合入 agent TOOLS 的 OpenAI 形状） */
...
let cache = new Map<string, LoadSkillsResult>(); // cacheKey -> { tools, permMap, names, hints, hookCount }
let hookOffs: Array<() => void> = []; // 已注册 hook 的取消函数（reload 时先清理再重扫，防重复注册）
 ...
 * force=true 时强制重扫（reload 用）：跳过缓存早退，重扫成功才 cache.set 覆盖（失败保留旧缓存）
 * only=技能名数组：仅加载名单内技能（场景装配）；缺省 = 全量（老调用方零感知） */
export async function loadSkills(skillsDir: string = SKILLS_DIR, { force = false, only = undefined }: { force?: boolean; only?: unknown } = {}): Promise<LoadSkillsResult> {
  // 缓存键：显式 only 优先；否则用激活集（activeOnly）；都不设 = 全量（向后兼容）
  const explicitOnly = Array.isArray(only) && only.length ? [...new Set(only.map((n) => String(n)))] : null;
  const key = explicitOnly
    ? `${skillsDir}|${explicitOnly.join(",")}`
    : cacheKeyFor(skillsDir);
  if (!force && cache.has(key)) return cache.get(key) as LoadSkillsResult;
  // ESM 模块版本号：Node 按 URL 缓存 import——拼 ?v= 让 reload 后 import 到新模块实例（代码/tools/hooks 生效）
  const modVersion = Date.now().toString(36);
  const tools: SkillToolSchema[] = [];
  const permMap: Record<string, string> = {};
  const names: string[] = [];
  const hints: SkillHint[] = []; // {name, description, system} 注入 agent system prompt
  let hookCount = 0;
    ...
    for (const dir of readdirSync(skillsDir, { withFileTypes: true })) {
      ...
      if (existsSync(mdPath)) {
        try {
          ...
          console.log(`[skills] ${dir.name}/SKILL.md 解析失败（隔离）: ${(e instanceof Error ? e.message : String(e)).slice(0, 80)}`);
        }
      }
      // 2) skill.mjs 可编程部分（tools/system/hooks）
      if (existsSync(modPath)) {
        try {
          const mod = await import(pathToFileURL(modPath).href + "?v=" + modVersion) as Record<string, unknown>;
          ...
          if (Array.isArray(mod.tools)) toolDefs = mod.tools as SkillToolDef[];
          if (mod.hooks && typeof mod.hooks === "object") hooks = mod.hooks as Record<string, unknown>;
          hadMod = true;
        } catch (e) {
          console.log(`[skills] ${dir.name}/skill.mjs 加载失败（隔离）: ${(e instanceof Error ? e.message : String(e)).slice(0, 120)}`);
      ...
      for (const t of toolDefs) {
        ...
        // 安全工单 M14：默认 confirm（fail-closed——与 getSkillPermission 注释一致）——
        // 只有工具显式声明 permission:"auto" 才自动执行
        permMap[full] = t.permission === "auto" ? "auto" : "confirm";
      }
      // 4) 注册 hooks（自动接线到 hooks 系统，抛错由 hooks 层隔离）
      if (hooks) {
        try {
          const { onHook } = await import("./hooks.ts");
          for (const [event, fn] of Object.entries(hooks)) {
            if (typeof fn === "function") { hookOffs.push(onHook(event, fn as (payload: Record<string, unknown>) => unknown)); hookCount++; }
          }
        } catch (e) {
          console.log(`[skills] ${skillName} hooks 注册失败（隔离）: ${(e instanceof Error ? e.message : String(e)).slice(0, 80)}`);
        }
      }
  ...
  const result: LoadSkillsResult = { tools, permMap, names, hints, hookCount, modVersion };
  cache.set(key, result);
  return result;
}

/** 已加载的 skill 工具 schema（合入 agent TOOLS；读取当前激活集缓存） */
export function getSkillTools(): SkillToolSchema[] {
  return cache.get(cacheKeyFor(SKILLS_DIR))?.tools || [];
}

/** skill 工具权限级别（默认 confirm：权限查询不到按敏感处理，fail-closed） */
export function getSkillPermission(fullName: string): string {
...
export async function reloadSkills(skillsDir: string = SKILLS_DIR): Promise<{ ok: boolean; names?: string[]; toolCount?: number; hookCount?: number; error?: string }> {
...
/** 单个技能的运行时概览 */
export interface SkillInspectEntry {
  name: string;
  description: string;
  toolCount: number;
  tools: Array<{ name: string; permission: string }>;
}
export interface InspectSkillsResult {
  ok: boolean;
  loaded: boolean;
  hint?: string;
  skills?: SkillInspectEntry[];
  hookCount?: number;
  totalTools?: number;
}

/** 运行时概览（面板/agent 查询；对标 DSH cordis_inspect_list） */
export function inspectSkills(skillsDir: string = SKILLS_DIR): InspectSkillsResult {
  const r = cache.get(cacheKeyFor(skillsDir));
...
export function buildSkillHintsPrompt(): string {
...
export async function callSkillTool(fullName: unknown, args: unknown, skillsDir: string = SKILLS_DIR): Promise<Record<string, unknown>> {
  const parts = String(fullName || "").split("__");
  if (parts.length !== 3 || parts[0] !== "skill") return { error: `未知 skill 工具: ${String(fullName)}` };
  ...
  if (!Object.prototype.hasOwnProperty.call(permMap, String(fullName))) {
    return { error: `skill 工具未在白名单: ${String(fullName)}` };
  }
  ...
  try {
    ...
    const mod = await import(pathToFileURL(modPath).href + "?v=" + modVersion) as { tools?: SkillToolDef[] };
    ...
    return r && typeof r === "object" ? r as Record<string, unknown> : { ok: true, result: r };
  } catch (e) {
    return { error: `skill ${skillName}.${toolName} 执行失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 150)}` };
  }
}
