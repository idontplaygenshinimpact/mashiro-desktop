...
import { sanitizeExternal } from "./prompt-guard.mjs"; // 外部素材包裹（防注入：知识库内容视为不可信数据）
 ...
 * @returns {Promise<{ok: boolean, added?: number, error?: string, batch?: number, kbUsed?: boolean}>}
 */
 ...
 * @returns {Promise<{ ok: boolean, total?: number, fromCache?: boolean, kbUsed?: boolean, error?: string }>} 生成结果
 */
export async function ensureQuiz(cardId) {
  ...
  const r = await generateQuiz(/** @type {any} */ (card));
  if (!r.ok) return r;
  ...
  try {
    ...
    return {
      ...
      questions: rows.map((r) => {
        let options = [];
        try { options = JSON.parse(String(r.options)); } catch { /* ignore */ }
...
export function submitQuiz(cardId, answers = []) {
  ...
  withTx(() => {
    ...
    for (const a of list) {
      ...
      if (!qid) continue;
      const row = db.prepare("SELECT id, answer, explain FROM quiz_questions WHERE id = ? AND card_id = ?").get(qid, String(cardId || ""));
      if (!row) continue;
      const chosenRaw = Number(a?.chosen ?? -1);
      const map = Array.isArray(a?.map) ? a.map : null;
