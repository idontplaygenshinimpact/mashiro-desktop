// lib/backup.ts —— 数据备份与恢复（数据安全：自动备份 + 一键恢复，重启生效）
...
// 全量 TS 升级工单阶段 3：lib/backup.mjs → .ts（db.mjs/self-check/widget/路由按 .mjs 路径加载 → 保留同名一行桶）
...
import { readJsonSafe, writeJsonAtomic } from "./atomic-json.ts";

// 打包版：MIANSHI_DATA_DIR 由主进程注入（asar 只读）——备份目录跟随数据根
...
const BACKUP_DIR = (): string => path.join(path.dirname(DB_FILE), "backups");
// 架构 P1-10：备份结果持久记录（成功/失败都记——面板可见"上次备份状态"，失败不再静默）
const LAST_STATUS_FILE = (): string => path.join(path.dirname(DB_FILE), "backup-last-status.json");
...
export interface CreateBackupResult { ok: boolean; name?: string; files?: string[]; note?: string; error?: string }
...
export async function createBackup(reason: unknown = "manual"): Promise<CreateBackupResult> {
  try {
    ...
    pruneBackups();
    recordBackupOutcome({ ok: true, reason: safeReason, name: path.basename(dir) });
    return { ok: true, name: path.basename(dir), files, note: `备份完成（${files.length} 项）` };
  } catch (e) {
    const error = `备份失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 200)}`;
    recordBackupOutcome({ ok: false, reason, error });
    return { ok: false, error };
  }
}

/** 架构 P1-10：持久记录上次备份结果（成功/失败都记——面板可见，失败不再静默） */
export function recordBackupOutcome({ ok, reason = "", error = "", name = "" }: { ok?: unknown; reason?: unknown; error?: unknown; name?: unknown }): void {
  try {
    writeJsonAtomic(LAST_STATUS_FILE(), {
      ok: !!ok,
...
/** 读取上次备份结果（无记录返回 null） */
export function getLastBackupOutcome(): BackupOutcome | null {
  return readJsonSafe<BackupOutcome | null>(LAST_STATUS_FILE(), null);
}
...
export function listBackups(): { ok: boolean; backups: BackupEntry[]; error?: string } {
  try {
    const dir = BACKUP_DIR();
    ...
    const backups = readdirSync(dir, { withFileTypes: true })
      .filter((d) => d.isDirectory())
      .map((d) => {
        const m = readJsonSafe<BackupManifest | null>(path.join(dir, d.name, "manifest.json"), null);
        let mtime = 0;
    ...
    return { ok: false, backups: [], error: (e instanceof Error ? e.message : String(e)).slice(0, 120) };
  }
}
 ...
 * @param name 备份目录名
 * @param dbFile 目标主库路径（测试注入独立目录模拟"新进程"；默认生产路径）
 */
export function markRestore(name: unknown, dbFile: string = DB_FILE): { ok: boolean; name?: string; note?: string; error?: string } {
  try {
    const safe = String(name || "").replace(/[\\/]/g, ""); // 防路径穿越
    const src = path.join(path.dirname(dbFile), "backups", safe, "mianshi.db");
    if (!existsSync(src)) return { ok: false, error: `备份不存在或缺少主库文件: ${String(name)}` };
    ...
    return { ok: false, error: `标记恢复失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 200)}` };
  }
}
 ...
 * @returns 是否执行了恢复
 */
export function applyPendingRestore(dbFile: string = DB_FILE): boolean {
  ...
  try {
    ...
    console.error(`[backup] 应用待恢复失败（保留原库）: ${(e instanceof Error ? e.message : String(e)).slice(0, 120)}`);
    return false;
  }
}
