...
import { kwHit, hasSpecificKw } from "./match-utils.mjs";
export { kwHit } from "./match-utils.mjs"; // 兼容旧导出（测试直测）
...
// 组合词表/独立成词/特异性门槛已沉淀到 lib/match-utils.mjs（7 个独立实现的公共层）。

// 分组归一（相似度引擎统一工单任务 5③）：知识树分类名 vs 兜底规则组名统一——
// RAG/Agent 相关组（RAG与LLM/RAG/Agent/Agent/LLM/AI Agent）合并为兜底规则组名"Agent与LLM"
// （2026-09 改名：RAG 是 LLM 应用的一种技术，LLM 是模型——不是平级概念；
//   "Agent与LLM"准确——LLM 是 Agent 的核心组件。含旧名 RAG与LLM 别名兼容存量）
const GROUP_ALIAS = { "RAG": "Agent与LLM", "Agent": "Agent与LLM", "Agent/LLM": "Agent与LLM", "LLM": "Agent与LLM", "AI Agent": "Agent与LLM", "RAG与LLM": "Agent与LLM" };
const normGroup = (g) => GROUP_ALIAS[g] || g;
