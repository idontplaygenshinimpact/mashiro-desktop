...
import { similarityRule } from "./similarity.mjs";
...
export const normalizeGroupName = normGroup;
...
const simGroupCache = new Map(); // topic → group（幂等缓存——同 topic 不重复判定）
/** similarity 规则层判定（同步）：topic 与各组代表词 weak 宽松匹配，≥0.5 归组（取最高分） */
export function similarityGroupRule(topic) {
  const t = String(topic || "").trim().toLowerCase();
  if (!t) return null;
  if (simGroupCache.has(t)) return simGroupCache.get(t);
  let best = null;
...
export async function normalizeGroupAsync(topic, grp = "", why = "") {
