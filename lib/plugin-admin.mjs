...
import { discoverPlugins, loadPlugin } from "./plugin-loader.ts";
...
export async function loadEnabledPlugins(api, pluginsDir = process.env.MIANSHI_PLUGINS_DIR || path.join(ROOT, "plugins")) {
  const results = [];
...
export function listPlugins(pluginsDir = process.env.MIANSHI_PLUGINS_DIR || path.join(ROOT, "plugins")) {
...
export function readPluginSettings(id, pluginsDir = process.env.MIANSHI_PLUGINS_DIR || path.join(ROOT, "plugins")) {
...
export function writePluginSetting(id, key, value, pluginsDir = process.env.MIANSHI_PLUGINS_DIR || path.join(ROOT, "plugins")) {
  const p = listPlugins(pluginsDir).find((x) => x.id === id);
...
export async function installPlugin(id, opts = {}) {
  const { fetcher = fetch, pluginsDir = process.env.MIANSHI_PLUGINS_DIR || path.join(ROOT, "plugins"), market = getPluginMarket().plugins } = opts;
