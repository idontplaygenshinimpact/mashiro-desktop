// lib/patrol.ts —— 主动推送巡检模块（纵向拆分：从 widget.mjs 迁出）
...
// 全量 TS 升级工单阶段 3：lib/patrol.mjs → .ts（widget.mjs 组装 + tests 按 .mjs 路径加载 → 保留同名一行桶）
...
/** 是否处于峰时（按本地时间） */
export function isDsPeakHour(d: Date): boolean {
  const h = d.getHours() + d.getMinutes() / 60;
  return (h >= 9 && h < 12) || (h >= 14 && h < 18);
}

/** 避开峰时：at 落在峰时窗口 → 推迟到该窗口结束（12:00 / 18:00，已过则次日 00:30 谷时）；谷时或 avoidPeak=false → 原样 */
export function avoidPeakTime(at: number, avoidPeak = true): number {
...
/** 巡检器依赖（widget.mjs 组装；也可独立测试） */
export interface PatrolDeps {
  /** 环境变量强制关闭（MIANSHI_DISABLE_PATROL=1） */
  disabled?: boolean;
  sendNotification?: (title: string, message: string) => void | Promise<void>;
  /** 全量爬取互斥 */
  crawlMutex?: { isRunning: () => boolean };
  runDiscoverHidden?: () => unknown;
}
/** 巡检运行时状态（面板/测试观察） */
...
/** 巡检配置视图（GET /api/patrol-config） */
export interface PatrolConfigView {
  ok: boolean;
  enabled: boolean;
  intervalMin: number;
  avoidPeak: boolean;
...
/** 待处理巡检帖 */
export interface PatrolPost { title: string; url: string; topic?: unknown }
export interface ProcessPostsResult { saved: string[]; skipped: Record<string, number> }
/** fetch-page 返回（只用到这几项；ok 为历史宽松字段） */
interface PageLike {
  ok?: boolean;
  invalid?: boolean;
  title?: string;
...
export interface Patrol {
  ...
  setBudget: (tokens: unknown) => { ok: boolean; error?: string };
...
export function createPatrol({
  ...
  let lastFullCrawl: number | null = null; // 全量爬取限频标记

  /** 读取巡检 token 预算（0=不限） */
  function dailyBudget(): number {
    try {
      const row = db.prepare("SELECT value FROM settings WHERE key=?").get(PATROL_BUDGET_KEY) as { value?: unknown } | undefined;
  ...
  function usedTokensToday(): number {
    try {
      const start = new Date(); start.setHours(0, 0, 0, 0);
      const r = db.prepare("SELECT COALESCE(SUM(input_tokens),0) i, COALESCE(SUM(output_tokens),0) o FROM trace_llm WHERE role='patrol' AND ts >= ?").get(start.getTime()) as { i?: unknown; o?: unknown } | undefined;
  ...
  function setBudget(tokens: unknown): { ok: boolean; error?: string } {
  ...
  function getConfig(): PatrolConfigView {
  ...
  function scheduleNext(): void {
    ...
    state.timer = setTimeout(async () => {
      ...
      try {
        ...
        console.log(`[widget] 巡检异常: ${(e instanceof Error ? e.message : String(e)).slice(0, 80)}`);
      }
      scheduleNext();
    }, delay);
  ...
  // 巡检方向白名单：跟随求职目标（方向画像 directions 多选；默认前端/Agent）
  // backend → 只巡检后端/全栈；fullstack → 全方向；frontend/agent/未设置 → 前端/Agent
  // 2026-09 多选：多个方向的白名单合并
  function goodDirs(): string[] {
    try {
      const { directions } = getCareerProfile() as { directions?: string[] };
      const ds = directions || [];
      const out = new Set<string>();
      for (const d of ds) {
        if (d === "backend") { out.add("backend"); out.add("fullstack"); }
        else if (d === "fullstack") { out.add("frontend"); out.add("agent"); out.add("fullstack"); out.add("backend"); }
        else if (d === "agent") { out.add("agent"); out.add("frontend"); }
        else { out.add("frontend"); out.add("agent"); }
      }
  ...
  async function processPosts(posts: PatrolPost[]): Promise<ProcessPostsResult> {
    const saved: string[] = [];
    const skipped: Record<string, number> = {};
    const skip = (reason: string): void => { skipped[reason] = (skipped[reason] || 0) + 1; };
    try {
      const { fetchPage } = await import("./fetch-page.mjs");
      const { classifyPage, detectQuestions, solveQuestion } = await import("./ai.ts");
      ...
      for (const p of posts) {
        try {
          ...
          // 修复：markSeen 移到处理成功之后（此前先 markSeen 再 fetchPage，
          // 抓取/分类失败的帖子被永久标记为已看 → 下次巡检不再重试）
          const page = await fetchPage(p.url, { maxTextChars: 6000 }) as PageLike;
          if (!page.ok || page.invalid || !page.text || page.text.length < 200) { skip("页面无效/404"); continue; }
          // 方向过滤：跟随求职目标
          const cls = await classifyPage({ title: String(page.title || ""), text: page.text }, "patrol");
          if (!GOOD_DIRS.includes(String(cls.direction))) { skip("非目标方向"); continue; }
          if (Number(cls.worth) < 40) { skip("内容价值低"); continue; }
          // 具体题目检测：攻略文跳过
          const dq = await detectQuestions({ title: String(page.title || ""), text: page.text }, "patrol");
          if (!dq.hasQuestion || !dq.questions?.length) { skip("攻略文/无具体题"); continue; }
          const title = String(page.title || "");
          // 完整讲解
          const md = await solveQuestion({
            title,
            text: dq.questions.slice(0, 3).map((q: { question?: unknown }, i: number) => `【题${i + 1}】${String(q?.question || "")}`).join("\n"),
            company: String(cls.company || ""),
            position: String(cls.position || ""),
            sourceUrl: p.url,
          }, "patrol");
          ...
          const fname = `${String(saved.length + 1).padStart(2, "0")}_${String(cls.company || cls.type || "patrol").replace(/[\\/:*?"<>|]/g, "_")}_${title.replace(/[\\/:*?"<>|]/g, "_").slice(0, 30)}.md`;
          writeFileSync(path.join(dir, fname), `# ${title}\n\n> 来源: ${p.url}\n\n${md}\n`, "utf8");
          saved.push(fname);
          // 处理成功后才标记已看（避免下次重复通知）
          memory.markSeen(p.url);
          console.log(`[widget] 巡检讲解完成: ${fname}`);
        } catch (e) {
          console.log(`[widget] 巡检讲解失败 ${p.url}: ${e instanceof Error ? e.message : String(e)}`);
          skip("讲解失败");
        }
      }
    } catch (e) {
      console.log(`[widget] processPatrolPosts 异常: ${e instanceof Error ? e.message : String(e)}`);
    }
  ...
  async function run(): Promise<void> {
    ...
    try {
      ...
      const prof = getCareerProfile() as { ignoreNote?: string };
      ...
      const newPosts: PatrolPost[] = [];
      // 取前 2 个关注点，每个搜多站（牛客/掘金/CSDN）
      const searchSites = [
        (q: string) => `https://www.nowcoder.com/discuss?type=2&query=${encodeURIComponent(q)}`,
        (q: string) => `https://juejin.cn/search?query=${encodeURIComponent(q + " 面经")}`,
        (q: string) => `https://so.csdn.net/so/search?q=${encodeURIComponent(q + " 面经")}`,
      ];
      for (const topic of interests.slice(0, 2)) {
        for (const makeUrl of searchSites) {
          const url = makeUrl(String(topic));
          try {
            const page = await fetchPage(url, { maxTextChars: 1500, collectLinks: true }) as PageLike;
      ...
      if (newPosts.length) {
        ...
        if (now - (lastFullCrawl || 0) > 2 * 60 * 60 * 1000) {
          if (crawlMutex.isRunning()) {
            ...
            // 修复：lastFullCrawl 只在真正调用 runDiscoverHidden() 的分支内推进——
            // 此前在 mutex 检查前就置位，忙时窗口被空耗（全量爬取没触发却进入冷却）
            lastFullCrawl = now;
