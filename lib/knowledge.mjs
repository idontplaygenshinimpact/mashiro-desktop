...
const MASTERY_FILE = path.join(DATA_DIR, "kp-mastery.json");
...
function loadMastery() {
  try {
    if (existsSync(MASTERY_FILE)) return JSON.parse(readFileSync(MASTERY_FILE, "utf8"));
  } catch { /* ignore */ }
  return {};
}
function saveMastery(m) {
  try {
    ...
    writeFileSync(MASTERY_FILE, JSON.stringify(m, null, 2), "utf8");
  } catch { /* ignore */ }
}
