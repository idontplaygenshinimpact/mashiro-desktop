// 知识点树：分类骨架（默认前端，可配置/多方向模板）+ 掌握度（动态）
// 面试/复习的题目打上 kp_id，答题结果写回该知识点掌握度
// 开源/转方向：设置中心「知识树」可整体替换；data/knowledge-trees.json 内置 frontend/backend/algorithm 模板
import { readFileSync } from "node:fs";
import path from "node:path";
import { db, withTx } from "./db.mjs";
import { hasSpecificKw } from "./match-utils.mjs";
...
export const KNOWLEDGE_TREE = [
  ...
  },
  {
    ...
    points: [
      { id: "br-render", title: "渲染管线与回流重绘", difficulty: 3, kws: ["渲染", "回流", "重绘", "dom", "url", "页面展示", "dns 解析"] },
      { id: "br-cache", title: "缓存策略", difficulty: 2, kws: ["缓存", "cache", "http"] },
      { id: "br-security", title: "XSS/CSRF/CORS", difficulty: 3, kws: ["xss", "csrf", "cors", "跨域", "同源"] },
      { id: "br-perf", title: "性能指标与优化", difficulty: 3, kws: ["首屏", "lcp", "fcp", "重排", "白屏", "加载性能", "性能指标"] },
    ],
  },
  {
  ...
  {
    id: "vue", title: "Vue", difficulty: 2,
    points: [
      { id: "vu-reactivity", title: "响应式原理 (Vue2/3)", difficulty: 3, kws: ["vue", "响应式", "proxy", "defineproperty", "依赖收集"] },
      { id: "vu-template", title: "模板编译与指令", difficulty: 2, kws: ["模板", "指令", "v-model", "v-for", "插槽"] },
      { id: "vu-computed", title: "computed/watch 与更新", difficulty: 3, kws: ["computed", "watch", "nexttick", "更新机制"] },
      { id: "vu-ecosystem", title: "Vue 生态选型", difficulty: 2, kws: ["pinia", "vuex", "vue-router", "nuxt"] },
    ],
  },
  {
    id: "css", title: "CSS/HTML", difficulty: 2,
    points: [
      { id: "css-layout", title: "布局 (Flex/Grid)", difficulty: 2, kws: ["flex", "grid", "布局", "居中", "垂直居中", "水平居中"] },
...
// ---------- 可配置知识树（settings knowledge_tree 覆盖；无则 data/knowledge-trees.json current 模板；再兜底默认树） ----------
// 开源多方向：data/knowledge-trees.json 内置 frontend/backend/algorithm 模板，设置中心一键切换
...
export function listTreeTemplates() {
  ...
  return Object.entries(t.templates).map(([name, tree]) => ({
    name,
    current: name === t.current,
    categories: Array.isArray(tree) ? tree.length : 0,
    points: Array.isArray(tree) ? tree.reduce((n, c) => n + (c.points?.length || 0), 0) : 0,
  }));
}
...
export function loadTreeTemplate(name) {
  ...
  if (!isValidTree(tree)) return { ok: false, error: "模板结构非法" };
  try {
    db.prepare("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?, ?, ?)")
      .run(TREE_KEY, JSON.stringify(tree), Date.now());
    t.current = name;
    try {
      createRequire(import.meta.url)("node:fs").writeFileSync(TREES_FILE, JSON.stringify(t, null, 2), "utf8");
    } catch { /* ignore */ }
    treeCache = null;
    const total = tree.reduce((n, c) => n + (c.points?.length || 0), 0);
...
export function getKnowledgeTree() {
  ...
  // 2) data/knowledge-trees.json 的 current 模板（开源用户直接改文件即可换方向）
  const tpl = getTreeTemplates();
  const cur = tpl.templates[tpl.current];
...
export function setMasteryScore(kpId, score) {
  ...
  const m = loadMastery();
  if (!m[kpId]) m[kpId] = { score: 50, attempts: 0, lastAt: "" };
  m[kpId].score = Math.max(0, Math.min(100, Number(score) || 50));
  m[kpId].lastAt = new Date().toISOString();
  saveMastery(m);
}
