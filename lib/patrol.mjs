...
// ---------- DS 高峰时段工具（纯函数，可单测） ----------
// DeepSeek 官方高峰 = UTC 16:30-00:30 = 北京时间 00:30-08:30（API 价格上浮 50%）
/** 是否处于高峰时段（按本地时间） */
...
/** 避开高峰：at 落在高峰 → 推迟到当天 08:30（若当天 08:30 已过则次日 08:30）；不在高峰或 avoidPeak=false → 原样 */
export function avoidPeakTime(at, avoidPeak = true) {
  if (!avoidPeak) return at;
  const d = new Date(at);
  if (!isDsPeakHour(d)) return at;
...
export function createPatrol({
  ...
  const state = {
    ...
    // 避开 DS 高峰时段（北京 00:30-08:30，API 价格上浮 50%）：高峰内的触发推迟到 08:30 后
  ...
  /** 避开高峰：把触发时间推迟到当天/次日 08:30（高峰结束）；纯函数见模块级 avoidPeakTime */
  function avoidPeakTime(at) {
    return avoidPeakTime(at, state.avoidPeak);
  }
