// 可观测性模块
// @typedef {import("./types.d.ts").Decision} Decision：记录每次 LLM 调用（模型/耗时/token/成败）+ 工具调用链
...
// 记录一次 LLM 调用（参数形状显式 JSDoc——修复 checkJs 从解构默认值推断出
// inputTokens?: null 等只许 null 的错误契约；llm.ts 迁移后真实值会触发）
/**
 * @param {{ role?: string, model?: string|null, stream?: boolean, inputTokens?: number|null,
 *   outputTokens?: number|null, durationMs?: number|null, ok?: boolean, error?: string|null,
 *   endpoint?: string|null }} info 调用信息（llm.ts 传实值；缺省回落默认）
 */
...
// 记录一次工具调用
// 记录一次工具调用（参数形状显式 JSDoc——修复 checkJs 从解构默认值推断出 args?: null
// 等只许 null 的错误契约；agent.ts 迁移后真实值会触发）
/**
 * @param {{ sessionId?: string|null, toolName: string, args?: unknown, ok?: boolean,
 *   error?: string|null, durationMs?: number|null }} info 调用信息（缺省回落默认）
 */
 ...
 *  @param {{sessionId?: string|null, decision: 'allow'|'deny'|'auto_allow'|'timeout'|'tool_error'|'injection_hit', toolName?: string|null, reason?: string|null, policyRef?: string|null, approvedBy?: string|null}} input
 */
...
export function getDecisionStats({ sinceTs = 0 } = {}) {
  ...
  const byDecision = /** @type {Record<string, number>} */ ({});
