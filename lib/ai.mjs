...
export async function solveAppendStream({ topic, existing, question }, onChunk) {
  ...
  const { getCareerProfile } = await import("./career.mjs");
  const prof = getCareerProfile();
  // 拆分：主文（首个"💬 追问"标记之前）与追问段落区——主文是稳定前缀，追问段是可变尾部
  const raw = String(existing || "");
  const firstMark = raw.indexOf("## 💬 追问");
  const mainText = (firstMark > 0 ? raw.slice(0, firstMark) : raw).slice(-15000);
  const followupTail = (firstMark > 0 ? raw.slice(firstMark) : "").slice(-8000); // 只带最近追问段
...
async function solveQuestionImpl({ title: _title, text, company, position, sourceUrl }, call) {
  const { getCareerProfile } = await import("./career.mjs");
  const prof = getCareerProfile();
