...
const AGENT_TOPIC_RE = /agent|工具调用|function\s*calling|tool\s*binding|mcp|llm|大模型|提示词|prompt|token|模型|rag|检索增强|multi-agent|多智能体|langchain|langgraph|微调|量化|推理|解析器|结构化输出|structured/i;
function topicDirection(title, text, prof) {
  const isAgent = AGENT_TOPIC_RE.test(String(title || "") + " " + String(text || ""));
  return isAgent
    ? { roleLabel: "资深 AI Agent 应用开发面试辅导老师", scopeNote: "AI Agent 应用开发（工具调用/LLM 机制/Agent 架构）" }
    : { roleLabel: prof.roleLabel, scopeNote: prof.scopeNote };
}
