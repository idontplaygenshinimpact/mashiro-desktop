...
export function topicDirection(title, text, prof) {
  ...
  const isFrontend = FRONTEND_TOPIC_RE.test(joined);
  const isAgent = AGENT_TOPIC_RE.test(joined);
  const isAlgo = isAlgoTopic(joined);
  if (isAgent && isFrontend) {
    // 双方向：前端 + Agent 都命中 → 双视角讲解（不丢任何一侧）
    return { roleLabel: "资深前端与 AI Agent 应用开发面试辅导老师", scopeNote: "前端 + AI Agent 应用开发（双视角）", dual: true, isAlgo };
  }
  if (isAgent) {
    return { roleLabel: "资深 AI Agent 应用开发面试辅导老师", scopeNote: "AI Agent 应用开发（工具调用/LLM 机制/Agent 架构）", dual: false, isAlgo };
  }
  return { roleLabel: prof.roleLabel, scopeNote: prof.scopeNote, dual: false, isAlgo };
}
...
// 约束：还原原题范围 + 标注改编 + 区分原题/改编场景理由。
const ADAPTATION_CONSTRAINT = `
【改编约束】（重要）：
- 先还原原题的真实范围（岗位/系统/数据形态）——不虚构"本质是 X"
- 改编必须标注"改编"（思想迁移），不把类比当本质
- 原题的核心约束（如高吞吐/审计/成本）是原题特有的——改编后要区分"原题理由"和"改编场景理由"
- 如果原题理由在改编场景不成立，明确说明（如"成本/吞吐是后端约束，前端场景不适用"）
- 讲解开头固定"原题范围 + 改编说明"段：
  ## 原题范围
  [原题问什么、真实岗位/系统/数据形态]
  ## 改编说明
  [改编成什么、思想迁移点、约束差异（哪些理由原题成立、改编场景不成立）]`;

// 追问一致性约束（2026-08 追加：追问是独立 LLM 调用，无一致性约束——改编/还原/映射三视角矛盾）
const CONSISTENCY_CONSTRAINT = `
【一致性约束】：
- 回答必须与已有讲解的立场一致
- 如用户追问与讲解冲突，先说明"讲解是改编视角（原题是 X，改编成 Y）"再回答
- 多视角回答时标注视角（原题视角/改编视角/映射视角）
- 不引入与已有讲解矛盾的新立场`;
