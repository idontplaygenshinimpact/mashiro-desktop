...
const ALGO_TOPIC_RE = /合并|排序|链表|数组|二叉树|动态规划|双指针|滑动窗口|回溯|贪心|哈希|栈|队列|堆|递归|dfs|bfs|二分|前缀和|拓扑|并查集|单调栈|字符串匹配|kmp|lru|lfu|topk|第k|中位数|反转|旋转|去重|子序列|子数组|岛屿|路径|排列|组合|背包|手写|手撕|算法/i;
function topicDirection(title, text, prof) {
  const joined = String(title || "") + " " + String(text || "");
  const isAgent = AGENT_TOPIC_RE.test(joined);
  const isAlgo = ALGO_TOPIC_RE.test(joined);
  return {
