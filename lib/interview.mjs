...
2. 一句话点评
3. 决定下一问：
   - 当前轮次内：回答有漏洞且追问深度 < ${MAX_DEPTH} → **深挖追问**（depth+1）；否则**切换新题**（depth 归 0）
   - **必须符合本轮类型**「${currentRound.name}」：项目拷打轮只出项目题、八股轮只出基础题、手写轮出手写/场景题、反问轮让候选人提问
   - 本轮完成后衔接下一轮（${nextRound ? `下一轮「${nextRound.name}」的第一问` : "面试结束"}）
   - 最后一轮（反问）结束后返回 finish: true

输出 JSON：
{"scores":{"tech":0,"expr":0,"depth":0,"edge":0,"reflect":0},"comment":"一句话点评","finish":false,"next_question":"下一问问题（finish=true 时为空）","next_basis":"追问依据","next_dimension":"考察维度","next_criteria":"合格标准","next_boundary":"边界","weak_topic":"本轮暴露的薄弱知识点（如有，如'事件循环'）","weak_hit":"本轮问题主题命中的薄弱点队列项（从队列原样抄主题名，非队列出题则为空）"}`;
  ...
  const depth = r.next_question && r.next_basis === current.basis ? Math.min(current.depth + 1, MAX_DEPTH) : 0;
  session.roundIndex = nextRoundIndex;
  session.current = {
