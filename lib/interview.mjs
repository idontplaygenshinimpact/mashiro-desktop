...
{"scores":{"tech":0,"expr":0,"depth":0,"edge":0,"reflect":0},"comment":"一句话点评","finish":false,"next_kind":"followup|new|stage","next_question":"下一问问题（finish=true 时为空）","next_basis":"追问依据","next_dimension":"考察维度","next_criteria":"合格标准","next_boundary":"边界","weak_topic":"本轮暴露的薄弱知识点（如有，如'事件循环'）","weak_hit":"本轮问题主题命中的薄弱点队列项（从队列原样抄主题名，非队列出题则为空）"}`;
  ...
  // 修复 B2：五维评分数字校验——此前无 Number.isFinite/clamp，LLM 返回字符串或缺维度
  // 会让 total=NaN → 后续 `if (total < 60)` 恒 false → 薄弱点回流静默失效。
  // 每个维度：Number() 转换 + isFinite 校验（非法回退 0）+ 0-100 clamp；合法输入分数不变
  const DIM_KEYS = ["tech", "expr", "depth", "edge", "reflect"];
  const cleanScore = (v) => {
    const n = Number(v);
  ...
  const scores = {};
  for (const d of DIM_KEYS) scores[d] = cleanScore((r.scores || {})[d]);
