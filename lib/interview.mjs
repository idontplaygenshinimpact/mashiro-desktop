...
export async function startInterview({ position, role = "技术深挖型", resume = "", focus = "", project: _project = "" }) {
...
2. 一句话点评
 3. 决定下一问类型 next_kind（**必须三选一，这是服务端轮次编排的唯一依据**）：
    ...
    防死循环兜底：深度达到 ${MAX_DEPTH} 后禁止 followup（正常按质量判断不会走到）；项目拷打轮只出项目题、八股轮只出基础八股题、手写轮出手写/场景题、反问轮让候选人提问
输出 JSON：
{"scores":{"tech":0,"expr":0,"depth":0,"edge":0,"reflect":0},"comment":"一句话点评","finish":false,"next_kind":"followup|new|stage","next_question":"下一问问题（finish=true 时为空）","next_basis":"追问依据","next_dimension":"考察维度","next_criteria":"合格标准","next_boundary":"边界","weak_topic":"本轮暴露的薄弱知识点（如有，如'事件循环'）","weak_hit":"本轮问题主题命中的薄弱点队列项（从队列原样抄主题名，非队列出题则为空）"}`;
  ...
  if (!followup && !sameRound) session.roundIndex = nextRoundIndex; // 仅 stage 推进轮次（追问/换题都不消耗轮次）
  session.current = {
...
[可以投递 / 基本准备好 / 需要继续打磨 / 建议先集中训练，一句话理由]`;
  ...
  // 复习卡若 submit 时未建（如 weak_topic 缺失但评分低），这里补建一次
  for (const r of session.rounds) {
    if (r.total < 50) {
      ...
      try {
        ...
        const dup = review.loadCards().cards.some((c) => c.topic === weakTopic && c.question === String(r.question || "").slice(0, 200));
        if (!dup) review.addCard({
          topic: weakTopic,
