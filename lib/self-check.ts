...
//   8. 数据备份健康（从未备份 / 距上次备份 >48h → 数据有丢失风险）→ 报告
// 全量 TS 升级工单阶段 3：lib/self-check.mjs → .ts（widget.mjs 惰性 import 与 tests 按 .mjs 路径加载 → 保留同名一行桶）
import { db } from "./db.mjs";
import { readFileSync, existsSync, readdirSync, statSync } from "node:fs";
import path from "node:path";
import { hoursSinceLastBackup, getLastBackupOutcome } from "./backup.mjs";
...
export function runSelfCheck({ now = Date.now(), outputDir = OUTPUT_DIR }: { now?: number; outputDir?: string } = {}): SelfCheckReport {
  const issues: SelfCheckIssue[] = [];
  const checks: SelfCheckItem[] = [];
  const addCheck = (name: string, status: "ok" | "warn" | "error", detail: string): void => { checks.push({ name, status, detail }); };
  const warn = (name: string, detail: string, fixed = false): void => {
    issues.push({ level: "warn", name, detail, ...(fixed ? { fixed } : {}) });
    addCheck(name, "warn", detail);
  };
  const okc = (name: string, detail: string): void => addCheck(name, "ok", detail);

  // ---------- 1) 表堆积：超限自动清理 ----------
  for (const [table, limit] of Object.entries(TABLE_LIMITS)) {
    try {
      const row = db.prepare(`SELECT COUNT(*) n FROM [${table}]`).get() as { n?: unknown } | undefined;
      ...
      warn("数据表堆积", `${table} 检查失败: ${errMsg(e).slice(0, 60)}`);
    }
  }
  ...
  try {
    ...
    warn("产出目录", `检查失败: ${errMsg(e).slice(0, 60)}`);
  }

  // ---------- 3) 巡检停摆 ----------
  try {
    const patrolRow = db.prepare("SELECT value FROM settings WHERE key='patrol_last_run'").get() as { value?: unknown } | undefined;
    const intervalRow = db.prepare("SELECT value FROM settings WHERE key='patrol_interval_min'").get() as { value?: unknown } | undefined;
    const lastRun = Number(patrolRow?.value || 0);
    const intervalMin = Math.max(15, Number(intervalRow?.value || 60) || 60);
    if (lastRun > 0) {
  ...
  // ---------- 4) LLM 失败率 ----------
  try {
    const rows = db.prepare("SELECT ok FROM trace_llm ORDER BY id DESC LIMIT 100").all() as Array<{ ok?: unknown }>;
    if (rows.length >= 10) {
  ...
  // ---------- 8) 数据备份健康（数据安全：从未备份 / 超 48h 未备份 / 上次失败 → 风险提示） ----------
  try {
    // 架构评审遗留收尾工单任务 1：上次备份失败原因可见（backup.mjs 持久记录——失败不再静默）
    const last = getLastBackupOutcome() as { ok?: boolean; ts?: number; error?: unknown } | null | undefined;
    if (last && last.ok === false) {
      const when = last.ts ? new Date(last.ts).toLocaleString("zh-CN", { hour12: false }) : "未知时间";
      warn("数据备份", `上次自动备份失败（${when}）：${String(last.error || "未知原因").slice(0, 100)}——检查磁盘空间/目录权限，或手动备份`);
    } else {
      const h = hoursSinceLastBackup();
      if (h === Infinity) {
        warn("数据备份", "从未备份——建议立即备份（设置→数据维护→💾 立即备份；之后每天自动备份一次）");
      } else if (h > 48) {
        warn("数据备份", `距上次备份已 ${Math.round(h)} 小时（>48h）——检查自动备份定时器或手动备份`);
      } else {
        okc("数据备份", `距上次备份 ${Math.round(h)} 小时`);
      }
    }
  } catch (e) {
    warn("数据备份", `检查失败: ${errMsg(e).slice(0, 60)}`);
  }
...
export function getLastSelfCheck(): SelfCheckReport | null {
  try {
    const r = db.prepare("SELECT value FROM settings WHERE key='self_check_report'").get() as { value?: unknown } | undefined;
    return r && r.value ? JSON.parse(String(r.value)) as SelfCheckReport : null;
  } catch { return null; }
}

/** 保存报告（供面板展示 + 通知去重） */
export function saveSelfCheck(report: unknown): void {
  try {
