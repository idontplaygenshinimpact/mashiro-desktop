...
// 全量 TS 升级工单阶段 3：lib/edge-session.mjs → .ts（保留同名一行桶：模块导出型工具，调用方按 .mjs 路径接入）
...
/** profile 探测结果 */
interface EdgeProfile { src: string; defaultDir: string; cookiesPath: string }
/** 会话初始化/取用结果 */
export interface EdgeSessionResult {
  ok: boolean;
  reason?: string;
  fetchWithEdge?: (url: string, opts?: EdgeFetchOpts) => Promise<EdgePageResult>;
}
/** fetchWithEdge 可选项（与 fetch-page 同口径） */
export interface EdgeFetchOpts { maxTextChars?: number; collectLinks?: boolean; waitUntil?: "domcontentloaded" | "load" | "networkidle"; waitMs?: number }
/** fetchWithEdge 返回（与 fetch-page 返回同形） */
export interface EdgePageResult { url: string; title: string; text: string; links: Array<{ text: string; href: string }>; length: number; invalid?: boolean }
...
function killStaleEdgeSession(): void {
  try {
...
// ========== Profile 探测与副本 ==========
function detectEdgeProfile(): EdgeProfile | null {
...
async function copyEdgeProfile(): Promise<{ ok: boolean; reason?: string; stale?: boolean }> {
  ...
  try { copyFileSync(localStateSrc, path.join(dest, "Local State")); } catch (e) {
    return { ok: false, reason: `Local State 拷贝失败: ${e instanceof Error ? e.message : String(e)}` };
  }
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      if (attempt === 4) {
        ...
        return { ok: false, reason: `Cookies 文件被独占锁（罕见），请稍后重试: ${e instanceof Error ? e.message : String(e)}` };
      }
  ...
  try {
    ...
    console.log(`[edge-session] Cookies 过滤失败（保留全量）: ${(e instanceof Error ? e.message : String(e)).slice(0, 80)}`);
  }
...
async function initSession(): Promise<{ ok: boolean; reason?: string }> {
  ...
  try {
    ...
    try {
      ...
      const isLoginPage = /登录|login|signin/i.test(testTitle) && testPage.url().includes("login");
    ...
    sessionError = `Edge 会话启动失败: ${e instanceof Error ? e.message : String(e)}`;
...
// ========== 导出 ==========
/** 取 Edge 登录态会话（复用真实 Edge 登录态抓取受保护页面） */
export async function getEdgeSession(): Promise<EdgeSessionResult> {
  const initResult = await initSession();
  if (!initResult.ok) return initResult;

  return {
    ok: true,
    fetchWithEdge: async (url: string, opts: EdgeFetchOpts = {}) => {
      ...
      const ctx = sessionCtx as BrowserContext;
      const page = await ctx.newPage();
      try {
        await page.goto(url, { waitUntil, timeout: config.navTimeout });
        await page.waitForTimeout(waitMs);
        const result = await page.evaluate(
          ({ collectLinks }): { title: string; html: string; links: Array<{ text: string; href: string }> } => {
        ...
        // jsdom 无类型声明：走非字面量 specifier（TS 不解析模块 → 不报 TS7016），形状本地收口。
        // （装了 @types/jsdom 会把 DOM 全局类型拉进来，反而让 tests/scripts 的 fetch mock 类型不兼容——不引入依赖）
        const jsdomSpec = "jsdom";
        const { JSDOM } = await import(jsdomSpec) as { JSDOM: new (html: string, opts?: Record<string, unknown>) => { window: { document: Document } } };
...
/** 关闭 Edge 会话（释放 profile 锁；下次 getEdgeSession 会重新初始化） */
export async function closeEdgeSession(): Promise<void> {
  if (sessionCtx) { try { await sessionCtx.close(); } catch { /* ignore */ } sessionCtx = null; }
  sessionReady = false;
  sessionError = null;
}
