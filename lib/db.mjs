...
export function ensureSchema() {
  ...
  `);
  // 迁移：旧库 chat_history 无 session_id 列（多会话功能）——ALTER 补列，旧消息归 'default' 会话
  // 注意：索引必须在 ALTER 之后建（旧库先建索引会因缺列崩溃）
  try {
    const cols = db.prepare("PRAGMA table_info(chat_history)").all().map((c) => c.name);
  ...
  } catch { /* 迁移失败不阻断（查询时按列存在性兜底） */ }
  // 迁移：review_cards 无 type 列（算法题标记——手写模式 + 多维自评）——ALTER 补列，存量卡按 topic 检测回填
  try {
    const rcCols = db.prepare("PRAGMA table_info(review_cards)").all().map((c) => c.name);
    if (!rcCols.includes("type")) {
      db.exec("ALTER TABLE review_cards ADD COLUMN type TEXT NOT NULL DEFAULT 'concept'");
    }
    // 迁移：priority 列（强化复习工单——必会/进阶/拓展，调度优先高优先级）
    if (!rcCols.includes("priority")) {
      db.exec("ALTER TABLE review_cards ADD COLUMN priority TEXT NOT NULL DEFAULT '拓展'");
    }
  } catch { /* 迁移失败不阻断 */ }
  // 记录版本
  db.prepare(
