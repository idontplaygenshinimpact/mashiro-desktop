...
export async function buildInterviewFocus() {
  ...
  // 7) 学习计划 scope 未完成单元（学习计划执行链路任务 2：计划是用户投入最重的承诺——优先考）
  // score 75：高于到期卡（70）——计划是主动投入（配额/周期），优先于被动积累的到期卡；
  // 低于复习错题（90）——答错多次的错题仍更优先
  try {
    const { getLearningPlans } = await import("./learning-plan.mjs");
    const { getPlan } = await import("./study.mjs");
    const activePlans = getLearningPlans().filter((p) => p.status === "active");
    if (activePlans.length) {
      const doneTopics = new Set((getPlan().items || []).filter((i) => i.done).map((i) => i.topic));
      for (const p of activePlans) {
        for (const s of p.scope || []) {
          if (!doneTopics.has(s)) add(s, `学习计划「${p.title}」未完成单元`, 75);
        }
      }
    }
  } catch { /* ignore */ }
