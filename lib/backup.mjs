...
import { readJsonSafe, writeJsonAtomic } from "./atomic-json.mjs";
...
const LAST_STATUS_FILE = () => path.join(path.dirname(DB_FILE), "backup-last-status.json");
...
export async function createBackup(reason = "manual") {
  try {
    ...
    const error = `备份失败: ${String(e?.message || e).slice(0, 200)}`;
...
export function recordBackupOutcome({ ok, reason = "", error = "", name = "" }) {
  try {
...
/** 读取上次备份结果（无记录返回 null） */
export function getLastBackupOutcome() {
  return readJsonSafe(LAST_STATUS_FILE(), null);
}

/** 清理超量备份（保留最近 maxBackups 份） */
export function pruneBackups() {  try {
