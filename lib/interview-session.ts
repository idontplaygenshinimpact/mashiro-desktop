...
// 全量 TS 升级工单阶段 3：lib/interview-session.mjs → .ts（lib/interview.mjs 桶 + tests 按 .mjs 路径加载 → 保留同名一行桶）
...
/** 优先考察队列项 */
export interface WeakQueueItem { topic: string; failCount: number; reason: string; asked: boolean }
/** 会话当前问题 */
export interface InterviewCurrent {
  round?: number;
  question?: string;
  basis?: string;
  dimension?: string;
 ...
 */
export interface InterviewSessionShape {
  finished?: boolean;
  position?: unknown;
  role?: unknown;
...
export interface EndInterviewResult {
  ...
  weakCovered?: number;
  weakTotal?: number;
  weakCoveredTopics?: string[];
  hint?: string;
}
/** getInterviewStatus 结果（面板"继续上一场"入口） */
...
function buildSubmitPrompt(session: InterviewSessionShape, current: InterviewCurrent, roundNum: number, roundIndex: number, currentRound: { type: string; name: string; desc: string }, nextRound: { name: string } | null, progress: string, depthLimit: number, pendingWeak: WeakQueueItem[], askedWeak: WeakQueueItem[], answer: unknown): string {
  return `你是${String(session.role)}面试官。面试进行到第 ${roundNum} 轮（面试长度动态：计划 ${ROUND_SEQ.length} 轮，**根据表现可提前结束（finish=true）或自动加试**——候选人表现充分/要求结束 → 提前收尾；薄弱点未考完 → 服务端会自动追加补考轮）。
...
本轮问题：${String(current.question || "")}
考察维度：${String(current.dimension || "")}
合格标准：${String(current.criteria || "")}
追问边界：${String(current.boundary || "")}
候选人回答：${String(answer).slice(0, 3000)}

面试历史：
${(session.rounds || []).map((r, i) => `轮${i + 1}【${r.question.slice(0, 50)}】答：${r.answer.slice(0, 80)} 分：${r.total}`).join("\n")}
...
/** Agent 化出题（两段式）：决策轮可检索项目资源（题库/知识库/薄弱点）→ 出题轮
...
async function askInterviewer(prompt: string, messages: Array<Record<string, unknown>> = []): Promise<{ raw: string; msgs: Array<Record<string, unknown>> }> {
  ...
  try {
    for (let i = 0; i < MAX_TOOL_ROUNDS; i++) {
      const res = await llmChat(msgs as unknown as AgentMessage[], { maxTokens: 3000, temperature: 0.3, tools: getInterviewerTools(), toolChoice: "auto" });
      const msg = res?.choices?.[0]?.message as unknown as (Record<string, unknown> & { tool_calls?: Array<{ id: string; function?: { name?: string; arguments?: unknown } }> }) | undefined;
      if (msg?.tool_calls?.length) {
        msgs.push(msg);
        for (const tc of msg.tool_calls.slice(0, 2)) {
          const tArgs = ivSafeParse(tc.function?.arguments);
          const tResult = await runInterviewerTool(String(tc.function?.name || ""), tArgs);
    ...
    const forced = await llmChat(
      [...msgs, { role: "system", content: "工具检索已结束（本轮工具调用达上限）：现在**禁止**再调用工具，直接输出面试官 JSON（五维评分 + 下一问）。" }] as unknown as AgentMessage[],
    ...
    const fallback = await llmChat([...msgs, { role: "user", content: "请直接输出面试官 JSON（scores/comment/next_kind/next_question 等字段），不要调用任何工具。" }] as unknown as AgentMessage[], { maxTokens: 2000, temperature: 0.3 });
    return { raw: getReplyText(fallback), msgs };
  } catch (e) {
    // 工具轮异常不阻塞出题：退化为单次调用（面试是核心流程，不能被检索失败拖垮）
    console.warn(`[interview] 面试官工具轮失败，退化单次出题: ${(e instanceof Error ? e.message : String(e)).slice(0, 120)}`);
    const res = await chat([...msgs], { maxTokens: 2000 });
    return { raw: getReplyText(res as unknown as Parameters<typeof getReplyText>[0]), msgs };
  }
}

/** 记录本轮 + 薄弱点回流（学习事件流/薄弱点/掌握度——失败不影响面试） */
async function recordRound(session: InterviewSessionShape, current: InterviewCurrent, answer: unknown, r: Record<string, unknown>, scores: Record<string, number>, total: number, weakQueue: WeakQueueItem[]): Promise<{ hitItem: WeakQueueItem | null; weakHitTopic: string | null }> {
  const weakHitTopic = cleanWeakTopic(r.weak_hit);
  const hitItem = weakHitTopic ? weakQueue.find((w) => !w.asked && w.topic === weakHitTopic) || null : null;
  if (hitItem) hitItem.asked = true;
  (session.rounds as InterviewRoundRecord[]).push({
  ...
  await recordKpFlow(String(current.question || ""), total);
...
export function isGiveUpAnswer(answer: unknown): boolean {
...
function advanceRound(session: InterviewSessionShape, r: Record<string, unknown>, scores: Record<string, number>, total: number, roundNum: number, roundIndex: number, currentRound: { type: string; name: string }, hitItem: WeakQueueItem | null, gaveUp = false): SubmitAnswerResult {
  ...
  const recentAvg = recent.length ? Math.round(recent.reduce((s, rr) => s + (rr.total || 0), 0) / recent.length) : 0;
  ...
  if (r.finish || roundNum >= MAX_ROUNDS || (seqExhausted && !extendWorthy)) {
    ...
    memory.setInterview(session as unknown as Parameters<typeof memory.setInterview>[0]);
    return {
  ...
  const current = session.current as InterviewCurrent;
  const depth0 = Number(current.depth) || 0;
  ...
  } else if (total < 50 && kind !== "followup" && qualityRound && depth0 < MAX_DEPTH - 1) {
    kind = "followup"; // 答错 → 强制追问（服务端兜底，不依赖 LLM 自觉）
  } else if (total >= 75 && kind === "followup") {
    kind = "new"; // 答得好 → 不再追问（服务端兜底：高质量还追问是浪费轮次）
  }
  const followup = kind === "followup" && depth0 < MAX_DEPTH; // 追问：同轮深挖（深度上限硬约束）
  const sameRound = kind === "new"; // 本轮内换题（不推进轮次）
  const depth = (followup || sameRound) ? (followup ? depth0 + 1 : 0) : 0;
  ...
  session.current = {
    ...
    question: String(r.next_question || "") || (kind === "followup"
      ? `请继续深入讲讲「${String(current.question || "").slice(0, 40)}」的${current.dimension || "技术"}方面——结合你的实际经验展开。`
      : gaveUp
    ...
    basis: String(r.next_basis || "") || "追问",
    dimension: r.next_dimension ? String(r.next_dimension) : current.dimension,
    criteria: r.next_criteria ? String(r.next_criteria) : current.criteria,
    boundary: r.next_boundary ? String(r.next_boundary) : current.boundary,
    depth,
...
/** 提交回答 → 评分 + 下一问 */
export async function submitAnswer(answer: unknown): Promise<SubmitAnswerResult> {
  const session = memory.getInterview() as unknown as InterviewSessionShape | null;
  ...
  const current = session.current as InterviewCurrent;
  const roundNum = Number(current.round) || 1;
  ...
  if (memory.getInterview() !== (session as unknown)) {
  ...
  const parsed = extractJson(raw) as Record<string, unknown> | null | undefined;
  ...
  const { scores, total } = parseScores(r.scores as Record<string, unknown> | null | undefined);
...
// ---------- 结束面试 → 复盘报告 ----------
export async function endInterview(): Promise<EndInterviewResult> {
  const session = memory.getInterview() as unknown as InterviewSessionShape | null;
  ...
  const roundsText = rounds.map((r, i) =>
  ...
  const avg = Math.round(rounds.reduce((s, r) => s + r.total, 0) / rounds.length);
  const dims = ["tech", "expr", "depth", "edge", "reflect"];
  const avgDims: Record<string, number> = {};
  for (const d of dims) {
    avgDims[d] = Math.round(rounds.reduce((s, r) => s + Number(r.scores[d] || 0), 0) / rounds.length);
  }
  ...
  const prompt = `你是面试复盘导师。下面是刚才的模拟面试记录（${String(session.role)}面试官，${String(session.position)}岗位，${rounds.length} 轮，均分 ${avg}）。
...
export function getInterviewStatus(): InterviewStatusResult {
  // 会话由 interview-session 定义形状——在边界断言（memory.ts 的 InterviewSession 只声明治理字段）
  const s = memory.getInterview() as unknown as InterviewSessionShape | null;
  ...
  const sum: Record<string, number> = { tech: 0, expr: 0, depth: 0, edge: 0, reflect: 0, total: 0 };
