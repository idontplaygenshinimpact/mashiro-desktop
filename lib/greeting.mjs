...
export function buildGreeting({ company = "", title = "" } = {}) {
  ...
  if (!snap) {
    return "您好，我是前端方向应届生，对贵司该岗位很感兴趣，这是我的简历，期待进一步沟通。";
  }
  ...
  if (hl.internCompany) identity.push(`${hl.internCompany}前端实习`);
  else if (snap.directions?.length) identity.push(`${snap.directions.includes("agent") ? "AI 应用前端" : "前端"}方向`);
  if (skills) identity.push(`熟悉 ${skills}`);
  parts.push(`您好！我是${identity.join(" · ")}。`);
  // 2) 项目亮点（最能打动 HR 的部分）
  if (hl.project) {
    const proj = hl.quant
      ? `独立开发过 ${hl.project}（${hl.quant}），对工程化与 AI 应用前端有实战经验`
      : `独立开发过 ${hl.project}，有完整前端项目落地经验`;
    parts.push(proj + "。");
  } else if (hl.quant) {
    parts.push(`有扎实的前端项目落地经验（${hl.quant}）。`);
  }
  // 3) 岗位意向 + 结尾
