...
export async function buildDigest(items = [], { today = null, llm = null } = {}) {
  ...
  if (cap.length) {
    const listText = cap
      ...
      .join("\n");
    const prompt = `你是前端求职技术资讯编辑。下面是从多个技术 RSS 源聚合到的今日资讯（标题+链接+时间）。请从中挑选 ${DIGEST_COUNT} 条**对「前端 / 求职 / 技术」最有价值**的资讯（前端框架/工程化/AI 前端/面试求职/工具生态优先，纯后端/硬件/无关闲聊跳过），并给每条写一句「为什么值得看」的理由（≤${REASON_MAX} 字）。
...
${sanitizeExternal(listText).wrapped}`;
  ...
  const digest = picks.slice(0, DIGEST_COUNT).map((pk) => {
  ...
  try {
    ...
    const ins = db.prepare(
      `INSERT INTO rss_items (feed, title, link, summary, reason, published_at, digest_date, seen)
       VALUES (?, ?, ?, ?, ?, ?, ?, 0)`
    );
    withTx(() => {
      del.run(date);
...
export async function runDailyDigest() {
  ...
  const ts = Date.now();
  setLastDigestAt(ts);
  return { digest, lastDigestAt: ts, feeds: feeds.length, total: items.length };
}
