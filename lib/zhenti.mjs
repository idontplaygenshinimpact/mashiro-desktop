...
import { clean } from "./text-utils.mjs"; // 技术债 L3：收敛单点
...
const COOKIE_FILE = () => path.join(DATA_DIR(), "nowcoder-cookie.json");
...
const SESSION_PROFILE = () => path.join(DATA_DIR(), "nowcoder-edge-profile");
let sessionCtx = null;
...
// clean 从 text-utils.mjs 导入（技术债 L3：收敛单点）
...
export async function collectZhentiDetails(limit = 20) {
  ...
  await Promise.all(
    rows.map(async (r) => {
      try {
        const page = await fetchPage(String(r.url), { maxTextChars: 2000, waitMs: 3000 });
