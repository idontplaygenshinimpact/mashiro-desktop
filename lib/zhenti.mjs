...
export async function fetchPaperQuestions(paperTestId) {
  ...
  if (existsSync(PROFILE_DIR)) {
    // 优先：Edge 内核持久化 profile（真实登录态）
    ctx = await chromium.launchPersistentContext(PROFILE_DIR, {
      channel: "msedge", headless: true, locale: "zh-CN", args: ["--disable-blink-features=AutomationControlled"],
    }).catch(() => null);
  }
  if (!ctx) {
    ...
    if (cookiePairs) await ctx.addCookies(cookiePairs.map((c) => ({ name: c.name, value: c.value, domain: ".nowcoder.com", path: "/" })));
  }
  try {
    ...
    // 点"开始答题"进入答题页（点击可能被登录弹窗拦截，失败静默）
    await page.locator("text=开始答题").first().click({ timeout: 8000 }).catch(() => {});
    // 轮询：等题目 DOM 出现（最多 12s）或检测登录弹窗
    const deadline = Date.now() + 12000;
    let questions = [];
    while (Date.now() < deadline) {
      // 登录弹窗判定：点答题后出现登录框 = 未登录/权限不足
      const popup = await page.locator("text=发送验证码, text=扫码登录").count().catch(() => 0);
      if (popup > 0) {
        return { ok: false, questions: [], error: "Cookie 无效或该试卷需登录/权限——请重新在浏览器登录牛客后复制新 Cookie，或运行 scripts/nowcoder-login.mjs 扫码登录" };
      }
    ...
    if (!questions.length) return { ok: false, questions: [], error: "未能提取到题目（答题页结构可能变化或需进一步交互）" };
    // 入库 exam_questions（重抓刷新：先删该试卷旧题）
    const now = Date.now();
    ...
    return { ok: true, questions, saved: questions.length };
  } finally {
    if (ctx && !browser) await ctx.close(); // persistent context（Edge profile）
    if (browser) await browser.close();     // 普通 chromium
  }
}
