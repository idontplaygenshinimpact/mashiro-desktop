...
export async function fetchPaperQuestions(paperTestId) {
  ...
  try {
    ...
    // 登录判定：点答题后出现登录弹窗（发送验证码/扫码登录）= 未登录或该试卷需权限
    const loginPopup = await page.locator("text=发送验证码, text=扫码登录").count().catch(() => 0);
    if (loginPopup > 0) {
      return { ok: false, questions: [], error: "Cookie 无效或该试卷需登录/权限——请重新在浏览器登录牛客后复制新 Cookie，或在牛客开通该试卷练习权限" };
    }
