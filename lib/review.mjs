...
function loadCards() {
  try {
    if (existsSync(CARDS_FILE)) return JSON.parse(readFileSync(CARDS_FILE, "utf8"));
  } catch { /* ignore */ }
  return { cards: [], lastReviewDate: "" };
}
function saveCards(db) {
  try {
    mkdirSync(DATA_DIR, { recursive: true });
    writeFileSync(CARDS_FILE, JSON.stringify(db, null, 2), "utf8");
  } catch { /* ignore */ }
}
