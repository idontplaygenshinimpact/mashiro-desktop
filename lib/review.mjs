...
async function generateMultiAngle(batch) {
...
export async function ensurePlanCoverage() {
  ...
  try {
    ...
    const levelByTopic = new Map(items.map((i) => [i.topic, i.level]));
    const priOf = (level) => (level === "必会" ? "必会" : level === "进阶" ? "进阶" : "拓展");
    // ① 补缺口：清单无卡条目 → 多角度补卡（幂等）
    const missing = items.filter((i) => !cardTopics.has(i.topic));
    let added = 0;
    ...
    const stale = cards.filter((c) => {
      const want = levelByTopic.get(c.topic);
    ...
    return { added: 0, error: String(e?.message || e).slice(0, 100) };
  } finally {
...
function loadCards() {
  const rows = db.prepare("SELECT id, topic, question, answer, source, type, priority, fsrs, created_at FROM review_cards").all();
...
export const review = {
  ...
  mergeSimilarCards() {
    ...
    let merged = 0;
    const keep = []; // 保留卡（按遍历序，簇内第一张为基准）
    for (const c of cards) {
      const dup = keep.find((k) => isSimilarTopicForArchive(String(c.topic), String(k.topic)));
      if (dup) {
        try {
          // question/answer 取更完整的（漂移卡可能带不同追问/答案）
          if (String(c.question || "").length > String(dup.question || "").length) {
            db.prepare("UPDATE review_cards SET question=?, updated_at=? WHERE id=?").run(c.question, Date.now(), dup.id);
          }
          if (String(c.answer || "").length > String(dup.answer || "").length) {
  ...
  addCard({ topic, question = "", answer = "", source = "", priority = "拓展" }) {    const data = loadCards();
    let card = data.cards.find((c) => c.topic === topic);
    if (!card) card = data.cards.find((c) => isSimilarTopicForArchive(topic, String(c.topic || "")));
    if (!card) {
      card = {
      ...
      if (card.topic === topic) {
        ...
        if (String(question || "").length > String(card.question || "").length) card.question = question;
        if (String(answer || "").slice(0, 500).length > String(card.answer || "").length) card.answer = answer.slice(0, 500);
      }
  ...
  reviewCard(id, rating) {
    ...
    const ratingNum = Number.isInteger(rating) && rating >= 0 && rating <= 3 ? rating : null;
    if (ratingNum === null) {
    ...
    let clearedWeak = null;
    if (ratingNum >= 2 && card.topic) {
      const hadWeak = memory.getWeakPoints().some((w) => w.topic === String(card.topic));
      memory.clearWeakPoint(String(card.topic));
      if (hadWeak) clearedWeak = String(card.topic);
    }
    ...
    try {
      withTx(() => {
        ...
        db.prepare("INSERT INTO card_reviews (card_id, rating, reviewed_at, is_first) VALUES (?, ?, ?, ?)")
          .run(id, ratingNum, Date.now(), isFirstReview ? 1 : 0);
      });
    } catch (e) {
  ...
  getDailySession(limit = 15) {
    const due = this.getDueCards();
    // 薄弱点集合（卡 topic 在薄弱点 → 优先）
    let weakSet = new Set();
    try { weakSet = new Set(memory.getTrustedWeakPoints(50).map((w) => w.topic)); } catch { /* ignore */ }
    const PRIORITY = { 必会: 0, 进阶: 1, 拓展: 2 };
    return due
      .map((c) => {
  ...
  getRetryQueue(limit = 10) {
    ...
    ).all(Date.now() - 3 * 24 * 3600 * 1000, limit);
    return rows.map((r) => ({
      id: r.card_id, topic: r.topic, question: r.question, answer: r.answer,
      type: r.type === "algo" ? "algo" : "concept", priority: r.priority || "拓展",
  ...
  getReviewFeedback() {
    ...
    const rows = db.prepare("SELECT rating, reviewed_at FROM card_reviews").all();
