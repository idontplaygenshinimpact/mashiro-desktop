...
/** 检测 topic 是否为算法题（标题/问题命中算法关键词 → type: 'algo'） */
...
function loadCards() {
  ...
  // 回填：逐行检测（id/topic 级 UPDATE，防覆盖用户已手改的 type）
  for (const r of rows) {
    if ((r.type || "concept") === "concept" && detectAlgoTopic(r.topic)) {
      try {
        db.prepare("UPDATE review_cards SET type='algo' WHERE id=?").run(String(r.id));
        r.type = "algo";
      } catch { /* ignore */ }
    }
  }
