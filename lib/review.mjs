...
// 记忆强度估算（FSRS 遗忘曲线 R(t,S)=e^(-t/S)，与 ts-fsrs forgetting_curve 同源）
...
function calcMemPct(fsrsState) {
  ...
  return Math.round(Math.exp(-t / fsrsState.stability) * 100);
}
...
export const review = {
  ...
  // 今天到期的卡片（按遗忘概率升序——最可能忘的先复习）
  // 记忆强度 memPct = e^(-elapsed/stability)，越低越危险；新卡（未复习）排最后（有 1 天缓冲期才进队列）
  getDueCards() {
    const data = loadCards();
    const now = new Date();
    ...
    return data.cards
      ...
      .sort((a, b) => {
        // 遗忘概率排序：memPct 越小越先；null（新卡）恒排最后
  ...
  getStats() {
    ...
    // 今日已完成复习次数（card_reviews 表：今天有评分记录的卡数）
    let todayDone = 0;
    try {
      const dayStart = new Date();
      dayStart.setHours(0, 0, 0, 0);
      todayDone = Number(db.prepare("SELECT COUNT(*) n FROM card_reviews WHERE reviewed_at >= ?").get(dayStart.getTime()).n || 0);
    } catch { /* ignore */ }
    return {
  ...
  getReviewTrend() {
    ...
    try {
      for (const r of db.prepare("SELECT reviewed_at FROM card_reviews").all()) {
        const d = new Date(Number(r.reviewed_at)).toISOString().slice(0, 10);
        byDay[d] = (byDay[d] || 0) + 1;
      }
    } catch { /* ignore */ }
    const out = [];
