 ...
 * @param {{ maxTextChars?: number, collectLinks?: boolean, waitUntil?: string, waitSelector?: string|null, apiPattern?: string|null, waitMs?: number, rawText?: boolean }} [opts] 抓取选项
 * @returns {Promise<{ url: string, title: string, text: string, links: Array<{text: string, href: string}>, invalid?: boolean, apiResponses?: Array<any>, length?: number, ok?: boolean }>} 页面内容
 */
export async function fetchPage(url, { maxTextChars = 60000, collectLinks = false, waitUntil = "domcontentloaded", waitSelector = null, apiPattern = null, waitMs = 800, rawText = false } = {}) {
  ...
  try {
    ...
    if (/context closed|browser closed|target closed|browser has been closed/i.test(String(e?.message || ""))) {
  ...
  try {
    ...
    // 修复：只拦主文档（resourceTypes: ["document"]）——此前 "**/*" 拦所有请求，牛客等页面大量子资源
    // 时 Playwright 的 route 处理导致 goto 超时（实测 30s TimeoutError）；子资源不拦（图片/脚本不执行，
    // 风险低），302 重定向的 SSRF 防护（document）保留
    await page.route("**/*", async (route) => {
      const reqUrl = route.request().url();
      try {
        const u = new URL(reqUrl);
        if (u.protocol === "http:" || u.protocol === "https:") {
          await assertPublicHostname(u.hostname); // 主文档/重定向：完整 DNS 校验
        }
