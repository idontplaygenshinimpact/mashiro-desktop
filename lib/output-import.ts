// lib/output-import.ts —— 手动导入面经到产出目录（与爬取产出同构）
 ...
 */
export function saveImportedPost({ title, content, source = "" }: { title: unknown; content: unknown; source?: string }, outputDir: string = config.outputDir): { ok: boolean; file?: string; name?: string; path?: string; error?: string } {
  ...
  try { mkdirSync(dir, { recursive: true }); } catch (e) { return { ok: false, error: "创建目录失败: " + (e instanceof Error ? e.message : String(e)).slice(0, 60) }; }
  ...
  try {
    ...
    return { ok: false, error: "写入失败: " + (e instanceof Error ? e.message : String(e)).slice(0, 60) };
  }
