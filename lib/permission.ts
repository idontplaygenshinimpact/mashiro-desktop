...
// 全量 TS 升级工单阶段 2 批次 S5：lib/permission.mjs 保留为桶（agent/routes/测试/脚本 13 处调用方零改动）
import { recordDecision } from "./trace.mjs";

const APPROVAL_TIMEOUT_MS = 60000; // 审批等待超时（默认拒绝）

/** 审批结果（allow 决定；timeout=超时默认拒绝；autoApproved=会话级延续） */
...
/** 待审批条目（pending Map 值：决策后 resolve + 清理 timer） */
...
const pending = new Map<string, PendingEntry>(); // toolName -> 待审批条目
const sessionApproved = new Set<string>();  // 会话级已批准的工具（本进程生命周期）
const sessionDenied = new Set<string>();    // 会话级已拒绝的工具（拒绝后本会话硬拦截，防 LLM 变参重试无限重发审批）
...
export function isSessionDenied(toolName: string): boolean {
  return sessionDenied.has(toolName);
}

/** 记录会话级拒绝（用户显式拒绝或审批超时） */
export function markSessionDenied(toolName: string): void {
...
export function requestApproval({ toolName, args, reason }: { toolName: string; args: unknown; reason?: string }): Promise<ApprovalResult> {
  ...
  const promise = new Promise<ApprovalResult>((resolve) => { resolveFn = resolve; });
...
/** 面板查询：当前待审批的请求列表 */
export function getPendingApprovals(): Array<{ toolName: string; reason: string; args: unknown; requestedAt: number }> {
...
export function resolveApproval(toolName: string, { allow, session = false }: { allow: boolean; session?: boolean }): { ok: boolean; error?: string } {
...
/** 会话级已批准清单（调试/展示用） */
export function getSessionApproved(): string[] {
...
export function resolveApprovalDecision(proposal: Record<string, unknown> | null | undefined, decision: unknown, reason?: string | null): "allow" | "deny" {
  const p = proposal || {};
  const allow = decision === "allow" || decision === true;
  const canonical = allow ? "allow" : "deny";
  try {
    recordDecision({
      sessionId: (p.sessionId ?? p.session_id ?? null) as string | null,
      decision: canonical,
      toolName: (p.toolName ?? p.tool ?? p.name ?? null) as string | null,
      reason: (reason ?? p.reason ?? null) as string | null,
      policyRef: (p.policyRef ?? p.policy_ref ?? p.policy ?? null) as string | null,
      approvedBy: canonical === "allow" ? (p.approvedBy as string | undefined ?? "user") : null,
    });
