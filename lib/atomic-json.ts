...
// 全量 TS 升级工单阶段 1⑤：lib/atomic-json.ts → .ts（读侧泛型化——调用方拿到 fallback 的类型而非 any）
...
/** 读 JSON（带 .tmp 残留清理）；主文件与 tmp 都不可用 → 返回 fallback
 * 泛型 T = fallback 类型（JSON.parse 是 any，由此收口为调用方声明的形状） */
export function readJsonSafe<T>(file: string, fallback: T): T {
  ...
  try {
    if (existsSync(file + ".tmp")) return JSON.parse(readFileSync(file + ".tmp", "utf8")) as T;
  } catch { /* ignore */ }
  return fallback;
}
