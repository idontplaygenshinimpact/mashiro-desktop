...
// 全量 TS 升级工单阶段 2 批次 S3：lib/agent.mjs 保留为桶（35+ 调用方零改动）
...
} from "./tools/exec-tools.ts";
import { ensureMcp, getMcpToolsCache } from "./tools/mcp.mjs";
import type { ToolPolicy } from "./tool-policy.ts";
import type { LLMChatOpts, LLMResponse, LLMMessage, LLMToolCall } from "./llm.ts";
...
/** 工具参数（LLM 产出经 safeParse——值类型未知，边界收窄） */
type ToolArgs = Record<string, unknown>;

/** 工具执行结果（成功 {ok} / 失败 {error, hint, kind}；kind=T2 失败分类） */
interface ToolExecResult {
  ok?: boolean;
  error?: string;
  hint?: string;
  kind?: "param" | "perm" | "transient" | "hard";
  [k: string]: unknown;
}

/** 对话消息（LLM 协议最小形状——tool_calls/占位响应共存；content 一律字符串化） */
...
/** 工具策略（惰性初始化；失败回退"全放行"——只要 filterTools/effectiveLevel） */
type MinimalToolPolicy = Pick<ToolPolicy, "filterTools" | "effectiveLevel">;

// ---------- 权限分级（human-in-the-loop） ----------
// auto：自动执行（只读/无副作用/流程内/用户已明确意图）；confirm：需用户批准（写库/大开销/外部影响）
// 对标 Claude Code permission：敏感操作 deny-first，用户批准后本会话同类不再询问
// 架构 P0-1 修复：全量显式列出全部内置工具（此前只列 7 项——browse_*/job_search_platform/spawn_subagent
// 静默 auto，与 tool-policy 宣称的 confirm 脱节）；未列出工具 fail-closed → confirm
const TOOL_PERMISSIONS: Record<string, string> = {
  // --- 只读/查询（auto） ---
  plan_task: "auto",
  search_posts: "auto",
  fetch_nowcoder_user: "auto",
  search_knowledge: "auto",
  web_search: "auto",
  fetch_page: "auto",
  detect_questions: "auto",
  remember: "auto",
  get_weak_points: "auto",
  get_memory: "auto",
  get_study_plan: "auto",
  get_recent_outputs: "auto",
  get_learning_plan_status: "auto",
  read_tool_result: "auto",
  ask_user: "auto",
  plan_mode: "auto",
  todo_init: "auto",
  todo_done: "auto",
  ...
  browse_open: "confirm",                  // 外部网站浏览（有外部影响/SSRF 攻击面）——此前静默 auto
  browse_click: "confirm",
  browse_scroll: "confirm",
  browse_type: "confirm",
  browse_screenshot: "confirm",
  browse_fetch: "confirm",
  job_search_platform: "confirm",          // 外部平台搜岗 + 入库——此前静默 auto
  spawn_subagent: "confirm",               // 子任务执行器（写文件/耗 LLM）——此前静默 auto
};
...
function checkArgValue(k: string, v: unknown, schema: ArgSchema | undefined): string | null {
  ...
  if (schema.type === "array" && schema.items?.type) {
    for (let i = 0; i < (v as unknown[]).length; i++) {
      const err = checkArgValue(`${k}[${i}]`, (v as unknown[])[i], schema.items);
      if (err) return err;
    }
  }
  return null;
...
async function checkToolPermission(name: string, args: unknown, _tStart: number): Promise<{ allow: boolean; error?: string; hint?: string }> {
  ...
  let level: string | undefined = TOOL_PERMISSIONS[name];
  if (!level && name.startsWith("mcp__")) {
    try {
      const { getMcpPermission } = await import("./mcp-client.ts");
  ...
  // 架构 P0-1：内置工具未列出 → fail-closed confirm（此前 undefined 走 auto 静默放行——
  // 与 tool-policy 宣称的 confirm 脱节；新增工具默认敏感，不静默放行）
  // 但**未知工具**（LLM 幻觉的工具名——不在任何工具列表）直接报错不审批：
  // 走审批会让幻觉工具触发审批流程（benchmark 场景"未知工具报错不崩溃"失效）
  if (!level) {
    const known = TOOL_PERMISSIONS[name] || name.startsWith("mcp__") || name.startsWith("skill__");
    if (!known) return { allow: false, error: `未知工具: ${name}` };
    level = "confirm";
  }
  if (level !== "confirm") {
  ...
  try {
    ...
    return { allow: false, error: `审批系统异常: ${e instanceof Error ? e.message : String(e)}` };
  }
}

async function executeTool(name: string, args: ToolArgs): Promise<ToolExecResult> {
  ...
  if (deniedToolNames.has(name)) {
    ...
    return { error: msg, kind: "perm" }; // T2：策略禁用 = 权限类（换方案，不重试）
  }
  ...
  if (v.ok !== true) {
    ...
    return { error: v.error, hint: "请检查参数后重新调用该工具", kind: "param" }; // T2：参数类（检查后重试）
  }
  args = v.args;
  ...
  try {
    ...
    const hookResults = await emitHook("before_tool", { toolName: name, args }) as Array<{ deny?: unknown; reason?: unknown } | null | undefined>;
    const denied = (hookResults || []).find((r) => r && r.deny);
    if (denied) {
      const msg = String(denied.reason ?? "无原因");
      try { const { traceTool } = await import("./trace.mjs"); traceTool({ toolName: name, args, ok: false, error: msg, durationMs: Date.now() - _tStart }); } catch { /* ignore */ }
      return { error: `工具被策略拦截: ${msg}`, hint: "请改用其他工具或说明原因", kind: "perm" }; // T2：策略拦截 = 权限类
    }
  } catch { /* hooks 失败不阻断工具执行 */ }
  // 权限门禁（内置 confirm 工具 + MCP 工具统一走审批，堵住 MCP 绕过漏洞）
  const perm = await checkToolPermission(name, args, _tStart);
  if (!perm.allow) return { error: perm.error, hint: perm.hint, kind: "perm" }; // T2：权限拒绝 = 权限类
  // 编排能力缺口工单 T1：计划待确认门禁——plan_task 创建计划后，必须先 plan_mode 确认才能执行步骤
  // （防 LLM 跳过确认直接执行；plan_mode/plan_task/todo_*/ask_user 为确认与推进工具，不受限）
  if (!["plan_mode", "plan_task", "todo_init", "todo_done", "ask_user"].includes(name)) {
    try {
      const { getPlan } = await import("./todo.mjs");
      const plan = getPlan();
      if (plan && plan.status === "pending") {
        const msg = `计划「${plan.goal}」待用户确认——必须先调用 plan_mode 提交计划内容，用户确认后才能执行步骤`;
        try { const { traceTool } = await import("./trace.mjs"); traceTool({ toolName: name, args, ok: false, error: msg, durationMs: Date.now() - _tStart }); } catch { /* ignore */ }
        return { error: msg, hint: "调用 plan_mode 让用户确认计划（不要跳过确认直接执行）", kind: "perm" }; // T2：流程门禁 = 权限类
      }
    } catch { /* 计划模块不可用不阻断 */ }
  }
  ...
  if (name.startsWith("mcp__")) {
    try {
      ...
      try {
        const { recordDecision } = await import("./trace.mjs");
        recordDecision({ decision: "tool_error", toolName: name, reason: String(e instanceof Error ? e.message : e).slice(0, 200) });
      } catch { /* ignore */ }
      return { error: `MCP 调用异常: ${e instanceof Error ? e.message : String(e)}`, kind: "transient" }; // T2：MCP 网络/超时类 = 瞬时（可重试）
    }
  }
  ...
  if (name.startsWith("skill__")) {
    try {
      ...
      try { const { traceTool } = await import("./trace.mjs"); traceTool({ toolName: name, args, ok: false, error: e instanceof Error ? e.message : String(e), durationMs: Date.now() - _tStart }); } catch { /* ignore */ }
      return { error: `skill 调用异常: ${e instanceof Error ? e.message : String(e)}`, kind: "hard" }; // T2：skill 异常 = 硬错误
    }
  }
  ...
  try {
    ...
    switch (name) {
      case "plan_task":
        // 编排能力缺口工单 T1：计划写库（PlanStore）——不再口头承诺；返回计划 id + 强制 plan_mode 确认
        {
          const { initPlan } = await import("./todo.mjs");
          const r = initPlan({ goal: String(args.goal ?? ""), steps: (args.steps as Array<{ title?: string } | string> | undefined) ?? [] });
          if (r.ok !== true) return { error: r.error, hint: "计划需要目标与至少一个步骤" };
          return {
            ok: true,
            planId: r.plan.id,
            goal: r.plan.goal,
            steps: r.plan.steps.map((s: { title?: string }) => s.title),
            message: `计划已创建（${r.plan.steps.length} 步）——**必须先调用 plan_mode 让用户确认**，确认后才能执行步骤`,
            hint: "下一步：调用 plan_mode 提交计划内容让用户确认",
          };
        }
      case "search_posts":
        // 搜索结果标题来自外部搜索引擎/页面，包裹为不可信数据
        {
          const r = await withRetry(() => toolSearchPosts(String(args.query ?? ""), args.site as string | undefined), 2) as ToolExecResult;
          const results = r?.results as Array<{ error?: unknown; title?: unknown }> | undefined;
          if (results) r.results = results.map((p) => (p.error ? p : { ...p, title: wrapUntrusted(String(p.title)) }));
          return r;
        }
      case "search_knowledge":
        return await execSearchKnowledge(args as { query?: string; topK?: number });
      case "web_search":
      ...
      case "detect_questions":
        return await toolDetectQuestions(args as { title: string; text: string });
      case "remember":
        return await toolRemember((args.topics as string[] | undefined) ?? []);
      ...
      case "create_review_card":
        return await toolCreateReviewCard(args as { topic: string; question?: string; answer?: string });
      case "get_recent_outputs":
        return await toolGetRecentOutputs();
      case "start_interview":
        return await execStartInterview(args as { position: string; role?: string; resume?: string; focus?: string });
      case "submit_answer":
        return await execSubmitAnswer(args);
      case "end_interview":
        return await execEndInterview();
      case "record_interview_topics":
        return await toolRecordInterviewTopics((args.topics as string[] | undefined) ?? [], (args.company as string | undefined) ?? "");
      case "spawn_subagent":
      ...
      case "read_tool_result":
        return await toolReadToolResult(String(args.file ?? ""));
      case "browse_open":
      case "browse_click":
      case "browse_scroll":
      case "browse_type":
    ...
    return { error: `${name} 执行失败: ${e instanceof Error ? e.message : String(e)}`, kind: "hard" }; // T2：执行异常 = 硬错误（收束）
  }
  ...
  // T2：工具业务失败默认 hard（工具内部已用 withRetry 处理瞬时错误——业务错误重试无意义）
  if (resultError && !result.kind) result.kind = "hard";
  try {
...
export async function chatWithAgent(
  ...
  onEvent: ((e: { type: string; [k: string]: unknown }) => void) | null = null,
  sessionId = "default",
): Promise<{ reply: string; voice: string; history: Array<{ role: string; content: string }> }> {
  // 过程事件（可观测性：面板对话实时展示 agent 在做什么——工具调用开始/结果/失败）
  // onEvent 可选；默认 noop（现有非流式调用方/测试不受影响）
  const emit = (type: string, payload: Record<string, unknown>) => { try { onEvent?.({ type, ...payload }); } catch { /* ignore */ } };
  ...
  const displayArgs = (args: Record<string, unknown>): Record<string, string> => {
  ...
  // 新对话（前端未带 history）= 新会话边界：重置会话级审批状态 + 清空计划
  // （防常驻进程数天运行后，历史某次"本会话不再询问"的批准延续到所有后续对话；
  //   编排能力缺口工单 T1：计划不跨会话继承——新对话从零开始）
  if (!history?.length) {
    try {
    ...
    try {
      const { clearPlan } = await import("./todo.mjs");
      clearPlan();
    } catch { /* ignore */ }
  }
  ...
  let messages: AgentMessage[] = [
    {
      ...
      content:
        ...
        "10b) 搜面经/笔试真题/面试题：**优先用搜面经工具（search_posts——牛客/掘金）**，不要用 web_search——Bing 中文搜索质量差（搜'美团'会返回'美的/美图'无关结果）；用户要'XX公司 笔试/面经/真题/题目'时直接 search_posts。\n" +
  ...
  // 编排能力缺口工单 T4：相关记忆动态注入——按用户当前问题检索 curated/薄弱点/近期对话
  // （query 相关条目优先；无命中保留固定 top-8 兜底——行为兼容）
  try {
    const relevant = await memory.retrieveRelevant(String(userMsg), 8);
    if (relevant.length) {
      const lines = relevant.map((r) => `- ${r.text}（来源:${r.source}）`).join("\n");
      messages[0] = { ...messages[0], content: `${messages[0].content}\n\n## 相关记忆（与当前问题相关，带来源）\n${lines}` };
    } else {
      // 兜底：固定 top-8 长期记忆（dreaming 夜间整合的 curated_memory）
      messages[0] = { ...messages[0], content: memory.injectCuratedIntoPrompt(String(messages[0].content), 8) };
    }
  } catch { /* 注入失败不影响对话 */ }
  // 编排能力缺口工单 T1：计划状态注入 system prompt（第 N/M 步 + 未完成步骤——防 LLM 中途跑偏/跳过步骤）
  try {
    const { planStatusText } = await import("./todo.mjs");
    const planText = planStatusText();
    if (planText) messages[0] = { ...messages[0], content: `${messages[0].content}\n\n${planText}` };
  } catch { /* 计划模块不可用不注入 */ }
  ...
  const toolCallStats = new Map<string, { count: number }>(); // key -> { count }（仅统计连续调用，出现不同 key 即重置，防合法重复被误判）
  let lastToolKey: string | null = null;
  const TOOL_REPEAT_LIMIT = 3;     // 同一参数连续调用上限
  const TOOL_FAIL_LIMIT = 3;       // 同一工具连续失败上限
  const toolFails = new Map<string, number>();     // toolName -> 连续失败数（按工具独立计数，跨工具失败不再误收束）
  for (; rounds < MAX_ROUNDS; rounds++) {
    ...
    try {
      const { recordContextUsage } = await import("./context-meter.ts");
      recordContextUsage(messages, rounds);
    } catch { /* ignore */ }
    // 架构评审遗留收尾工单任务 2：流式中途抛错（SSE 中断/abort）→ 收束不崩——错误回填给 LLM 继续
    let res: LLMResponse | undefined;
    try {
      res = await callLLM(messages, (delta) => emit("delta", { delta }));
    } catch (e) {
      console.log(`[agent] 流式调用中断: ${String(e instanceof Error ? e.message : e).slice(0, 120)}`);
      messages.push({
        role: "tool",
        tool_call_id: `interrupt_${rounds}`,
        content: JSON.stringify({ error: `流式响应中断: ${String(e instanceof Error ? e.message : "未知错误").slice(0, 120)}`, hint: "基于已有信息总结回答，或让用户重试。" }),
      });
      // 中断后给 LLM 一次收束机会（不再调用工具）
      try {
        const finalRes = await callLLM(messages, (delta) => emit("delta", { delta }));
        reply = finalRes.choices?.[0]?.message?.content || "";
      } catch { reply = "对话流式传输中断，请重试。"; }
      break;
    }
    ...
    if (msg?.tool_calls?.length) {
      messages.push(msg as unknown as AgentMessage); // assistant 消息（含 tool_calls）
      ...
      let loopGuard = false;
      // 闭包内属性窄化失效：先捕获 const（推位函数跨函数边界读 tool_calls）
      const calls = msg.tool_calls;
      // 为当前 assistant 消息里尚未执行的剩余 tool_call 补占位响应（否则下轮 callLLM 缺 role:"tool" 响应 → provider 报错）
      const pushPlaceholdersFrom = (startIdx: number) => {
        for (let j = startIdx; j < calls.length; j++) {
          messages.push({
            role: "tool",
      ...
      for (let i = 0; i < calls.length; i++) {
        ...
        if (!name) continue; // 无工具名（协议异常）——跳过该调用，避免误计死循环
        ...
        // 编排能力缺口工单 T2：失败处理表——按 kind 分类处理
        //   transient（网络/超时）不计入连续失败（工具内部 withRetry 已重试，抖动不触发收束）
        //   param/perm/hard 计入；按 kind 注入"换策略提示"（失败反思：不重复犯同类错）
        const kind = result?.kind || (result?.error ? "hard" : null);
        const failCount = (toolFails.get(name) || 0) + (result?.error && kind !== "transient" ? 1 : 0);
        toolFails.set(name, result?.error && kind !== "transient" ? failCount : 0);
        if (result?.error && kind) {
          const strategyHint = {
            param: "参数有误——检查工具参数（必填/类型/枚举）后重试，不要原样重复",
            perm: "该操作被拒绝/受限——改用只读或替代方案，不要反复尝试",
            transient: "网络/服务瞬时错误——稍后重试或换一种方式",
            hard: "该工具执行失败——停止重试，基于已有信息回答或换工具",
          }[kind];
          if (strategyHint) result.hint = result.hint ? `${result.hint}；${strategyHint}` : strategyHint;
        }
  ...
  memory.appendChat("user", String(userMsg), sessionId);
...
let toolPolicy: MinimalToolPolicy | null = null;
let deniedToolNames = new Set<string>();
async function ensureToolPolicy(): Promise<MinimalToolPolicy> {
  if (toolPolicy) return toolPolicy;
  // 惰性动态 import（避免循环依赖/模块加载副作用）；失败回退"全放行"（策略不可用不能卡住对话）
  try {
    const mod = await import("./tool-policy.ts");
    toolPolicy = mod.createToolPolicy({ profiles: mod.DEFAULT_PROFILES, overrides: {} });
  } catch (e) {
    console.log(`[agent] 工具策略初始化失败(回退全放行): ${String(e instanceof Error ? e.message : e).slice(0, 80)}`);
    toolPolicy = {
      filterTools: (tools: unknown[]) => ({ allowed: tools, hidden: [], hiddenCount: 0 }),
      effectiveLevel: () => ({ level: "allow", source: "builtin", key: "*", reason: "fallback" }) as never,
    };
  }
  return toolPolicy;
}

async function callLLM(messages: AgentMessage[], onDelta: ((delta: string) => void) | null = null): Promise<LLMResponse> {
  ...
  const fullTools: unknown[] = [...TOOLS, ...mcpTools, ...skillTools];
  // 知识库开关关闭时隐藏 search_knowledge 工具（不让 LLM 白调；schema 也不发）
  try {
    const { ragEnabled } = await import("./rag.ts");
    if (!ragEnabled()) {
      const idx = fullTools.findIndex((t) => (t as { function?: { name?: string } })?.function?.name === "search_knowledge");
  ...
  if (skillHintsPrompt) {
    ...
    if (sysIdx >= 0 && !String(messages[sysIdx].content).includes("可用技能")) {
      messages[sysIdx] = { ...messages[sysIdx], content: String(messages[sysIdx].content) + skillHintsPrompt };
    }
  }
  const filtered = policy.filterTools(fullTools, { activeProfile });
  deniedToolNames = new Set((filtered.hidden as Array<{ function?: { name?: string } }>).map((t) => t?.function?.name).filter((n): n is string => Boolean(n)));
  ...
  const opts: LLMChatOpts = { tools: filtered.allowed as LLMChatOpts["tools"], temperature: 0.4, maxTokens: 6000, timeout: 30000 };
  if (onDelta) {
    // llmChatStream 可能返回纯文本（普通流）或 {choices}（工具调用）；统一成 {choices} 结构
    const r = await llmChatStream(messages as LLMMessage[], opts, onDelta);
    return typeof r === "string" ? { choices: [{ message: { content: r, role: "assistant" } }] } : r;
  }
  return await llmChat(messages as LLMMessage[], opts);
}

function safeParse(s: unknown): ToolArgs {
