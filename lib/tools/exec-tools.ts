 ...
 */
export async function execWebSearch(args: { query?: string } = {}) {
 ...
 */
export function execStartInterview(args: { position: any; role?: string; resume?: string; focus?: string }) {
 ...
 */
export function execSubmitAnswer(args: { answer?: string } = {}) {
 ...
 */
export async function execSpawnSubagent(args: { name?: string; system?: string; task?: string; context?: string; maxResult?: number; maxContext?: number; maxTokens?: number } = {}) {
 ...
 */
export async function execJobSearchPlatform(args: { platform?: string; keyword?: string; limit?: number } = {}) {
  try {
 ...
 */
export async function execJobApply(args: { id?: string; platform?: string; url?: string; greeting?: string; jobId?: string } = {}) {
  try {
...
export async function execAskUser(args: { question?: string; options?: Array<{ label: string; description?: string }>; multiSelect?: boolean } = {}) {
  try {
    ...
    const r = await askUser({ question: String(args.question || ""), options: opts, multiSelect: !!args.multiSelect });
    if (r.timeout) {
...
export async function execPlanMode(args: { plan?: string } = {}) {
  try {
    ...
    if (r.selected.includes("✅ 执行")) {
      // 编排能力缺口工单 T1：确认后标记计划 confirmed（PlanStore 状态机推进）
      try {
        const { confirmPlan } = await import("../todo.mjs");
        confirmPlan();
      } catch { /* 计划模块不可用不影响确认 */ }
      return { ok: true, approved: true, hint: "用户已确认计划，开始按步骤执行" };
    }
    if (r.selected.includes("✏️ 修改")) return { ok: false, approved: false, modify: true, hint: `用户要求修改：${r.reason || "请询问具体修改点"}。修改后重新提交 plan_mode` };
    // 用户取消 → 计划标记 cancelled（不再拦截后续工具）
    try {
      const { cancelPlan } = await import("../todo.mjs");
      cancelPlan();
    } catch { /* ignore */ }
 ...
 */
export async function execTodoInit(args: { items?: Array<any> } = {}) {
  try {
 ...
 */
export async function execTodoDone(args: { index?: number; content?: string; done?: boolean } = {}) {
  try {
    const { updateTodoItem } = await import("../todo.mjs");
    const r = updateTodoItem({ index: args.index ?? null, content: args.content, done: args.done !== false });
    if (!r.ok) return { error: r.error, hint: "检查序号或内容后重试" };
    // 编排能力缺口工单 T1：todo_done 完成时联动推进 PlanStore（每完成一步 currentStep 后移）
    let planHint = "";
    try {
