...
// ---------- 牛客用户主页全部面经抓取（逛完方案任务 1：contentPage 分页循环，SSR 无需登录） ----------
// 研究结论（2026-09）：牛客用户主页动态列表分页参数是 ?contentPage=N（不是 page/pageIndex），
// SSR 直接渲染无需登录；HTML 内联 JSON 含完整面经全文（"contentId":"...","title":"...","content":"..."）。
// 置顶帖每页重复 → 按 contentId 去重。真白此前 browse 工具绕远路失败——Node fetch 直连即可。
const NOWCODER_UA =
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36";

/**
 * 抓取牛客用户主页全部动态（面经全文）——contentPage 分页循环 + contentId 去重
...
export async function toolFetchNowcoderUser({ userId, maxPages = 5 }: { userId: string; maxPages?: number }) {
  ...
  for (let p = 1; p <= maxPages; p++) {
    const url = p === 1 ? `https://www.nowcoder.com/users/${uid}` : `https://www.nowcoder.com/users/${uid}?contentPage=${p}`;
    let html;
    try {
      const r = await fetch(url, {
        headers: { "User-Agent": NOWCODER_UA, "Accept-Language": "zh-CN,zh;q=0.9" },
    ...
    if (pageInfo) totalPages = Number(pageInfo[2]) || 1;
    const nm = html.match(/"nickname":"([^"]+)"/);
    if (nm) nickname = nm[1];
    const idm = html.match(/"authDisplayInfo":"([^"]+)"/);
    if (idm) identity = idm[1];
    // 解析动态块（contentId + title + content 完整面经全文）
    const re = /"contentId":"(\d+)","contentType":74.*?"title":"((?:[^"\\]|\\.)*)","newTitle".*?"content":"((?:[^"\\]|\\.)*)","newContent"/gs;
    let mm;
    let found = 0;
    while ((mm = re.exec(html))) {
      ...
      const unesc = (s: string) =>
        s
          ...
          .replace(/&amp;/g, "&")
          .replace(/&quot;/g, '"');
      moments.push({ id, title: unesc(mm[2]), content: unesc(mm[3]).slice(0, 8000) });
    }
    if (found === 0 && p > 1) break; // 第 2 页起无新内容 → 到底
    if (p >= totalPages) break;
  }
  if (!moments.length) return { ok: false, error: "未解析到动态（页面结构可能变化，可换 browse 工具）" };
  return { ok: true, user: { userId: uid, nickname, identity }, moments, totalPages };
}
