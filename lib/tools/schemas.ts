// lib/tools/schemas.ts —— 工具 schema 定义（纵向拆分：从 lib/agent.mjs 迁出）
// 纯数据模块（零依赖）：DeepSeek function calling 格式的工具清单，供 agent 循环与 MCP server 复用
// TS 升级（阶段 3）：本文件为唯一实现，lib/tools/schemas.mjs 降为同名薄桶（引用方零改动）

/** JSON Schema 节点（工具参数 schema：只声明用到的字段，其余键透传） */
...
/** OpenAI function calling 工具声明（TOOLS 元素） */
...
export const TOOLS: ToolSchema[] = [
  ...
  },
  {
    type: "function",
    function: {
      name: "fetch_nowcoder_user",
      description: "抓取牛客用户主页的全部动态/面经（含完整面经全文）。用户给牛客主页链接/用户 id/昵称时用——自动分页抓全（contentPage 循环，SSR 无需登录）+ 去重。比 browse 工具快且稳。",
      parameters: {
        type: "object",
        properties: {
          userId: { type: "string", description: "牛客用户 id（主页 URL 里的数字，如 500303394）" },
          maxPages: { type: "number", description: "最多抓几页（默认 5，每页约 8-10 条动态）" },
        },
        required: ["userId"],
      },
    },
