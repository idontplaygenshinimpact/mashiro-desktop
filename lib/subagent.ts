...
import { llmChat, getReplyText, type LLMMessage } from "./llm.mjs";
import { UNTRUSTED_DECLARATION, safeExternalBlock } from "./prompt-guard.mjs";
import { executeSubagentTool, SUBAGENT_TOOLS } from "./subagent-tools.ts";
...
/** runSubagent 选项（tools：工具白名单——调用方控制能力面；root：路径白名单根） */
...
/** 子任务结果（ok 成功带 result；失败带 error；超时/轮次上限同理） */
...
export async function runSubagent(opts: SubagentOpts = {}): Promise<SubagentResult> {
  ...
  for (let rounds = 0; rounds < MAX_ROUNDS; rounds++) {
    ...
    try {
      ...
      return { ok: false, error: `子任务失败: ${String(e instanceof Error ? e.message : e).slice(0, 150)}`, durationMs: Date.now() - t0, rounds };
    }
    ...
    for (const tc of msg.tool_calls) {
      ...
      try {
        ...
        result = { error: `工具执行异常: ${String(e instanceof Error ? e.message : e).slice(0, 100)}` };
      }
      messages.push({
        role: "tool",
        tool_call_id: tc.id,
...
/** v1 单次调用（无工具退化路径） */
async function runSingle({ sys, user, maxTokens, temperature, maxResult, t0 }: { sys: string; user: string; maxTokens: number; temperature: number; maxResult: number; t0: number }): Promise<SubagentResult> {
  try {
    ...
    return { ok: false, error: `子任务失败: ${String(e instanceof Error ? e.message : e).slice(0, 150)}`, durationMs: Date.now() - t0 };
  }
}
