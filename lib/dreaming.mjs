...
function defaultLogDir() {
  return path.join(import.meta.dirname, "..", "data", "dreams");
}
...
export async function runDreaming({ llm, now = Date.now(), dbImpl, logDir } = {}) {
  try {
    ...
    // 3. 写 curated_memory（INSERT OR REPLACE 按 topic 去重；已存在主题记为 updated）
    ...
    for (const e of parsed.entries || []) {
      ...
      if (prev) updated++; else { added++; addedTopics.push(topic); }
    }
    ...
    for (const t of parsed.drop || []) {
      ...
      if (!topic) continue;
      try { dbi.prepare("DELETE FROM curated_memory WHERE topic = ?").run(topic); dropped++; } catch { /* ignore */ }
    }
