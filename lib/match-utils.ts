...
// （study-groups.ts、knowledge.mjs matchKp）/ 正则方向判定（ai.mjs）/ bigram 编辑距离
// （followup-cache.mjs）/ normName includes（study-files.ts）各自为政——每次修复只补
// 单个调用点，组合词排除/独立成词/特异性门槛/词边界经验没有沉淀，其他调用点继续踩同类坑。
// 本模块统一：组合词表（一处更新全局生效）+ 独立成词 + 词边界 + n-gram + 组合词正则生成器。
// 红线：纯规则零 LLM 判断，零成本。
// 全量 TS 升级工单阶段 1②：lib/match-utils.mjs → .ts（纯函数零依赖叶子模块——迁移风险最低的一类）

// ---------- 1) 组合词表（统一维护） ----------
// 单字/双字强语义词（数据结构等）命中时，若文本含已知组合词（该词作为组合词一部分），
// 视为组合词不独立 → 不命中。只排除"非目标语义"的组合——"二叉树"含"树"但二叉树本身
// 就该命中算法组，不在表内。
export const COMPOUND_EXCLUDE: string[] = [
...
export function kwHit(text: unknown, kw: string): boolean {
...
export function hasWordBoundary(long: unknown, short: unknown): boolean {
...
export function grams(s: unknown, n: number): Set<string> {
...
export function compoundRegex(words: readonly string[] = COMPOUND_EXCLUDE): RegExp {
...
export function hasSpecificKw(hitKws: readonly unknown[]): boolean {
