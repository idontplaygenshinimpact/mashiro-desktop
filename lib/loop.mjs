...
export function loopSuggest() {
  ...
  try {
    ...
    if (activePlans.length) {
      const doneTopics = new Set((plan.items || []).filter((i) => i.done).map((i) => i.topic));
...
export function suggestFocusGoal(limit = 3) {
  const out = [];
  const seen = new Set(); // 已推荐 topic（归一化，防跨源重复）
  const push = (text, topic) => {
  ...
  // 1) 到期复习卡（遗忘曲线最急）
  try {
    for (const c of review.getDueCards().slice(0, 4)) push(`复习：${c.topic}`, c.topic);
  } catch { /* ignore */ }
  // 2) 薄弱点（failCount 高优先）
  try {
    for (const w of memory.getTrustedWeakPoints(8)) push(`补强：${w.topic}`, w.topic);
  } catch { /* ignore */ }
  // 3) 清单未完成
  try {
    const plan = getPlan();
