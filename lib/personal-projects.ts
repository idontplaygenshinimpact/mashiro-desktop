// lib/personal-projects.ts —— 简历个人项目源码 → 项目档案（清单讲解/学习文档素材）
// 思路：简历项目源码是"模型训练数据里没有、但讲解必须落到真实代码"的独家信息。
// 扫描用户配置的项目目录，生成结构化"项目档案"（技术栈/目录树/README 摘要/核心源码预览）
// 存 knowledge_items（source=project:<name>, kind=project），清单讲解 getProjectArchiveContext 检索注入。
// 用法：lib / 面板设置配置 personal_projects=[{name,dir}]；indexPersonalProjects() 重建。
// 注（2026-09 用户反馈）：模拟面试**不再消费**项目源码档案（面试官只看简历）；本模块服务知识库/讲解。
// 全量 TS 升级工单阶段 3：lib/personal-projects.mjs → .ts（rag.ts/skills/mcp-server/interview-agent 等按 .mjs 路径加载 → 保留同名一行桶）
import { readFileSync, readdirSync, statSync, existsSync, writeFileSync, mkdirSync } from "node:fs";
import path from "node:path";
import { db, withTx } from "./db.mjs";

/** 个人项目配置项 */
export interface PersonalProject { name: string; dir: string }
/** 项目档案（标题 + 正文） */
export interface ProjectArchive { title: string; content: string }
/** 源码关键词命中 */
export interface SourceHit { file: string; snippet: string; score: number }
...
export function getPersonalProjects(): PersonalProject[] {
  try {
    ...
    const arr = row ? JSON.parse(String(row.value)) as unknown : [];
    return Array.isArray(arr) ? (arr as PersonalProject[]).filter((p) => p && p.name && p.dir && existsSync(p.dir)) : [];
  } catch { return []; }
}
export function savePersonalProjects(list: unknown): { ok: boolean; projects: PersonalProject[] } {
  const clean = (Array.isArray(list) ? list : [])
    .map((raw) => {
      const p = raw as { name?: unknown; dir?: unknown };
      return { name: String(p.name || "").trim().slice(0, 40), dir: String(p.dir || "").trim() };
    })
...
function readFirst(file: string, max: number): string {
  try { return readFileSync(file, "utf8").slice(0, max); } catch { return ""; }
}

// 扫描源码目录树（过滤忽略；限深度/条目数防爆）
function scanTree(dir: string, baseDir: string, depth: number, acc: string[]): void {
...
function techStack(dir: string): string {
  ...
  try {
    const pkg = JSON.parse(readFirst(pkgFile, 20000)) as { dependencies?: Record<string, unknown>; devDependencies?: Record<string, unknown>; engines?: { node?: unknown }; scripts?: Record<string, unknown>; description?: unknown };
...
async function keySourcePreview(dir: string, files: string[]): Promise<string> {
...
/** 为单个项目生成档案文本（供 knowledge_items + 清单讲解注入）——异步（并行读全部核心源码） */
export async function buildProjectArchive(proj: PersonalProject): Promise<ProjectArchive> {
  const { name, dir } = proj;
  const tree: string[] = [];
...
export async function indexPersonalProjects(): Promise<{ ok: number; fail: number }> {
  ...
  for (const p of projects) {
    try {
      ...
      // 先删旧档案（幂等）再插新——包事务（修复 M11：此前删除+插入不在事务，崩溃丢整份项目档案）
      withTx(() => {
      ...
      fail++;
      console.log(`[personal-projects] ${p.name} 索引失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 80)}`);
    }
  }
  return { ok, fail };
}

/** 按项目名取档案原文（讲解注入用；返回 null 无） */
export function getProjectArchive(name: unknown): string | null {
  try {
    const row = db.prepare("SELECT content FROM knowledge_items WHERE source=? ORDER BY updated_at DESC LIMIT 1").get(`project:${String(name)}`) as { content?: unknown } | undefined;
...
// 归一化：小写 + 去分隔符（匹配升级用）
const NORM_RE = /[\s\-_/\\|（）()【】[\].:：·]/g;
const normKey = (s: unknown): string => String(s || "").toLowerCase().replace(NORM_RE, "");

/** 项目匹配候选名：档案名 + 目录 basename + package.json name（package.json 读取失败忽略） */
function projectMatchKeys(p: PersonalProject): string[] {
  const keys = [p.name, path.basename(String(p.dir || ""))];
  try {
    const pkg = JSON.parse(readFileSync(path.join(p.dir, "package.json"), "utf8")) as { name?: unknown };
...
function findSourceByKeyword(proj: PersonalProject, keyword: unknown): SourceHit | null {
  const { dir } = proj;
  const files: string[] = [];
 ...
 * @param topic 清单条目主题（如"低代码平台"）
 * @param source 条目来源（如"简历"/"产出"）
 * @returns 注入文本（无匹配返回空串；含项目名 + 档案内容）
 */
export async function getProjectArchiveContext(topic: unknown, source = "", force = false, scene = "learn"): Promise<string> {
  try {
    const projects = getPersonalProjects();
    ...
    if (!match) {
      ...
      // 项目关联场景分流工单任务 2：通用语言机制词（事件循环/闭包/原型链/this/作用域——注入价值低、
      // 视角偏移）→ 跳过路径 2（不注入项目片段）；项目技术栈词（状态管理/路由/SSE 等）→ 正常注入
      const GENERIC_MECHANISM_WORDS = ["事件循环", "闭包", "原型链", "this", "作用域", "宏任务", "微任务", "异步", "回调", "垃圾回收", "内存泄漏", "继承", "多态", "作用域链", "提升", "柯里化", "深拷贝", "浅拷贝", "防抖", "节流", "promise", "generator", "迭代器", "proxy", "reflect", "事件冒泡", "事件委托"];
      if (GENERIC_MECHANISM_WORDS.some((w) => String(topic || "").toLowerCase().includes(w))) return "";
      ...
      if (searchWords.length) {
        for (const p of projects) {
          ...
          if (hit) {
            // 项目关联场景分流工单任务 1+3：按场景标注——学习场景"可选参考"（知识主体不变，
            // 讲解末尾独立段）；面试场景"面试重点"（项目里怎么用的就是用户要的）
            const label = scene === "interview"
              ? "【项目关联·面试重点】（面试场景：这个技术点在项目里怎么用的、为什么这么用、有什么坑——面试官可能深挖；只引用【源码结构】里真实存在的文件，不编造文件名）"
              : "【项目关联·可选参考】（学习场景：知识主体不变，这是项目里的真实用法——可跳过；面试前专门看这段；只引用【源码结构】里真实存在的文件，不编造文件名）";
            return `\n${label}\n${hit.snippet}\n`;
          }
        }
      }
    ...
    if (!force) {
      try {
        const { studyNotesDir, sanitizeFilename } = await import("./study-files.ts");
    ...
    try {
        ...
        for (let i = 0; i < groups.length; i += 1) {
          ...
          const rs = await Promise.all(batch.map(async (g) => {
            ...
            const r = await runSubagent({
              ...
              context: content,
            }) as { ok?: boolean; result?: string } | null | undefined;
            return r?.ok ? String(r.result || "") : "";
          }));
          summaries.push(...rs);
        }
      })();
      // 定时器超时 helper（修复：Promise.race 的 setTimeout 不清理 → 定时器保持 open →
      // 进程/测试不退出（open handle）——withTimeout 在 race 完成后 clearTimeout）
      ...
      await withTimeout(collectAll, 600000, "源码搜集超时"); // 串行 10 分钟上限（后台跑——完成不降级）
      ...
      if (fullSummary.length > 500) {
        ...
        try {
          ...
          let baSection = "";
          try {
            const ba = await withTimeout(runSubagent({
              name: "八股提取",
            ...
            }) as Promise<{ ok?: boolean; result?: string } | null | undefined>, 30000, "八股提取超时");
            if (ba?.ok && ba.result) baSection = String(ba.result);
          ...
          let qaSection = "";
          try {
            const qa = await withTimeout(runSubagent({
              name: "模拟面试问答生成",
            ...
            }) as Promise<{ ok?: boolean; result?: string } | null | undefined>, 30000, "问答生成超时");
            if (qa?.ok && qa.result) qaSection = String(qa.result);
