...
export async function loadSkills(skillsDir = SKILLS_DIR) {
  if (cache.has(skillsDir)) return cache.get(skillsDir);
  const tools = [];
  const permMap = {};
  const names = [];
  ...
  const result = { tools, permMap, names, hints, hookCount };
...
/** skill 工具权限级别（默认 auto 只读；confirm 走审批链） */
export function getSkillPermission(fullName) {
  return cache.get(SKILLS_DIR)?.permMap[fullName] || "auto";
}
...
export async function callSkillTool(fullName, args, skillsDir = SKILLS_DIR) {
  ...
  const modPath = path.join(skillsDir, skillName, "skill.mjs");
