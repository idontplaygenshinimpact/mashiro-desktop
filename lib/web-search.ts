...
// 全量 TS 升级工单阶段 3：lib/web-search.mjs → .ts（调用方 4 处，其中 2 处是 mock.module 的 URL 绑定 →
// 必须保留同名 .mjs 桶，否则 mock 静默失效）
...
/** SERP 抓取结果（invalid=true 表示被安全验证/非 200） */
...
/** 可注入的抓取函数（测试 seam） */
export type FetchFn = (url: string) => Promise<SerpPage>;
...
async function fetchBingHtml(url: string): Promise<SerpPage> {
...
function buildSnippets(text: string, titles: string[]): Map<string, string> {
...
function extractResults(page: Partial<SerpPage>, limit: number): SearchResult[] {
...
async function fetchBaiduHtml(url: string): Promise<SerpPage> {
  const r = await fetch(url, {
    headers: { "User-Agent": BING_UA, "Accept-Language": "zh-CN,zh;q=0.9" },
    signal: AbortSignal.timeout(15000),
  });
...
/**
 * 网页搜索（默认 fetch 实现；fetchFn 可注入测试 seam：默认百度优先 + Bing 兜底）
 * 健壮性：网络失败/抓取异常一律返回 []，绝不抛错（工具层会据此降级）
 * @param query 搜索词
 * @param opts limit 返回条数上限（默认 5，最大 10）；fetchFn 注入抓取函数
 */
export async function searchWeb(query: string, { limit = 5, fetchFn }: { limit?: number; fetchFn?: FetchFn } = {}): Promise<SearchResult[]> {
  ...
  try {
    ...
    const page = await bingFn(`${BING_SEARCH_URL}?q=${encodeURIComponent(searchQ)}`);
    if (!page || page.invalid || !Array.isArray(page.links)) return [];
    const results = extractResults(page, cap);
