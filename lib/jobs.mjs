...
export async function collectFromOfficialSites() {
  ...
  for (const site of sites) {
    if (!site?.url || !site?.company) continue;
    try {
      const page = await fetchPage(site.url, { maxTextChars: 8000, collectLinks: true });
      if (page.invalid || !page.text) { results.push({ company: site.company, error: "页面无效" }); continue; }
      ...
      totalNew += added;
      results.push({ company: site.company, found: jobs.length, added });
    } catch (e) {
      results.push({ company: site.company, error: e.message.slice(0, 60) });
    }
  }
...
只输出 JSON：{"jobs":[{"company":"","title":"","job_type":"校招","direction":"frontend","apply_url":"","deadline":"","summary":""}]}`;
  ...
  const data = await llmChat(
    [{ role: "system", content: "你是校招信息解析助手，只输出合法 JSON。" }, { role: "user", content: prompt }],
    { maxTokens: 1500, temperature: 0.1 }
  );
  const parsed = extractJson(getReplyText(data));
  return (parsed?.jobs || []).filter((j) => j?.company && j?.title).slice(0, 8);
}
...
export async function collectCompanyList() {
  ...
  const r = await toolSearchPosts("2026届秋招 公司名单 招聘", "nowcoder");
...
${String(text).slice(0, 4000)}`;
  ...
  const data = await llmChat(
    [{ role: "system", content: "你是秋招信息助手，只输出合法 JSON。" }, { role: "user", content: prompt }],
...
export async function collectJobsForCompaniesWithoutSite() {
  ...
  for (const c of companies) {
    try {
      ...
      for (const p of posts) {
        const { fetchPage } = await import("./fetch-page.mjs");
        const page = await fetchPage(p.url, { maxTextChars: 3000 });
        if (page.invalid || !page.text) continue;
...
/** 推荐岗位：简历驱动匹配（有简历画像时按技能命中排序；否则退回方向权重）+ 未投递优先 + 截止临近 */
...
${String(resume).slice(0, 4000)}`;
  ...
  db.prepare("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?,?,?)")
    .run("resume_skills", JSON.stringify({ skills, directions, updatedAt: Date.now() }), Date.now());
  return { ok: true, skills, directions };
}
...
招聘标题：${title}
招聘内容：${String(text).slice(0, 3000)}`;

  const data = await llmChat(
    [{ role: "system", content: "你是秋招信息解析助手，只输出合法 JSON。" }, { role: "user", content: prompt }],
    { maxTokens: 800, temperature: 0.1 }
  );
  const parsed = extractJson(getReplyText(data));
