...
/** 岗位笔试 → 统一日程（schedule_events，email_id=`job_<id>` 幂等去重）：
 * 岗位域笔试信息进入「面试/笔试日程」页与日程提醒（此前只有邮件邀约能进日程表）
 * 日期解析注意：new Date("YYYY-MM-DD") 按 UTC 解析 → 东八区会漂移 +8h（纯日期笔试
 * 变成当天 08:00，影响日程窗口判断）。纯日期按本地时区解析为当天 00:00。 */
export function syncJobBishiToSchedule(id, company, title, bishiDate) {
  ...
  try {
    ...
    // UPSERT：笔试时间变更时更新日程（修复：原 INSERT OR IGNORE 只增不更新 → 变更后提醒错时）
    ...
    db.prepare(
      `INSERT INTO schedule_events (company, role, interview_at, form, location, link, email_id, created_at)
       VALUES (?, ?, ?, '笔试', '', '', ?, ?)
       ON CONFLICT(email_id) DO UPDATE SET company=excluded.company, role=excluded.role,
         interview_at=excluded.interview_at`
    ).run(String(company).slice(0, 60), String(title || "").slice(0, 60), t, `job_${String(id)}`, Date.now());
  } catch { /* 日程表暂不可用忽略 */ }
}
...
/** 本地时区日期解析（修复：new Date("YYYY-MM-DD") 按 UTC 解析 → 东八区 +8h 漂移，
 * 影响"一周内截止/3 天内截止"窗口判断）：纯日期按本地 00:00，带时间按本地精确时间 */
...
export function setJobStatus(id, status) {
  ...
  let r;
  if (status === "ready") {
    // 转"已投递"时记录首次投递时间（重复标记不刷新，保留首次投递时间）
...
export function getUpcomingJobDeadlines(jobs, now = Date.now()) {
  const horizon = 3 * 24 * 3600 * 1000; // 3 天窗口
  const upcoming = [];
  for (const j of jobs || []) {
