...
// 模拟面试 agent 化工单任务 3：循环内 tool_calls（对标 subagent v2 loop）——查完资料立即调整，不只出题前
...
async function askInterviewer(prompt, messages = []) {
  ...
  try {
    for (let i = 0; i < 4; i++) { // 每轮 tool_calls 上限（防塞爆）
      ...
      return { raw: getReplyText(res), msgs };
    }
    return { raw: "", msgs };
  } catch (e) {
...
function advanceRound(session, r, scores, total, roundNum, roundIndex, currentRound, hitItem) {
  ...
  session.current = {
    // 模拟面试 agent 化工单任务 5：next_question 空 → 基于当前问题生成追问（不再"请继续"）
    question: r.next_question || `请继续深入讲讲「${current.question.slice(0, 40)}」的${current.dimension || "技术"}方面——结合你的实际经验展开。`,
