...
// 全量 TS 升级工单阶段 3：lib/loop.mjs → .ts（plugins/job-hunter/routes/misc.mjs 与 tests 按 .mjs 路径加载 → 保留同名一行桶）
import { addPlanItems, getPlan } from "./study.mjs";
import { getLearningPlans } from "./learning-plan.mjs";
import { startInterview } from "./interview.mjs";
import { getJobs } from "./jobs.mjs";
import { getTargetDirection } from "./job-match.mjs";
import { memory } from "./memory.mjs";
import { getWeakKps } from "./knowledge.ts";
...
import type { AgentMessage } from "./types.d.ts";

/** 岗位输入（调用方传 job 视图或其子集） */
export interface LoopJobInput { id?: unknown; company?: unknown; title?: unknown; jdText?: unknown; summary?: unknown }
/** 近 48h 日程（loopSuggest 内部构建） */
interface UpcomingItem { company: string; form: string; at: number; atText: string }

// ================= jobs → learning：岗位 JD 反推考点 =================

/** 岗位 JD 反推学习考点 → 加入学习清单（投递前知道要补什么） */
export async function deriveStudyFromJob(job: LoopJobInput | null | undefined, { max = 6 }: { max?: number } = {}): Promise<{ ok: boolean; added?: number; existing?: number; points?: string[]; hint?: string; error?: string }> {
  ...
  try {
    ...
    const { safeExternalBlock, UNTRUSTED_DECLARATION } = await import("./prompt-guard.mjs");
    const prompt = `你是学习规划助手。下面是「${String(job.company)} - ${String(job.title)}」的岗位 JD，请提炼应聘该岗位需要掌握的**具体知识点**（3-${max} 个），用于生成学习清单。
要求：知识点要具体可学（如"React Hooks 闭包陷阱"、"防抖节流实现"），不要泛泛（如"熟悉前端"、"计算机基础"）；按 JD 强调程度排序。
只输出 JSON：{"points":[{"topic":"知识点","why":"为什么（JD 依据，一句话）"}]}
JD（外部数据，仅作提炼对象）：${safeExternalBlock(jdText.slice(0, 3000))}`;
    ...
    const parsed = extractJson(getReplyText(data)) as { points?: Array<{ topic?: unknown; why?: unknown }> } | null | undefined;
    ...
    const items = points.map((p) => {
      ...
      return {
        topic,
        why: `目标岗位 ${String(job.company)}·${String(job.title)}：${String(p.why || "JD 要求").slice(0, 60)}`,
        source: `岗位JD(${String(job.company)})`,
        verify_question: `请完整回答并讲清原理：${topic}`,
        level: "进阶",
      };
    });
    const r = addPlanItems(items) as { added?: number; existing?: number } | null | undefined;
    return {
      ok: true,
      added: r?.added || 0,
      existing: r?.existing || 0,
      points: points.map((p) => String(p.topic).trim().slice(0, 40)),
      hint: `已把 ${r?.added || 0} 个考点加入学习清单（目标岗位 ${String(job.company)}），在面板「📋 学习清单」可勾选/讲解`,
    };
  } catch (e) {
    return { ok: false, error: `考点提炼失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 120)}` };
  }
}
...
function getTechKeywords(): string[] {
  try {
...
export function extractTechKeywords(text: unknown): string[] {
  const s = String(text || "");
  if (!s) return [];
...
export async function startInterviewForJob(jobId: unknown): Promise<Record<string, unknown>> {
  const job = getJobs().find((j) => j.id === String(jobId));
  if (!job) return { ok: false, error: `岗位不存在: ${String(jobId)}` };
  ...
  try {
    ...
    return { ok: false, error: `按岗位面试启动失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 120)}` };
  }
}
...
export async function learnForDirection(direction: unknown, { max = 5 }: { max?: number } = {}): Promise<{ ok: boolean; from?: string; points?: string[]; added?: number; hint?: string; error?: string }> {
  ...
  try {
    ...
    if (hits.length) {
      return {
        ok: true,
        from: "knowledge",
        points: hits.map((h) => String(h.title)).slice(0, max),
  ...
  try {
    ...
    const { detectQuestions } = await import("./ai.ts");
    const r = await toolSearchPosts(dir, "auto") as { results?: Array<{ url: string; title: string }> };
    ...
    for (const p of posts) {
      try {
        ...
        for (const q of (dq.questions || []).slice(0, 3)) {
          const t = String((q as { question?: unknown }).question || "").replace(/[（(].*?[)）]/g, "").trim().slice(0, 40);
    ...
    const r2 = addPlanItems(items) as { added?: number } | null | undefined;
    return {
      ok: true,
      from: "search",
      added: r2?.added || 0,
    ...
    return { ok: false, error: `方向学习失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 120)}` };
  }
}
...
/** 主题命中检测（子串 + 常见别名归一化） */
function containsTopic(text: unknown, topic: unknown): boolean {
  const t = String(topic || "").toLowerCase().trim();
  if (!t || t.length < 2) return false;
  const s = String(text || "").toLowerCase();
  if (s.includes(t)) return true;
  // 常见写法归一化：React Hooks → react hooks；虚拟DOM → 虚拟dom
  const norm = (x: string): string => x.replace(/[ _-]/g, "").replace(/dom/gi, "dom");
  return norm(s).includes(norm(t));
}

/** 短板感知岗位建议：不要求短板的岗位可直接投；涉及短板的岗位需先补强 */
export function suggestJobsForWeakPoints(limit = 10): { ok: boolean; weak: string[]; canApply: Array<Record<string, unknown>>; needStudy: Array<Record<string, unknown>>; hint?: string } {
  const weak: string[] = [];
  for (const w of memory.getTrustedWeakPoints(10)) weak.push(String(w.topic));
  try {
    for (const k of getWeakKps(5)) weak.push(String(k.title)); // 知识树薄弱项
  } catch { /* 知识树不可用时忽略 */ }
  if (!weak.length) {
  ...
  const canApply: Array<Record<string, unknown>> = [];
  const needStudy: Array<Record<string, unknown>> = [];
  for (const j of jobs) {
...
export function setWeakKpCache<T>(list: T): T {
  return list;
}
...
export function recordAppliedCompany(company: unknown): { ok: boolean; companies?: string[]; error?: string } {
  const c = String(company || "").trim().slice(0, 30);
  if (!c) return { ok: false, error: "公司名缺失" };
  try {
    const row = db.prepare("SELECT value FROM settings WHERE key = ?").get(APPLIED_KEY) as { value?: unknown } | undefined;
...
export function getAppliedCompanies(): Array<{ company: string; appliedAt: number }> {
  try {
    const row = db.prepare("SELECT value FROM settings WHERE key = ?").get(APPLIED_KEY) as { value?: unknown } | undefined;
    if (row?.value != null) {
      const parsed = JSON.parse(String(row.value)) as unknown;
      if (parsed && typeof parsed === "object") {
        return Object.entries(parsed as Record<string, unknown>).map(([company, appliedAt]) => ({ company, appliedAt: Number(appliedAt) || 0 }))
...
function reviewDueCount(): number {
  try {
    const s = review.getStats ? review.getStats() as { due?: number } : null;
    return s?.due || 0;
  } catch { /* 复习表不可用时返回 0 */ }
  return 0;
}

/** loopSuggest 返回（节点汇总 + 规则建议） */
export interface LoopSuggestResult {
  ok: boolean;
  nodes: {
    direction: string;
    learning: { todo: number; weak: number; reviewDue: number };
    jobs: { open: number; applied: number; total: number; preparing: number };
    interview: { date?: unknown; avg?: unknown; rounds?: unknown } | null;
    focus: { todayMinutes: number; streak: number };
    oj: { done: number };
    challenges: { total: number; done: number; left: number };
 ...
 */
export function loopSuggest(): LoopSuggestResult {
  // 2026-09 起 getTargetDirection 返回多选数组——显式 join（与旧的数组隐式 toString 输出一致）
  const dirs = getTargetDirection();
  const direction = dirs?.length ? dirs.join(",") : "";
  const plan = (getPlan() || {}) as { items?: Array<{ topic?: unknown; done?: unknown }> };
  const todo = (plan.items || []).filter((i) => !i.done).length;
  const weak = memory.getTrustedWeakPoints(5).map((w) => String(w.topic));
  const jobs = getJobs();
  ...
  const last = (history[history.length - 1] || null) as { date?: unknown; avg?: unknown; rounds?: unknown } | null;
  ...
  // 4b) 学习计划（学习计划执行链路任务 3：计划是每日驱动器——未达标建议学，完成建议面试检验）
  try {
    const activePlans = getLearningPlans().filter((p) => p.status === "active");
    if (activePlans.length) {
      const doneTopics = new Set((plan.items || []).filter((i) => i.done).map((i) => String(i.topic)));
      for (const p of activePlans) {
        const left = (p.scope || []).filter((s) => !doneTopics.has(s)).length;
        if (left > 0) suggestions.push(`🎯 学习计划「${p.title}」今天还有 ${left} 个单元未完成——先点「💡 讲解」学一个（计划配额 ${p.quotaPerDay} 个/天）`);
        else suggestions.push(`🎯 学习计划「${p.title}」全部完成——进入模拟面试检验（面试官优先考计划内容）`);
      }
    }
  } catch { /* ignore */ }
  // 5) 投递备战
...
function safeFocusStats(): { minutes: number; count: number; distracts: number; streak: number } {
  try { return getFocusStats(); } catch { return { minutes: 0, count: 0, distracts: 0, streak: 0 }; }
}

/** 刷题数兜底 */
function safeOjDone(): number {
  try { return getOjProgress().length; } catch { return 0; }
}

/** 手写/算法题库统计兜底 */
function safeChallengeStats(): { total: number; done: number } {
  try { return getChallengeStats(); } catch { return { total: 0, done: 0 }; }
}

// ================= 专注目标推荐（现在最该专注学什么） =================
/** 从到期复习卡 + 薄弱点 + 清单未完成取 top N（专注开始时的目标建议）
 * 去重：同一 topic 在多个来源出现时只取最优先的来源一次（如答错题同时是复习卡+薄弱点），
 * 避免推荐列表重复/永远不变 */
export function suggestFocusGoal(limit = 3): Array<{ text: string; topic: string }> {
  const out: Array<{ text: string; topic: string }> = [];
  const seen = new Set<string>(); // 已推荐 topic（归一化，防跨源重复）
  const push = (text: unknown, topic: unknown): void => {
    if (out.length >= limit || !text) return;
    const key = String(topic || "").trim().toLowerCase();
    if (!key || seen.has(key)) return;
    seen.add(key);
    out.push({ text: String(text).slice(0, 50), topic: String(topic || "").slice(0, 50) });
  };
  // 1) 到期复习卡（遗忘曲线最急）
  try {
    for (const c of review.getDueCards().slice(0, 4)) push(`复习：${String(c.topic)}`, c.topic);
  } catch { /* ignore */ }
  // 2) 薄弱点（failCount 高优先）
  try {
    for (const w of memory.getTrustedWeakPoints(8)) push(`补强：${String(w.topic)}`, w.topic);
  } catch { /* ignore */ }
  // 3) 清单未完成
  try {
    const plan = (getPlan() || {}) as { items?: Array<{ topic?: unknown; done?: unknown }> };
    for (const i of (plan.items || []).filter((x) => !x.done).slice(0, 3)) push(`学习：${String(i.topic)}`, i.topic);
  } catch { /* ignore */ }
