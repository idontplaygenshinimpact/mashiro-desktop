...
// 全量 TS 升级工单阶段 3：lib/mail.mjs → .ts（routes/core、loop、routes/mail、server.ts、tests 按 .mjs 路径加载 → 保留同名一行桶）
import { extractJson } from "./llm.mjs";
import { ImapFlow } from "imapflow";
import { chat } from "./ai.ts";
import { withTimeout } from "./text-utils.ts"; // 技术债 L3：收敛单点
...
/** 邮箱配置（settings.mail_config） */
export interface MailConfig { email: string; authCode: string; enabled: boolean }
/** IMAP 连接凭据（clientFactory/testConnection/fetchUnreadEmails/markSeenEmails 只需要这两项） */
export interface MailAuthConfig { email: string; authCode: string }
/** 拉取到的邮件（正文已截断） */
export interface MailListItem { id: string; from: string; subject: string; date: string; text: string }
/** LLM 识别出的邀约条目 */
...
/** 可注入的 IMAP 客户端工厂（测试用假 client） */
export type ClientFactory = (config: MailAuthConfig) => ImapFlow;
/** 可注入的 LLM（测试用假实现） */
export type MailLlm = (messages: Array<Record<string, unknown>>, opts: Record<string, unknown>) => Promise<string>;
/** runMailCheck 结果 */
export interface RunMailCheckResult {
  ok: boolean;
  error?: string;
  emails?: number;
  added?: number;
...
export function getLastMailError(): string | null { return lastMailError; }
function setMailError(err: unknown): string {
  lastMailError = String(err || "").slice(0, 200);
...
export function getConfig(): MailConfig {
  try {
    const row = db.prepare("SELECT value FROM settings WHERE key = ?").get(CONFIG_KEY) as { value?: unknown } | undefined;
    if (row?.value) {
      const cfg = JSON.parse(String(row.value)) as { email?: unknown; authCode?: unknown; enabled?: unknown };
      return {
...
export function setConfig({ email, authCode, enabled }: { email?: string; authCode?: string; enabled?: boolean } = {}): { ok: boolean; email?: string; enabled?: boolean; error?: string } {
...
// withTimeout 从 text-utils.ts 导入（技术债 L3：收敛单点——统一签名含 onTimeout）

/** forceCloseClient 需要触达的 imapflow 内部字段（未公开 API，按需收口） */
interface ClientInternals {
  close?: () => void;
  transport?: { destroy?: () => void };
  socket?: { destroy?: () => void };
}

// 强制关闭 IMAP 底层连接：client.close() 优先，回退 transport/socket 销毁；全部 try/catch 防二次抛错
function forceCloseClient(client: ImapFlow | undefined | null): void {
  if (!client) return;
  const c = client as unknown as ClientInternals;
  try { if (typeof c.close === "function") c.close(); } catch { /* ignore */ }
  try { if (c.transport && typeof c.transport.destroy === "function") c.transport.destroy(); } catch { /* ignore */ }
  try { if (c.socket && typeof c.socket.destroy === "function") c.socket.destroy(); } catch { /* ignore */ }
}

const LOGOUT_TIMEOUT_MS = 5000; // logout 兜底超时（防登出挂起卡死）

// 安全登出：包 withTimeout（5s），超时同样强制关闭连接；任何失败忽略
async function safeLogout(client: ImapFlow | undefined | null): Promise<void> {
...
function defaultClientFactory(config: MailAuthConfig): ImapFlow {
  return new ImapFlow({
    host: IMAP_HOST,
    port: IMAP_PORT,
    secure: true,
...
// ---------- 连接测试 ----------
export async function testConnection(config: MailAuthConfig, { clientFactory = defaultClientFactory }: { clientFactory?: ClientFactory } = {}): Promise<{ ok: boolean; error?: string }> {
...
export async function fetchUnreadEmails(config: MailAuthConfig, { sinceDays = 3, max = 30, clientFactory = defaultClientFactory }: { sinceDays?: number; max?: number; clientFactory?: ClientFactory } = {}): Promise<MailListItem[]> {
  if (!config?.email || !config?.authCode) return [];
  let client: ImapFlow | undefined;
  try {
    client = clientFactory(config);
    await withTimeout(client.connect(), CONNECT_TIMEOUT_MS, () => forceCloseClient(client));
    ...
    try {
      ...
      let uids: number[] = [];
      try {
        const r = await client.search({ unseen: true } as never, { uid: true });
        uids = Array.isArray(r) ? r as number[] : [];
      } catch { uids = []; }
      // 2) 未读为空 → 回退最近 sinceDays 天
      if (!uids.length) {
        try {
          const since = new Date(Date.now() - sinceDays * 86400e3);
          const r = await client.search({ since } as never, { uid: true });
          uids = Array.isArray(r) ? r as number[] : [];
        } catch { uids = []; }
      }
      ...
      type FetchedMsg = { uid?: unknown; envelope?: { from?: Array<{ address?: unknown }>; subject?: unknown; date?: Date }; source?: unknown };
      for await (const msg of client.fetch(uids, { uid: true, envelope: true, source: true }) as AsyncIterable<FetchedMsg>) {
...
export async function markSeenEmails(config: MailAuthConfig, uids: Array<string | number> = [], { clientFactory = defaultClientFactory }: { clientFactory?: ClientFactory } = {}): Promise<number> {
  ...
  try {
    ...
    try {
      lock = await client.getMailboxLock("INBOX");
      await client.messageFlagsAdd(uids as never, ["\\Seen"], { uid: true });
      return uids.length;
    } finally {
      try { lock?.release?.(); } catch { /* ignore */ }
    }
...
function decodeAndClean(partBody: string, contentType: string, encoding: string): string {
...
function stripHtml(html: unknown): string {
  return String(html || "")
    ...
    .replace(/&quot;/gi, '"')
    .replace(/&#39;|&apos;/gi, "'")
    .replace(/&#(\d+);/g, (m, d: string) => { try { return String.fromCodePoint(Number(d)); } catch { return m; } })
    .replace(/\s+/g, " ")
    .trim();
}
...
function normalizeEvents(arr: unknown[]): InterviewEvent[] {
...
export async function extractInterviewEvents(emails: MailListItem[] = [], { llm = null }: { llm?: MailLlm | null } = {}): Promise<InterviewEvent[]> {
  const callLlm: MailLlm = llm || ((messages, opts) => chat(messages as unknown as AgentMessage[], opts as LLMOptions));
...
${safeExternalBlock(emailText)}`;
  ...
  try {
    ...
    const arr = Array.isArray(parsed) ? parsed : (Array.isArray((parsed as { events?: unknown[] })?.events) ? (parsed as { events: unknown[] }).events : []);
    return normalizeEvents(arr);
  } catch (e) {
    // LLM 错误 → 空数组（契约不变）+ 记录错误（runMailCheck 区分"识别失败"与"无邀约"）
    setMailError(String((e as Error)?.message || e || "LLM 调用失败").slice(0, 120));
    return [];
  }
}
...
// 保持"同邮件同公司同时间不重复"语义
export function saveEvents(events: InterviewEvent[] = []): { added: number; skipped: number } {
  ...
  for (const raw of events || []) {
    ...
    if (Number(r.changes) > 0) added++; else skipped++;
  }
...
function rowToEvent(r: Record<string, unknown>): ScheduleEvent {
  return {
    ...
    company: r.company,
    role: String(r.role || ""),
    interviewAt: r.interview_at == null ? null : Number(r.interview_at),
    form: String(r.form || ""),
    location: String(r.location || ""),
    link: String(r.link || ""),
    emailId: String(r.email_id || ""),
    lastNotifiedAt: r.last_notified_at == null ? null : Number(r.last_notified_at),
    createdAt: r.created_at,
  };
}

/** 未来 withinDays 天内的日程（提醒用） */
export function getUpcomingEvents({ withinDays = 3 }: { withinDays?: number } = {}): ScheduleEvent[] {
  const now = Date.now();
  ...
  try {
    ...
    ).all(now, horizon) as Array<Record<string, unknown>>;
...
export function getSchedule(): ScheduleEvent[] {
  ...
  try {
    ...
    ).all(now, SCHEDULE_CAP) as Array<Record<string, unknown>>;
    return rows.map(rowToEvent);
  } catch { return []; }
}

/** 标记已提醒（更新 last_notified_at，防同一事件反复轰炸） */
export function markNotified(id: unknown): void {
  try {
