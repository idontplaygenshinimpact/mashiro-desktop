...
import { sanitizeExternal , safeExternalBlock} from "./prompt-guard.mjs";
...
import { normalizeGroup, normalizeGroupName, EXTRA_GROUP_RULES } from "./study-groups.mjs";
import { normalizeTopic, isSimilarTopic } from "./study-topic.mjs";

// 复习卡强化激活工单任务 1②：新条目自动补卡（多角度题 + 优先级——幂等）
// 触发是 fire-and-forget（不阻断 addPlanItems 同步返回）——测试环境默认关闭：
// 异步补卡会消费 mock LLM 队列/污染后续测试断言（测试间干扰）。接线测试
// （tests/review.test.mjs 激活②）用 setAutoPlanCoverage(true) 显式开启验证。
...
${excerpts.map((x) => safeExternalBlock(x)).join("\n\n---\n\n").slice(0, 20000)}
