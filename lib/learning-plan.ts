...
// 职责边界（L8 命名澄清，不重命名）：
//   learning-plan.ts —— 长期学习计划引擎（本文件：计划实体 + 学习事件流 + 趋势聚合，领域无关）
//   study.mjs        —— 学习清单的"数据 + 领域逻辑"（桶 re-export：topic/groups/store 子域）
//   study-plan.ts    —— 学习清单的"生成/勾选/同步/回填"编排（调 study 子域 + LLM 生成计划）
...
// 全量 TS 升级工单阶段 3：lib/learning-plan.mjs → .ts（loop/review/today-brief/tests 按 .mjs 路径加载 → 保留同名一行桶）
import { localDateKey } from "./date-utils.ts";
import { db } from "./db.mjs";
import { EXTRA_GROUP_RULES } from "./study-groups.ts"; // 算法关键词词表（与 detectAlgoTopic 同源——防两套词表漂移）

const PLANS_KEY = "learning_plans"; // settings JSON 数组 [{id,title,scope,quotaPerDay,durationDays,milestones,status,createdAt}]

// 算法关键词词表（与 review.mjs detectAlgoTopic 同源：从"算法与手写"组规则提取——防两套词表漂移）
const ALGO_KWS = (EXTRA_GROUP_RULES.find((r) => r.g === "算法与手写") || { kws: [] as unknown[] }).kws;
// scope 关键词 = 工单列出的算法主题词（排序/链表/二叉树/DP/回溯/LRU/栈/队列/哈希/双指针/
// 滑动窗口/岛屿/子串/子序列/回文/全排列/mex/差分/前缀）+ ALGO_KWS 词表合并（覆盖更全）
const ALGO_SCOPE_WORDS = [...new Set([
  "排序", "链表", "二叉树", "dp", "回溯", "lru", "栈", "队列", "哈希", "双指针",
  "滑动窗口", "岛屿", "子串", "子序列", "回文", "全排列", "mex", "差分", "前缀",
  ...ALGO_KWS.map((k) => String(k).toLowerCase()),
])];

// 半截词/缩写（相似度引擎统一工单任务 5①）：scope 是归属钥匙不是知识点——
// "未出现/手撕/差分/mex/前缀/岛屿" 等半截词入清单会产生无法独立学习的条目
// （"请完整讲解：mex"？）——createLearningPlan 入清单前过滤；完整主题（链表/排序/二叉树）保留
const HALF_SCOPE_WORDS = new Set([
  "未出现", "手撕", "差分", "mex", "前缀", "岛屿", "子串", "回文", "子序列", "滑动窗口",
  "lru", "lfu", "dp", "手写", "防抖", "节流", "深拷贝", "浅拷贝", "拼接", "机器人",
  "四则", "二进制", "字典序", "版本号", "大顶堆", "递归", "复杂度", "双指针", "哈希", "堆",
]);

/** 计划实体（settings.learning_plans 元素） */
...
// 算法专项提升计划工单任务 2①：scope 自动提取——从复习卡 algo 卡 + 学习清单 + 手写题库
// 提取实际出现的算法关键词（命中 ALGO_SCOPE_WORDS）→ 去重
// scope 是 matchPlanForTopic 的归属钥匙（子串匹配）——只收数据里真实出现的词，避免空转
export function extractAlgoScope(): string[] {
  const kws = new Set<string>();
  const hit = (t: unknown): void => {
    const s = String(t || "").toLowerCase();
    for (const k of ALGO_SCOPE_WORDS) if (s.includes(k)) kws.add(k);
  };
  try { for (const r of db.prepare("SELECT topic FROM review_cards WHERE type='algo'").all() as Array<Record<string, unknown>>) hit(r.topic); } catch { /* ignore */ }
  try { for (const r of db.prepare("SELECT topic FROM study_plan_items").all() as Array<Record<string, unknown>>) hit(r.topic); } catch { /* ignore */ }
  try { for (const r of db.prepare("SELECT title FROM challenges").all() as Array<Record<string, unknown>>) hit(r.title); } catch { /* ignore */ }
  return [...kws];
}

// ---------- 计划 CRUD ----------
function loadPlans(): LearningPlan[] {
  try {
    const row = db.prepare("SELECT value FROM settings WHERE key=?").get(PLANS_KEY) as { value?: unknown } | undefined;
    if (!row?.value) return [];
    const arr = JSON.parse(String(row.value)) as unknown;
    return Array.isArray(arr) ? arr as LearningPlan[] : [];
  } catch { return []; }
}
function savePlans(plans: LearningPlan[]): void {
 ...
 */
export async function createLearningPlan({ title, scope = [], quotaPerDay = 2, durationDays = 30, milestones = [] }: { title?: unknown; scope?: unknown; quotaPerDay?: unknown; durationDays?: unknown; milestones?: unknown } = {}): Promise<{ ok: boolean; plan?: LearningPlan; error?: string }> {
  ...
  // createdAt 严格单调（2026-09-11 修 CI flake）：同一毫秒内连续建两个计划时 Date.now() 相等，
  // 于是"最近激活计划"（按 createdAt 排序）退化成随机——CI 实测 not ok 408（期望"新计划"实得"旧计划"）。
  // id 带随机后缀（newPlanId）不能表达时间序，所以只能在这里保证：新计划一定比历史最大值大 1ms。
  const prevMax = plans.reduce((m, p) => Math.max(m, Number(p.createdAt) || 0), 0);
  const plan: LearningPlan = {
    ...
    status: "active",
    createdAt: Math.max(Date.now(), prevMax + 1),
  };
  plans.push(plan);
  savePlans(plans);
  // 学习计划执行链路任务 1：scope 单元 → 学习清单（计划单元可一键讲解——study-detail-stream 按清单 id）
  // 计划从"纯记账本"变"每日驱动器"：清单条目天然有讲解入口 + 复习卡 + 复盘
  // 相似度引擎统一工单任务 5①：半截词/缩写过滤（scope 是归属钥匙不是知识点——"mex/差分/前缀"不入清单）
  try {
    const { addPlanItems } = await import("./study.mjs");
    addPlanItems(cleanScope
      .filter((s) => !HALF_SCOPE_WORDS.has(s))
      .map((s) => ({
        topic: s,
        why: `学习计划「${t}」每日任务（${String(quotaPerDay)} 个/天）`,
        source: `学习计划·${t}`,
        verify_question: `请完整讲解：${s}`,
        level: "必会",
      })));
  } catch { /* 清单写入失败不阻断计划创建 */ }
  return { ok: true, plan };
...
export function matchPlanForTopic(topic: unknown): string | null {
...
export function recordLearningEvent({ topic, kind, result = null, quality = null, durationMs = null, planId = null, ts = Date.now() }: LearningEventInput): { ok: boolean; error?: string; planId?: string | null } {
 ...
 *   c. 当前计划当日配额达成度
 * @returns 提示文案（无基线/无对比意义时 null）
 */
export function buildFeedbackTip({ topic, kind, result = null, quality = null, durationMs = null, planId = null, now = Date.now() }: { topic: unknown; kind: unknown; result?: string | null; quality?: number | null; durationMs?: number | null; planId?: string | null; now?: number }): string | null {
  const t = String(topic || "").trim();
  if (!t) return null;
  ...
  ).all(k, t, now) as Array<{ result?: unknown; quality?: unknown; duration_ms?: unknown; ts?: unknown }>;
  // b) 同 kind 全体基线
  const kindAvg = db.prepare(
    "SELECT AVG(duration_ms) avg_ms FROM learning_events WHERE kind=? AND duration_ms IS NOT NULL"
  ).get(k) as { avg_ms?: unknown } | undefined;
  ...
  try {
    ...
    const todayRow = db.prepare("SELECT COUNT(*) n FROM learning_events WHERE plan_id=? AND ts >= ?").get(pid, dayStart.getTime()) as { n?: unknown } | undefined;
    const plans = loadPlans();
    const plan = plans.find((p) => p.id === pid);
    if (plan) {
      const quota = Number(plan.quotaPerDay) || 0;
      const n = Number(todayRow?.n) || 0;
...
// ---------- 趋势聚合（惰性计算） ----------
/** 计划状态：进度 + 趋势（按天聚合事件） */
export function getLearningPlanStatus(planId?: string): {
  ok: boolean;
  error?: string;
  plan?: LearningPlan & { elapsedDays: number; remainDays: number };
  status?: {
    days: number; activeDays: number; doneTotal: number; passRate: number; avgMs: number;
    todayDone: number; todayQuota: number;
    trend: Array<{ date: string; done: number; passRate: number; avgMs: number }>;
    weakTopics: string[];
  };
} {
  const plans = loadPlans();
  // 缺省：最近激活的计划（按 createdAt 新 → 旧）
  // 2026-09-11 修 CI flake：同一毫秒创建的两个计划 createdAt 相同 → 原排序不稳定，
  // "最近激活"会随机取到旧计划（CI 实测 not ok 408：期望"新计划"实得"旧计划"）。
  // id 形如 plan-<base36 时间戳>，同长度下字典序与时间序一致 → 作为确定性兜底。
  ...
  ).all(plan.id) as Array<{ topic?: unknown; result?: unknown; quality?: unknown; duration_ms?: unknown; ts?: unknown }>;

  // 按天聚合（本地日期）
  const dayMs = 24 * 3600 * 1000;
  const byDay = new Map<string, { done: number; pass: number; durationMs: number; results: string[] }>();
  for (const r of rows) {
