...
// 全量 TS 升级工单阶段 3：本文件为唯一实现，lib/platforms/boss.mjs 降为同名薄桶（引用方零改动）
import { createRequire } from "node:module";
import { getAccount } from "../platform-accounts.ts";
import { readBrowserCookies } from "../chrome-cookies.ts";

// jsdom 无类型声明（装 @types/jsdom 会把 DOM 全局拉进来破坏 tests/scripts 的 fetch mock 类型）——
// 走 createRequire（CJS 包，同步加载，保持解析层纯同步语义）+ 本地形状收口（与 lib/fetch-page.ts 同口径）
...
/** 注入 Playwright 的 cookie（domain/path 固定 zhipin.com 根） */
...
export function parseJobCards(html: string): BossJobCard[] {
...
export function parseJobDetail(html: string): BossJobDetail {
  const dom = new JSDOM(html);
  const doc = dom.window.document;
...
export function detectBlock(html: string, title = ""): string | null {
...
export function parseCookieHeader(cookieStr: unknown): BossCookie[] {
  const raw = String(cookieStr || "").trim();
  if (!raw) return [];
  return raw.split(";")
    .map((pair) => pair.trim())
    .filter(Boolean)
    .map((pair): BossCookie | null => {
    ...
    .filter((c): c is BossCookie => !!c);
}

/** 获取 BOSS 登录态 cookie：配置的 Cookie 头优先，其次浏览器会话；无 → null */
async function resolveCookies(): Promise<BossSession | null> {
...
/** 打开页面（SSRF 防护 + 登录态注入）；调用方 finally close */
async function openWithSession(url: string) {
  ...
  try {
    ...
    console.log(`[boss] cookie 注入失败: ${String(e instanceof Error ? e.message : e).slice(0, 80)}`);
  }
  return { ...opened, session };
}

/** 搜索岗位 */
export async function searchJobs(keyword: string, { limit = 15, city = "100010000" }: { limit?: number; city?: string } = {}) {
  ...
  try {
    ...
    return { ok: false, error: `搜索失败: ${String(e instanceof Error ? e.message : e).slice(0, 150)}` };
  } finally {
...
export async function fetchDetail(url: string) {
  ...
  try {
    ...
    return { ok: false, error: `详情抓取失败: ${String(e instanceof Error ? e.message : e).slice(0, 150)}` };
  } finally {
    await opened.context.close().catch(() => {});
  }
}
...
export async function prepareApply(url: string, { greeting = "" }: { greeting?: string } = {}) {
  const opened = await openWithSession(String(url));
  ...
  try {
    ...
    return { ok: false, error: `投递执行失败: ${String(e instanceof Error ? e.message : e).slice(0, 150)}` };
  } finally {
    await opened.context.close().catch(() => {});
  }
}
