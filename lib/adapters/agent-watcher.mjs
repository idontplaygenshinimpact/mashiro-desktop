    ...
    tick() {
      ...
      for (const file of files) {
        ...
        for (const line of text.split("\n")) {
          ...
          if (!r) continue;
          if (r.type === "meta") { if (!s.label && r.payload.cwd) s.label = path.basename(String(r.payload.cwd)); continue; }
          if (r.type === "started") {
          ...
          if (r.type === "tool_use") { s.toolCount++; send("agent:tool_use", { tool: r.payload.tool }); events++; continue; }
          if (r.type === "assistant_reply") { send("agent:assistant_reply", { replyLen: r.payload.replyLen }); events++; continue; }
...
export function createDshSource({ dir = defaultDshDir(), emit = () => {}, idleTimeoutMs = 120000, now = () => Date.now(), maxBytes = 524288, maxFrames = 128 } = {}) {
  ...
  function resolveLabel(file) {
    ...
    if (h?.cwd) return path.basename(h.cwd);
    ...
    tick() {
      ...
      for (const file of files) {
        ...
        for (const line of delta.text.split("\n")) {
          ...
          if (r.type === "tool_use") { prev.toolCount++; send("agent:tool_use", { tool: r.payload.tool }); events++; }
          else if (r.type === "assistant_reply") { send("agent:assistant_reply", { replyLen: r.payload.replyLen }); events++; }
        }
        prev.sawGrowth = true;
  ...
  return {
    ...
    tick() {
      ...
      for (const p of parts) {
        ...
        if (!ev) continue;
        if (ev.type === "tool_use") { if (a) a.toolCount++; else active.set(sid, { label: sid, firstSeenAt: now(), lastActivity: now(), toolCount: 1 }); send("agent:tool_use", { tool: ev.payload.tool }); events++; }
        else if (ev.type === "assistant_reply") { send("agent:assistant_reply", { replyLen: ev.payload.replyLen }); events++; }
      }
      firstScan = false;
