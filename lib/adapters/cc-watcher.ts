...
// 全量 TS 升级工单阶段 3：本文件为唯一实现，lib/adapters/cc-watcher.mjs 降为同名薄桶（agent-watcher.mjs 零改动）
import { existsSync, readdirSync, statSync, openSync, readSync, closeSync } from "node:fs";
import path from "node:path";
import os from "node:os";
import type { UnifiedEvent } from "../events.mjs";

export const defaultCcDir = (): string => path.join(os.homedir(), ".claude", "projects");

/** 解析出的单条 CC 事件（type/payload + 可选 ts） */
export interface CcParsedEvent {
  type: string;
  payload: Record<string, unknown>;
  ts?: number;
}

/** 事件发射回调（形状直接复用 lib/events.ts 的统一事件模型——多源适配器同一把尺） */
export type CcEmit = (ev: UnifiedEvent) => void;

/** CC watcher 选项 */
export interface CcWatcherOpts {
  ccDir?: string;
  emit?: CcEmit;
...
export function parseCcLine(line: string): CcParsedEvent | null {
...
/** CC 会话文件状态 */
interface CcFileState {
  bytes: number; started: boolean; finished: boolean; sawGrowth: boolean;
  lastGrowAt: number; firstSeenAt: number; toolCount: number;
}

/**
 * 创建 CC watcher（手动 tick 驱动：widget 用 setInterval 调 tick；测试直接调 tick）
 */
export function createCcWatcher({ ccDir = defaultCcDir(), emit = () => {}, idleTimeoutMs = 90000 }: CcWatcherOpts = {}): CcWatcher {
  const state = new Map<string, CcFileState>();
  let firstScan = true;
  ...
  function scanOne(file: string): number {
    let st;
    ...
    if (delta) {
      for (const l of delta.split("\n")) {
        ...
        if (r.type === "tool_use") {
          s.toolCount++;
          emitEvent("agent:tool_use", { tool: r.payload.tool, sessionId: path.basename(file, ".jsonl"), label: path.basename(path.dirname(file)), dir: path.dirname(file) });
        } else if (r.type === "assistant_reply") {
          emitEvent("agent:assistant_reply", { replyLen: r.payload.replyLen, sessionId: path.basename(file, ".jsonl"), label: path.basename(path.dirname(file)), dir: path.dirname(file) });
        }
        newEvents++;
      }
   ...
   */
  function checkFinished(file: string, now = Date.now()): number {
