...
export function setConfig({ email = "", authCode = "" } = {}) {
  const cleanEmail = String(email || "").trim();
  const cleanAuth = String(authCode || "").trim();
  if (!cleanEmail || !cleanAuth) return { ok: false, error: "邮箱地址和授权码不能为空" };
  const cfg = { email: cleanEmail, authCode: cleanAuth, enabled: true, updatedAt: Date.now() };
  try {
    db.prepare("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?, ?, ?)")
      .run(CONFIG_KEY, JSON.stringify(cfg), Date.now());
    return { ok: true, email: cleanEmail, enabled: true };
  } catch (e) {
...
export async function fetchUnreadEmails(config, { sinceDays = 3, max = 30, clientFactory = defaultClientFactory } = {}) {
  ...
  try {
    ...
    try {
      ...
      let uids = [];
      try {
        const r = await client.search({ unseen: true }, { uid: true });
...
export function saveEvents(events = []) {
  ...
  const exists = db.prepare("SELECT 1 FROM schedule_events WHERE email_id = ?");
  ...
  for (const e of events || []) {
    ...
    const t = new Date(String(e.interviewAt ?? "")).getTime();
    if (Number.isNaN(t)) { skipped++; continue; } // interviewAt 缺失/无法解析 → 跳过
    if (exists.get(emailId)) { skipped++; continue; }
    const r = ins.run(
      company.slice(0, 60),
...
// 全部未来日程（面板展示），上限 SCHEDULE_CAP
export function getSchedule() {
  const now = Date.now();
  try {
    const rows = db.prepare(
      "SELECT * FROM schedule_events WHERE interview_at >= ? ORDER BY interview_at ASC LIMIT ?"
...
export async function runMailCheck({ clientFactory = defaultClientFactory, llm = null } = {}) {
  ...
  try {
    ...
    return { ok: true, emails: emails.length, added: saved.added, skipped: saved.skipped, upcoming };
  } catch (e) {
    return { ok: false, error: String(e.message).slice(0, 200) };
  }
}
