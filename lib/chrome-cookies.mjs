...
/**
 * 读取浏览器中指定域名匹配的 Cookie（实时同步用户会话，泛化版）
 * @param {string} domainLike SQL LIKE 模式，如 '%zhipin.com' / '%nowcoder.com'
...
export async function readBrowserCookies(domainLike, browser) {
  ...
  for (const b of candidates) {
    ...
    try {
      ...
      // 遍历该浏览器的所有 profile（Default/Profile N）找匹配 cookie
      ...
      for (const profile of profiles) {
        ...
        try {
          ...
          if (out.length) return out; // 找到匹配登录态
        } finally {
