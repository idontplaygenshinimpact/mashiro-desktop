...
/** 单站点版本检测结果
 * @typedef {{version:string,date:string,checkedAt:string,ok:boolean,error?:string,note?:string,source?:string,localVersion?:string}} DocCheck */
/** 版本检测缓存表：站点名 → 检测结果；_lastCheck = 整表最近检查时间
 * @typedef {Record<string, DocCheck> & { _lastCheck?: string }} VersionCache */
...
export function getLearningDocs() {
  ...
  try {
    ...
    const cached = readJsonSafe(VERSIONS_FILE, /** @type {VersionCache} */ ({}));
