...
// ---------- 面试方向池（模拟面试 agent 化工单任务 1：固定序列 → 方向池——开场固定，之后面试官自主选） ----------
...
// 兼容：ROUND_PLAN/ROUND_SEQ 保留（旧引用——新逻辑用 DIRECTION_POOL 自主选）
export const ROUND_PLAN = [OPENING_ROUND, ...DIRECTION_POOL];
// 扁平化为逐轮类型（rounds 展开）
...
// 全量 TS 升级工单叶子批次 D：lib/interview-config.mjs → .ts（纯常量表，strict 零报错）
