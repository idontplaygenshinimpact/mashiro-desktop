...
export async function runPipeline(stages, ctx, { onStage, onError } = {}) {
  ...
  for (const s of stages) {
    const t0 = Date.now();
    // 观察者回调（onStage）失败不应影响阶段结果：移出 try，单独保护
    let stageError = null;
    try {
      await s.run(ctx);
    } catch (e) {
    ...
    if (stageError) {
      ...
      ctx.errors.push({ stage: s.name, error: msg });
      ...
      try { if (onStage) onStage(s.name, ctx, ctx.stageTimings[s.name]); } catch { /* observer 异常不影响管线 */ }
    }
  }
  return ctx;
}
