...
export const SENSITIVE_MCP_TOOLS: ReadonlySet<string> = new Set([
...
/** 是否敏感数据工具 */
export function isSensitiveMcpTool(name: unknown): boolean {
  return SENSITIVE_MCP_TOOLS.has(String(name || ""));
}

/** 门控是否开启（默认关——零配置可用承诺；env 开启） */
export function mcpReadGateEnabled(): boolean {
  return process.env.MIANSHI_MCP_GATE === "on";
}

/** 门控检查结果（allow 放行；拒绝带 error 提示） */
 ...
 */
export function checkMcpReadGate(name: string, args: Record<string, unknown> = {}): McpGateResult {
 ...
 */
export function recordMcpRead(name: string, args: unknown = {}, ok = true): void {
  try {
...
export const SENSITIVE_CONFIRM_FIELD: Record<string, { type: string; enum: string[]; description: string }> = {
  confirm: {
    type: "string",
    enum: ["yes"],
