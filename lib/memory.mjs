...
// 泛词 3-gram（"的区别/原理/应用"等常见后缀——共享不代表知识点相似：
// "NSP 与 MLM 的区别" vs "LangChain 和 LangGraph 的区别和应用" 共享"的区别"被误判相似 →
// 讲解复用张冠李戴（NSP 条目命中了 LangChain 的存档）——泛词共享不触发强相似，回落 2-gram 重叠率）
const GENERIC_3GRAM = new Set(["的区别", "的原理", "的应用", "是什么", "怎么用", "如何用", "常见问题", "的理解", "有哪些"]);
/**
...
export function isSimilarWeakTopic(a, b) {
  ...
  const g3Shared = [...g3s].filter((x) => g3l.has(x));
  if (g3Shared.some((x) => !GENERIC_3GRAM.has(x))) return true; // 有意义 3-gram 共享 → 强相似
  if (g3Shared.length) return false; // 只有泛词 3-gram 共享（"的区别"）→ 不判相似
  // （NSP vs LangChain：共享"的区别"后缀 → 语义不同知识点；且其 2-gram"的区/区别"会拉高重叠率 0.67，
  //   单靠 2-gram 门槛挡不住——泛词共享直接判否）
...
// 背景修复：讲解复用曾直接用 isSimilarWeakTopic（薄弱点合并的宽松 3-gram 判定），
// 把"数组中第K个最大元素"与"1-n数组中未出现数"判为同一知识点（共享 3-gram"数组中"）→
// 讲解张冠李戴，且重置后仍复用同一错档（用户"重新生成不生效"）。讲解复用额外要求中文
// 2-gram 重叠率 ≥0.6（版本号比较/比较版本号 → 0.75 ✓；数组中第K大/数组中未出现数 → 0.33 ✗）。
// 2026-08 再修：数据结构词差异——"合并有序链表"与"合并有序数组"共享"合并有序"前缀
// （2-gram 重叠 0.6 达标）被判相似 → 链表题复用了数组题讲解（解法不同，张冠李戴）。
// 数据结构词不同 → 不同知识点，不得复用讲解。
const STRUCT_WORDS = ["二叉树", "链表", "数组", "哈希表", "队列", "栈", "图", "堆", "字符串", "矩阵", "集合", "字典", "树"];
function structOf(s) {
...
export function isSimilarTopicForArchive(a, b) {
  if (!isSimilarWeakTopic(a, b)) return false;
  const zh = (s) => String(s).replace(/[^\u4e00-\u9fff]+/g, "");
  const za = zh(a), zb = zh(b);
  // 数据结构词差异：链表 vs 数组 → 不同知识点（解法不同，不得复用讲解）
  const sa = structOf(za), sb = structOf(zb);
...
export const memory = {
  ...
  _cleanTopic(topic) {
    ...
    if (!t) return null;
    if (/考察维度|综合能力|表达能力|^维度|^综合|^沟通|^态度|^思维|^逻辑|面试表现|整体表现|^无$|^暂无|^none$|无具体技术|缺乏项目描述|无技术知识点|无实质|无内容|无知识点|^无[^，。]{0,6}$/i.test(t)) return null;
    return t;
  },
  ...
  /**
   * 相似薄弱点归并（表述漂移整理——幂等；低频任务，数据量通常 <200 行）
