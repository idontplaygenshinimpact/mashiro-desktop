...
function load() {
  ...
  for (const r of rows) {
    try { d[r.key] = JSON.parse(r.value); } catch { /* ignore */ }
  }
  // interests
  ...
  d.interviewHistory = db.prepare("SELECT date, position, role, rounds, avg, dims, report FROM interview_history").all()
    .map((r) => ({ date: r.date, position: r.position, role: r.role, rounds: r.rounds, avg: r.avg, dims: r.dims ? JSON.parse(r.dims) : null, report: r.report }));
  return d;
}
