...
export const memory = {
    ...
    try {
      ...
      if (row) {
        db.prepare(`INSERT INTO weak_points (id, topic, fail_count, last_failed_at, source, origin, updated_at)
          ...
          .run(`wp_${Date.now().toString(36)}${randomUUID().slice(0, 8)}`, row.topic, row.failCount, row.lastFailedAt, row.source || null, row.origin || "agent", Date.now());
      }
    } catch { /* ignore */ }
  ...
  // 把长期记忆注入基础提示词（追加"长期记忆（带来源）"章节）
  injectCuratedIntoPrompt(basePrompt) {
    const cur = this.getCuratedMemory();
