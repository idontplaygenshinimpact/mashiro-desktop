...
function defaults() {
  // 默认求职目标跟随方向画像（转方向/开源自动跟随；画像不可用时"求职"）
  let target = "求职";
  try {
...
export const memory = {
  ...
  appendChat(role, content) {
    ...
    try {
      ...
      // 防无限堆积：上下文只用最近 40 条，DB 保留最近 200 条足够（面板恢复显示 + 复盘留余量），超出自动删最旧
  ...
  addWeakPoint(topic, source, origin = "agent", meta = {}) {
    ...
    // 先按 clean 主题做 DB 增量 UPSERT（不依赖裁剪后的镜像——被挤出 top20 的主题
    ...
    try {
      db.prepare(`INSERT INTO weak_points (id, topic, fail_count, last_failed_at, source, origin, updated_at)
        ...
        .run(`wp_${Date.now().toString(36)}${randomUUID().slice(0, 8)}`, clean, now, source || null, origin || "agent", Date.now());
    } catch { /* ignore */ }
    ...
    if (found) {
      ...
      list.push({ topic: clean, failCount: 1, lastFailedAt: now, source, origin });
    }
