...
/** 注入命中（name=模式名；match=命中片段，截 60 字） */
...
export function sanitizeExternal(text: unknown): { wrapped: string; injections: InjectionHit[] } {
 ...
 * @returns 可安全放入 prompt 的文本
 */
export function safeExternalBlock(text: unknown): string {
  const s = String(text ?? "");
  if (!s.trim()) return s;
...
export const UNTRUSTED_DECLARATION: string =
