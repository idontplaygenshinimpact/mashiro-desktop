...
export function registerMiscRoutes(router) {
    ...
    readBody(req, res, (body) => {
      try {
        ...
        res.end(JSON.stringify({
          ...
          hasKey: !!key,
          message: key
            ? `✅ 已保存：${base ? base + " / " : ""}${mdl || "默认模型"}（立即生效）`
            : `已清除 API Key（回退 .env/环境变量）${base ? "；Base URL 保留" : ""}`,
        }));
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: e.message }));
  ...
  router.route("/api/greeting", "POST", (req, res) => {
    readBody(req, res, async (body) => {
      try {
        ...
        // polishGreeting 返回字符串（LLM 精修或规则版兜底）——此前按对象取 r?.greeting 恒 undefined，精修从未生效
        const txt = await polishGreetingText({ company, title, summary });
        const polished = typeof txt === "string" && txt.trim().length > 0;
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
        res.end(JSON.stringify({ ok: true, greeting: polished ? txt : buildGreetingText({ company, title }), polished: polished }));
      } catch (e) {
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: e.message }));
      }
  ...
  router.route("/api/platforms/apply", "POST", (req, res) => {
    readBody(req, res, async (body) => {
      try {
        const { name = "boss", url, greeting, jobId } = JSON.parse(body || "{}");
