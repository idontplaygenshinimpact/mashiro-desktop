...
import { generateQuiz, ensureQuiz, drawQuiz, submitQuiz } from "../quiz.mjs";
    ...
    readBody(req, res, (body) => {
      try {
        ...
        ensureQuiz(String(cardId)).then((r) => {
          res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
          res.end(JSON.stringify({ ok: r.ok, total: r.total || 0, fromCache: !!r.fromCache, error: r.error || undefined }));
        }).catch((e) => {
      ...
      try {
        ...
        if (r.ok && Array.isArray(r.results) && r.results.some((x) => x && x.correct === false)) {
          try {
            ...
            if (card?.topic) {
              memory.addWeakPoint(String(card.topic).slice(0, 40), "复习自测", "agent", { question: String(card.question || "").slice(0, 300) });
            }
          } catch { /* 回流失败不影响判分返回 */ }
        }
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
