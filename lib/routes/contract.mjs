...
/**
 * 契约路由包装器：body 读取 → input 校验（strip 未知字段，默认不 reject 未知）→
 * 纯函数执行（返回数据，不写 res）→ output 校验（SCHEMA_MISMATCH 即 bug，500）→ 统一序列化。
 * @param {(input: any, ctx: {req: import("node:http").IncomingMessage, res: import("node:http").ServerResponse}) => Promise<any>} fn
 * @param {{ input?: import("zod").ZodType, output?: import("zod").ZodType }} [schema]
 */
export function withContract(fn, { input, output } = {}) {
