...
export function registerCoreRoutes(router, { laneSubmit = (fn) => fn(), runtime = {} } = {}) {
  ...
  router.route("/api/chat", (req, res) => {
    ...
    readBody(req, res, async (body) => {
      try {
        ...
        const result = await laneSubmit(() => chatWithAgent(message, history || [], null, sessionId || "default"));
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
  ...
  router.route("/api/chat/sessions", (req, res) => {
    try {
      res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
      res.end(JSON.stringify({ ok: true, sessions: memory.listChatSessions() }));
    } catch (e) {
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: e.message }));
    }
  });
  router.route("/api/chat/messages", (req, res) => {
    try {
      const u = new URL(req.url, "http://x");
      const session = u.searchParams.get("session") || "default";
      res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
      res.end(JSON.stringify({ ok: true, messages: memory.getChatMessages(session, 40) }));
    } catch (e) {
      res.writeHead(500, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ error: e.message }));
    }
  });
  router.route("/api/chat/session", "DELETE", (req, res) => {
    readBody(req, res, (body) => {
      try {
        const { id } = JSON.parse(body || "{}");
        if (!id) { res.writeHead(400); res.end(JSON.stringify({ error: "id required" })); return; }
        const r = memory.deleteChatSession(String(id));
  ...
  router.route("/api/chat-stream", (req, res) => {
    ...
    readBody(req, res, async (body) => {
      try {
        ...
        res.on("error", () => {});
        const push = (obj) => { if (!res.destroyed && !res.writableEnded) res.write(`data: ${JSON.stringify(obj)}\n\n`); };
        push({ type: "start" });
        ...
        if (!res.destroyed && !res.writableEnded) {
          res.write(`data: ${JSON.stringify({ type: "error", error: String(e?.message || e).slice(0, 200) })}\n\n`);
          res.end();
        }
      }
    });
