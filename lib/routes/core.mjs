...
export function registerCoreRoutes(router, { getCorsOrigin = () => "*", laneSubmit = (fn) => fn(), runtime = {} } = {}) {
  ...
  router.route("/api/health", (req, res) => {
    ...
    res.end(JSON.stringify(buildHealthPayload(dbOk, Math.round(process.uptime()), rt.getActualPort())));
  });
  ...
  router.route("/api/notify-test", (req, res) => {
    rt.sendNotification("✅ 通知测试", "mianshi-agent 小组件通知正常");
