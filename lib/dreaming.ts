...
// 全量 TS 升级工单阶段 3：lib/dreaming.mjs → .ts（widget.mjs 惰性 import 与 tests 按 .mjs 路径加载 → 保留同名一行桶）
...
/** 候选记忆（带溯源：origin 门禁 + sourceRef 引用链） */
...
/** 收集来源（各表行形状由调用方 SQL 决定，此处按动态行收口） */
export interface CollectSource {
  chatHistory?: Array<Record<string, unknown>>;
  weakPoints?: Array<Record<string, unknown>>;
  masteredPoints?: Array<Record<string, unknown>>;
  interviewHistory?: Array<Record<string, unknown>>;
  now?: number;
}

/** node:sqlite 最小面（表可能不存在 → readRows 兜空） */
interface DbLike {
  prepare: (sql: string) => {
...
/** LLM 调用器（默认 ai.ts chat 包装；测试注入假实现） */
export type DreamingLlm = (messages: AgentMessage[]) => Promise<string>;
...
function recencyScore(now: number, ts: unknown): number {
  const t = parseTs(ts);
  if (!t) return 0;
...
export function collectCandidates({ chatHistory = [], weakPoints = [], masteredPoints = [], interviewHistory = [], now = Date.now() }: CollectSource = {}): MemoryCandidate[] {
  ...
  for (const w of weakPoints || []) {
    ...
    if (!topic) continue;
    const failCount = Number(w.failCount ?? w.fail_count ?? 0) || 0;
    out.push({
      text: `薄弱点：${topic}（答错 ${failCount} 次）`,
      origin: String(w.origin || "agent"),
  ...
  for (const c of chatHistory || []) {
    ...
    if (!text) continue;
    out.push({
      text: `对话(${String(c.role || "unknown")})：${text.slice(0, 200)}`,
      origin: c.role === "user" ? "owner" : "agent",
      sourceRef: `chat:${String(c.ts || 0)}`,
  ...
  for (const iv of interviewHistory || []) {
    ...
    out.push({
      text: `面试记录：${String(label)}（${String(iv.date || "未知日期")}）${body}`,
      origin: "agent",
      sourceRef: `interview:${String(iv.date || "unknown")}`,
      recencyScore: recencyScore(now, iv.date),
      importanceScore: Math.min(1 + (Number(iv.rounds) || 0), 5),
    });
  }
...
export function selectCandidates(candidates: MemoryCandidate[] | null | undefined, { maxCount = 12 }: { maxCount?: number } = {}): MemoryCandidate[] {
...
export function buildConsolidationPrompt(candidates: MemoryCandidate[] | null | undefined, existingMemory?: Array<Record<string, unknown>> | null): string {
  const candLines = (candidates || []).map((c, i) =>
    `${i + 1}. [来源:${c.sourceRef}] ${c.text}`).join("\n");
  const existing = (existingMemory || []).length
    ? (existingMemory || []).map((m) => `- ${String(m.topic)}（来源:${String(m.source_ref || m.sourceRef || "未知")}）`).join("\n")
    : "（无）";
...
// ---------- 默认依赖 ----------
async function defaultLlm(messages: AgentMessage[]): Promise<string> {
  const { chat } = await import("./ai.ts");
...
function readRows(dbi: DbLike, sql: string): Array<Record<string, unknown>> {
 ...
 * llm: async (messages) => string（默认 ai.ts chat 包装）；now: 当前时间戳；dbImpl: SQLite 实例（默认 db.mjs db）；logDir: 日志目录
 */
export async function runDreaming({ llm, now = Date.now(), dbImpl, logDir }: { llm?: DreamingLlm; now?: number; dbImpl?: DbLike; logDir?: string } = {}): Promise<DreamingResult> {
  try {
    const dbi = dbImpl || ((await import("./db.mjs")).db as unknown as DbLike);
    const useLlm: DreamingLlm = llm || defaultLlm;
    ...
    const parsed = extractJson(raw) as { entries?: Array<Record<string, unknown>>; drop?: unknown[] } | null | undefined;
    ...
    const dropAllowed = new Set(readRows(dbi, "SELECT topic FROM curated_memory").map((r) => String(r.topic)));
    withTx(() => {
    ...
    try {
      ...
      const lines = [
        ...
        ...(parsed.entries || []).filter((e) => e && e.topic).map((e) =>
          `- **${String(e.topic)}**（重要度 ${e.importance ?? "?"}，来源 ${e.sourceRef ?? "?"}）：${e.content ?? ""}`),
        "",
        "## 丢弃主题",
        ...(parsed.drop || []).map((t) => `- ${String(t)}`),
        "",
      ];
