...
// 全量 TS 升级工单阶段 3：lib/rss.mjs → .ts（widget.mjs / plugins 路由 / tests 按 .mjs 路径加载 → 保留同名一行桶）
...
/** 可注入 parser（真实 rss-parser 或测试假实现） */
export interface RssParserLike {
  parseURL: (url: string) => Promise<{ title?: unknown; items?: Array<Record<string, unknown>> }>;
}
/** 可注入 LLM（测试用假实现；默认走 ai.ts chat） */
export type DigestLlm = (messages: Array<Record<string, unknown>>, opts: Record<string, unknown>) => Promise<string>;
...
// withTimeout 从 text-utils.ts 导入（技术债 L3：收敛单点）

/** 单个 item 发布时间戳（毫秒）；无日期回退当前时间（保证排序稳定） */
function itemPublishedAt(item: Record<string, unknown>): number {
  const s = String(item.isoDate || item.pubDate || "");
 ...
 * parser 可注入（测试用假 parser）：需实现 parseURL(url) → Promise<{title, items:[...]}>
 * 返回扁平化 items：[{feed, title, link, summary, publishedAt}]
 */
export async function fetchRss(feedUrls: string[] = DEFAULT_FEEDS, { parser = null, timeoutMs = FEED_TIMEOUT_MS }: { parser?: RssParserLike | null; timeoutMs?: number } = {}): Promise<RssItem[]> {
  const p: RssParserLike = parser || (new Parser({ timeout: timeoutMs }) as unknown as RssParserLike);
  const lists = await Promise.all(feedUrls.map(async (url): Promise<RssItem[]> => {
    try {
      // SSRF 防护（修复：此前仅 ^https?:// 前缀——内网 http://127.0.0.1/feed 可通过）。
      // RSS 源是用户静态配置（非外部数据驱动 URL），用静态校验（协议 + 内网 hostname/IP 字面量，
      // 无 DNS 解析——对离线/测试友好且覆盖主要风险面）
      let u: URL;
      try { u = new URL(String(url)); } catch { return []; }
      if (u.protocol !== "http:" && u.protocol !== "https:") return [];
      const { isPrivateHostname } = await import("./fetch-page.mjs");
      if (isPrivateHostname(u.hostname)) return [];
      const feed = await withTimeout(p.parseURL(url), timeoutMs);
      return ((feed.items || []) as Array<Record<string, unknown>>).map((it) => ({
        feed: String(feed.title || url),
...
/** LLM picks 条目的规范形状 */
interface Pick { title: string; link: string; reason: string }

/** 规整 LLM picks：去重（按 link）+ 限长 + 截断到 DIGEST_COUNT 条 */
function normalizePicks(parsed: unknown): Pick[] | null {
  if (!parsed) return null;
  const arr: unknown[] | null = Array.isArray(parsed) ? parsed : (Array.isArray((parsed as { picks?: unknown[] })?.picks) ? (parsed as { picks: unknown[] }).picks : null);
  if (!arr?.length) return null;
  const seen = new Set<string>();
  const out: Pick[] = [];
  for (const p of arr) {
    const pk = p as { link?: unknown; title?: unknown; reason?: unknown } | null | undefined;
    const link = String(pk?.link || "").trim();
    if (!link || seen.has(link)) continue;
    seen.add(link);
    out.push({
      title: String(pk?.title || "").trim().slice(0, 120),
      link,
      reason: String(pk?.reason || "").trim().slice(0, REASON_MAX),
    });
 ...
 */
export async function buildDigest(items: RssItem[] = [], { today = null, llm = null }: { today?: string | null; llm?: DigestLlm | null } = {}): Promise<DigestEntry[]> {
  const date = today || localToday();
  const callLlm: DigestLlm = llm || ((messages, opts) => chat(messages as unknown as AgentMessage[], opts as LLMOptions));
  ...
  if (cap.length) {
    ...
    const prof = getCareerProfile() as { scopeNote?: string; ignoreNote?: string; roleLabel?: string };
...
${safeExternalBlock(listText)}`;
  ...
  const validPicks = picks.filter((p) => cap.some((it) => it.link === p.link));
  const digest: DigestEntry[] = validPicks.slice(0, DIGEST_COUNT).map((pk) => {
  ...
  try {
    ...
    console.log(`[rss] 摘要入库失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 80)}`);
  }
  return digest;
}

/** 面板展示用的今日摘要条目 */
export interface DigestViewEntry { feed: unknown; title: unknown; link: unknown; reason: string; publishedAt: unknown }

/** 读取今日摘要（供面板 / GET /api/rss/digest） */
export function getDigest(): DigestViewEntry[] {
...
export function getLastDigestAt(): number {
  try {
    const row = db.prepare("SELECT value FROM settings WHERE key = ?").get(LAST_DIGEST_KEY) as { value?: unknown } | undefined;
    const ts = Number(row?.value);
    return Number.isFinite(ts) && ts > 0 ? ts : 0;
  } catch { return 0; }
}
export function setLastDigestAt(ts: number = Date.now()): void {
  try {
...
export function getFeeds(): string[] {
  try {
    const row = db.prepare("SELECT value FROM settings WHERE key = ?").get(FEEDS_KEY) as { value?: unknown } | undefined;
    if (row?.value) {
...
export function setFeeds(feeds: unknown): { ok: boolean; feeds?: string[]; error?: string } {
  const arr = (Array.isArray(feeds) ? feeds : []).map((f) => String(f).trim()).filter((f) => /^https?:\/\//i.test(f));
...
/** 完整跑一次的结果 */
export interface DailyDigestResult { digest: DigestEntry[]; lastDigestAt: number; feeds: number; total: number }
