...
function loadPlan() {
  ...
  plan.date = dateRow?.date || "";
  plan.items = db.prepare(`SELECT id, topic, why, source, verify_question, done, reviewed, done_at, reviewed_at, level, from_interview
    ...
    level: r.level, fromInterview: !!r.from_interview,
  }));
  return plan;
}
    ...
    for (const it of plan.items || []) {
      db.prepare(`INSERT OR REPLACE INTO study_plan_items
        ...
        .run(
          ...
          it.level || null, it.fromInterview ? 1 : 0, Date.now()
        );
    }
  } catch { /* ignore */ }
}
...
只输出 JSON：
{"items":[{"topic":"...","why":"...","source":"...","verify_question":"...","level":"必会|进阶|拓展"}]}
...
${excerpts.join("\n\n---\n\n").slice(0, 20000)}`;
  ...
  if (!items.length) {
    return { date: "", items: [], error: "解析失败: LLM 未输出有效知识点" };
  }

  // 合并保留：不覆盖旧清单——已完成条目移除，未完成条目（含面试实录回流/手动添加）保留，
  // 新生成条目按 topic 去重追加（避免点"生成"把之前的学习进度清空）
...
export function checkItem(id, done) {
  ...
  if (item.done) {
    ...
    import("./review.mjs").then(({ review }) => {
      review.addCard({
        topic: item.topic,
        question: item.verify_question || `请简述：${item.topic} 的核心要点`,
        answer: "",
        source: "学习清单",
      });
    }).catch(() => { /* ignore */ });
