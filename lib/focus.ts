...
// 全量 TS 升级工单阶段 3：lib/focus.mjs → .ts（widget/loop/插件路由/tests 按 .mjs 路径加载 → 保留同名一行桶）
...
// focus_sessions 表加 goal 列（架构 P0-2：统一 ensureColumn 幂等补列——不再散落裸 try/catch）
ensureColumn("focus_sessions", "goal", "TEXT");
...
/** 分心判定入参 */
export interface FocusRuleInfo { title?: unknown; processName?: unknown }
export interface FocusStats { minutes: number; count: number; distracts: number; week: Array<{ date: string; minutes: number }>; streak: number }
export interface FocusStatus {
  active: boolean;
  phase: "idle" | "focusing" | "resting";
  mode: string | null;
  sessionId: number | null;
  goal: string | null;
...
let lastStop: LastStop | null = null;

// 时长模式 → 分钟；休息时长（可配，默认 5 分钟）
const MODES: Record<string, number> = { "25": 25, "45": 45 };
...
export function setBlacklist(list: unknown): { ok: boolean; blacklist?: string[]; error?: string } {
  if (!Array.isArray(list)) return { ok: false, error: "blacklist 必须是数组" };
  const clean = [...new Set(list.filter((s): s is string => typeof s === "string" && !!s.trim()).map((s) => s.trim()))];
  try {
...
export function setWhitelist(list: unknown): { ok: boolean; whitelist?: string[]; error?: string } {
...
//   /正则/              → 标题正则匹配（如 "/抖音|快手/"）
export function matchRule(rule: unknown, { title, processName }: FocusRuleInfo): boolean {
...
export function isDistracting({ title, processName }: FocusRuleInfo, blacklist?: unknown, whitelist?: unknown): { distracting: boolean; rule?: string; reason?: string } {
  const info: FocusRuleInfo = { title: String(title || ""), processName: String(processName || "") };
  if (Array.isArray(whitelist)) {
    for (const w of whitelist) {
      if (matchRule(w, info)) return { distracting: false, reason: `白名单命中: ${String(w)}` };
    }
  }
  const list = Array.isArray(blacklist) ? blacklist : [];
  for (const kw of list) {
    if (matchRule(kw, info)) return { distracting: true, rule: String(kw) };
  }
  return { distracting: false };
}

/** 兼容旧签名：仅标题 + 黑名单（桌面监督旧路径/测试） */
export function isDistractingTitle(title: unknown, blacklist?: unknown): boolean {
...
function sweepExpired(now: number = Date.now()): string | null {
  if (!session) return null;
  if (session.phase === "focusing" && now >= (session.endAt ?? 0)) {
  ...
  if (session.phase === "resting" && now >= (session.restEndAt ?? 0)) {
...
export function startFocus(mode: unknown, { goal = "", restMinutes }: { goal?: unknown; restMinutes?: unknown } = {}): { ok: boolean; error?: string; sessionId?: number; mode?: string; goal?: string | null; endAt?: number; phase?: string; restMinutes?: number } {
  sweepExpired();
  const m = String(mode || "");
...
export function stopFocus(completed = true): { ok: boolean; error?: string; phase?: string; restDone?: boolean; durationMinutes?: number; completed?: boolean; goal?: string | null } {
  sweepExpired();
...
export function recordDistract(): { ok: boolean; error?: string; distracts?: number } {
  sweepExpired();
...
export function getFocusStats(now: number = Date.now()): FocusStats {
  sweepExpired(now);
  const start = startOfToday(now);
  const rows = db.prepare(
    "SELECT started_at, ended_at, completed, distracts FROM focus_sessions WHERE started_at >= ?"
  ).all(start) as Array<Record<string, unknown>>;
  ...
  for (const r of rows) {
    ...
    // 修复：手动结束（completed=0，中断）也计入实际专注时长——"结束专注不更新专注时间"根因：
    // 此前只统计 completed=1 的完整会话，手动结束（哪怕专注了 20 分钟）不计入今日分钟数。
    // completed 只影响"完成次数"（count），不影响"实际专注分钟"（minutes）。
    if (ended) {
      if (completed) count++;
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    ).all(dayStart, dayEnd) as Array<Record<string, unknown>>;
    let dayMinutes = 0;
    for (const r of dayRows) {
      // 与今日统计同口径：有结束时间就计入实际时长（手动结束/中断也记录实际专注分钟）
      if (r.ended_at) {
  ...
  for (let i = 0; i < 7; i++) {
    const dayStart = startOfToday(now) - i * dayMs;
    const dayEnd = dayStart + dayMs;
    const n = db.prepare("SELECT COUNT(*) n FROM focus_sessions WHERE started_at >= ? AND started_at < ? AND completed = 1").get(dayStart, dayEnd) as { n?: unknown } | undefined;
...
export function getFocusStatus(now: number = Date.now()): FocusStatus {
  const evt = sweepExpired(now);
  const stats = getFocusStats(now);
  const base: FocusStatus = {
    active: false,
    phase: "idle",
    mode: null,
    sessionId: null,
  ...
  if (session.phase === "resting") {
    const restEndAt = session.restEndAt ?? 0;
    return {
      ...base,
      active: true,
      phase: "resting",
      ...
      remainingSeconds: Math.max(0, Math.ceil((restEndAt - now) / 1000)),
      distracts: 0,
    };
  }
  const endAt = session.endAt ?? 0;
  return {
    ...base,
    active: true,
    phase: "focusing",
...
export function getRecentSessions(limit = 5): Array<{ id: number; mode: string; startedAt: number; endedAt: number | null; completed: boolean; distracts: number; goal: string | null }> {
  try {
    return (db.prepare(
