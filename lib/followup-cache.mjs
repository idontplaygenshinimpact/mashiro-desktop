...
import { similarityRule } from "./similarity.mjs"; // 相似度引擎统一工单：追问缓存走 semantic 严格度
...
export function findSimilarFollowup(question, followups, threshold = 0.72) {
  ...
  for (const f of followups) {
    ...
    const r = similarityRule(question, f.question, "semantic");
 ...
 * @param {{ text: string, source: string }|null} [ref] 引用段落（有引用时要求历史追问同引用才命中）
 * @returns {{question: string, answer: string, similarity: number, fromCache: true} | null}
 */
export function queryFollowupCache(topic, question, threshold = 0.72, ref = null) {
  ...
  if (ref?.text) {
    ...
    const sameRef = followups.filter((f) => {
      ...
      return fRef && normalizeQuestion(fRef[1]).slice(0, 40) === refKey;
    });
    if (sameRef.length) {
