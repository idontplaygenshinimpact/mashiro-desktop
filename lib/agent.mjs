...
async function toolFetchPage(url) {
  ...
  try {
    ...
    const isPrivate = host === "localhost" || host === "127.0.0.1" || host === "::1" || host === "0.0.0.0"
      || /^(10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.)/.test(host)
      || /\.local$/.test(host)
      || host === "169.254.169.254"; // 云元数据端点
    if (isPrivate) return { error: "拒绝访问内网/本机地址（SSRF 防护）", title: "" };
  } catch { return { error: "URL 无效", title: "" }; }
