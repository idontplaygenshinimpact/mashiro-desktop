  ...
  let messages = [
    {
      ...
      content:
        ...
        "11) 个人数据：你可以通过 MCP 工具读取用户的个人数据环境——mcp__mashiro__get_personal_profile（个人主页简历）、mcp__mashiro__get_jobs_status（校招推荐/投递状态）、mcp__mashiro__get_schedule_events（面试/笔试日程）、mcp__mashiro__get_study_progress（学习进度总览）；get_memory 也会顺带返回简历摘要+推荐岗位。用户问「我的简历/我投了哪些/我什么时候面试/我学到哪了」时，先调用对应工具拿真实数据再回答，不要凭空猜测。\n" +
        `用户画像：${profile}`,
    },
    ...effectiveHistory,
