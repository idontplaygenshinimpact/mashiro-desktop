...
async function executeTool(name, args) {
  ...
  // 权限门禁：confirm 级工具需用户批准（deny-first；超时/拒绝 → 明确错误回填给 LLM）
  if (TOOL_PERMISSIONS[name] === "confirm") {
    try {
      const { requestApproval } = await import("./permission.mjs");
      const approval = await requestApproval({
        toolName: name,
        args,
        reason: PERMISSION_REASONS[name] || `${name} 是敏感操作`,
      });
      if (!approval.allow) {
...
// 搜索帖子：牛客/掘金（并行抓取加速）——导出供 MCP server 复用
export async function toolSearchPosts(query, site = "auto") {
  ...
  // AI 挑帖：从候选里挑真正的技术面经/笔试（排除求职咨询/闲聊/泛泛内容）
  // 候选少时直接返回；多时用 LLM 判断，避免关键词穷举
  if (all.length > 4) {
    try {
      const { pickPosts } = await import("./ai.mjs");
      const picked = await pickPosts(all.map((p) => ({ text: p.title, href: p.url })), Math.min(6, all.length), []);
      if (picked?.length) {
...
async function callLLM(messages) {
  ...
  return await llmChat(messages, { tools: [...TOOLS, ...mcpToolsCache], temperature: 0.4, maxTokens: 6000 });
}
