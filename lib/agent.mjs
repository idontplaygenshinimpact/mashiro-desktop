...
// @typedef {import("./types.d.ts").ToolErrorKind} ToolErrorKind
...
async function executeTool(name, args) {
  ...
  try {
    ...
    /** @type {Array<{deny?: unknown, reason?: unknown} | null | undefined>} */
    ...
    if (denied) {
      ...
      try { const { traceTool } = await import("./trace.mjs"); traceTool({ toolName: name, args, ok: false, error: msg, durationMs: Date.now() - _tStart }); } catch { /* ignore */ }
      return { error: msg, hint: "请改用其他工具或说明原因", kind: "perm" }; // T2：策略拦截 = 权限类
    }
  ...
  if (name.startsWith("mcp__")) {
    try {
      ...
      return { error: `MCP 调用异常: ${e.message}`, kind: "transient" }; // T2：MCP 网络/超时类 = 瞬时（可重试）
    }
  }
  ...
  if (name.startsWith("skill__")) {
    try {
      ...
      try { const { traceTool } = await import("./trace.mjs"); traceTool({ toolName: name, args, ok: false, error: e.message, durationMs: Date.now() - _tStart }); } catch { /* ignore */ }
      return { error: `skill 调用异常: ${e.message}`, kind: "hard" }; // T2：skill 异常 = 硬错误
    }
  }
  let result;
  try {
    ...
    switch (name) {
      case "plan_task":
        // 编排能力缺口工单 T1：计划写库（PlanStore）——不再口头承诺；返回计划 id + 强制 plan_mode 确认
        {
          ...
          const r = initPlan({ goal: args.goal, steps: args.steps });
          if (!r.ok) return { error: r.error, hint: "计划需要目标与至少一个步骤" };
          return {
            ok: true,
            planId: r.plan.id,
            goal: r.plan.goal,
            steps: r.plan.steps.map((s) => s.title),
    ...
    return { error: `${name} 执行失败: ${e.message}`, kind: "hard" }; // T2：执行异常 = 硬错误（收束）
  }
...
export async function chatWithAgent(userMsg, history = [], onEvent = null, sessionId = "default") {
  ...
  try {
    const relevant = await memory.retrieveRelevant(userMsg, 8);
    if (relevant.length) {
  ...
  for (; rounds < MAX_ROUNDS; rounds++) {
    ...
    try {
      ...
      console.log(`[agent] 流式调用中断: ${String(e?.message || e).slice(0, 120)}`);
      messages.push({
        role: "tool",
        tool_call_id: `interrupt_${rounds}`,
        content: JSON.stringify({ error: `流式响应中断: ${String(e?.message || "未知错误").slice(0, 120)}`, hint: "基于已有信息总结回答，或让用户重试。" }),
      });
