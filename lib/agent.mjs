...
const TOOL_PERMISSIONS = {
  ...
  add_study_items: "confirm",              // 对话写学习清单（用户可见的写操作）
  create_review_card: "confirm",           // 对话建复习卡（用户可见的写操作）
...
export async function chatWithAgent(userMsg, history = []) {
  ...
  // 记忆对话（先剥离语音标记再入库，修复：原顺序导致脏标记持久化）
  ...
  // 回传 history：只含 user/assistant/tool 对话消息（排除 system——面板原样回传会让 system 翻倍，
  // 长期记忆/画像重复注入）；取最近 6 条
