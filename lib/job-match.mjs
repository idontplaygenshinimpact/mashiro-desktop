...
/** 本地时区日期解析（技术债 L2：收敛到 date-utils.mjs 单点） */
import { parseLocalDate } from "./date-utils.mjs";
...
export function scoreJob(job, profile) {
  ...
  if (profile?.skills?.length) {
    ...
    if (targets.includes(job.direction)) s += 15;                 // 意向方向命中（最高权重）
    ...
    if (targets.includes(job.direction)) s += 15;
  }
...
export function setTargetDirection(directions) {
  const list = Array.isArray(directions) ? directions : [directions];
  const clean = list.filter((d) => DIRECTION_NAMES[d] || d === "algorithm");
...
export function getTargetDirection() {
  try {
    ...
    if (Array.isArray(j.directions) && j.directions.length) return j.directions;
