...
// 职责边界（L8 命名澄清，不重命名）：
//   learning-plan.mjs —— 长期学习计划引擎（本文件：计划实体 + 学习事件流 + 趋势聚合，领域无关）
...
export function extractAlgoScope() {
  const kws = new Set();
  const hit = (t) => {
    const s = String(t || "").toLowerCase();
    for (const k of ALGO_SCOPE_WORDS) if (s.includes(k)) kws.add(k);
  };
  try { for (const r of db.prepare("SELECT topic FROM review_cards WHERE type='algo'").all()) hit(r.topic); } catch { /* ignore */ }
  try { for (const r of db.prepare("SELECT topic FROM study_plan_items").all()) hit(r.topic); } catch { /* ignore */ }
  try { for (const r of db.prepare("SELECT title FROM challenges").all()) hit(r.title); } catch { /* ignore */ }
  return [...kws];
}
 ...
 */
export async function createLearningPlan({ title, scope = [], quotaPerDay = 2, durationDays = 30, milestones = [] } = {}) {
  ...
  try {
    ...
    addPlanItems(cleanScope
      ...
      .map((s) => ({
        topic: s,
        why: `学习计划「${t}」每日任务（${quotaPerDay} 个/天）`,
        source: `学习计划·${t}`,
