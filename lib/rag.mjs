...
// 默认 bge-small-zh-v1.5：WASM 常驻内存 ~100MB（bge-m3 568M 参数要 800MB+，知识库几千条用不上）
// 换模型后旧向量维度不匹配 → 自动全量重建（见 RAG_MODEL_KEY 指纹）
const EMBED_MODEL = process.env.RAG_EMBED_MODEL || "Xenova/bge-small-zh-v1.5";
const RAG_MODEL_KEY = "rag_embed_model"; // 上次构建用的 embedding 模型（指纹）
let embedderPromise = null; // promise 缓存：并发首次调用只加载一次
let embedRetryAt = 0;      // 失败冷却：到该时间戳后才允许重试（避免每次调用都重下模型）
async function getEmbedder() {
  if (!ragEnabled()) return null; // 知识库开关关闭：不加载模型（0 内存占用）
...
async function embedBatch(ed, items) {
  ...
  for (let i = 0; i < items.length; i += CHUNK) {
    ...
    // 让出事件循环：WASM embedding 推理在主线程同步执行，连续推理会阻塞 HTTP 服务（重启后 8899 假死）
...
export async function rebuildKnowledgeBase(onProgress = null) {
  ...
  return withBuildLock(async () => {
    ...
    // 记录本次构建用的 embedding 模型（换模型指纹 → 增量重建时强制全量）
    db.prepare("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?,?,?)")
      .run(RAG_MODEL_KEY, EMBED_MODEL, now);
...
export function getKnowledgeStats() {
  ...
  return { total, byKind, lastBuild: last, embedding: !!embedder, embedError, enabled: ragEnabled() };
}
...
/**
 * 增量重建：只重嵌 新增/修改 的 md 文件切片 + DB 资产（快照 hash 变化才重刷，避免每次全量重嵌阻塞主线程）
...
export async function incrementalRebuild() {
  if (!ragEnabled()) return { ok: false, disabled: true, message: "本地知识库未启用（设置中心可开启）" };
  // 模型指纹检查（在 withBuildLock 外，避免嵌套死锁）：库非空但指纹缺失/不同 →
  // 旧模型向量维度不匹配，直接委托全量重建（含新模型下载 + 重嵌全部资产）
  try {
    const libCount = db.prepare("SELECT COUNT(*) n FROM knowledge_items").get().n;
    const prevModel = readSetting(RAG_MODEL_KEY, "");
    if (libCount > 0 && prevModel !== EMBED_MODEL) {
      console.log(`[rag] embedding 模型变更 ${prevModel || "(未知)"} → ${EMBED_MODEL}，触发全量重建（旧向量维度不匹配）`);
      const r = await rebuildKnowledgeBase();
      return { ...r, changed: true, added: r.items, removed: 0, modelChanged: true };
    }
  } catch { /* 表不存在等：交给正常增量路径 */ }
  return withBuildLock(async () => {
    const t0 = Date.now();
    ...
    // DB 资产（学习清单/复习卡/岗位/文档/真题/刷题）：快照 hash 变化才重刷（避免每次全量重嵌）
    ...
    // 持久化 mtime（清除已删除文件的记录）+ DB 资产快照 hash
    const cleaned = {};
