...
export async function askKnowledge(query) {
  ...
  const context = hits.map((h, i) => `【资料${i + 1}】${h.title}\n${h.content}`).join("\n\n");
...
export async function incrementalRebuild() {
  ...
  for (const [src, items] of bySource) {
    ...
    // 删旧条目（该 source 全部切片，按 id 精确删除）
    const old = db.prepare("SELECT id FROM knowledge_items WHERE source=?").all(src);
    db.exec("BEGIN");
    try {
      for (const d of old) {
        db.prepare("DELETE FROM knowledge_fts WHERE id=?").run(String(d.id));
        db.prepare("DELETE FROM knowledge_items WHERE id=?").run(String(d.id));
        removed++;
      }
      const vectors = await embedBatch(ed, items);
      items.forEach((it, i) => insertItem(genId(idBase), it, vectors[i], now));
  ...
  // 已从磁盘删除的文件：清理其残留条目（mtimes 有记录但 bySource 不再包含）
  ...
  // DB 资产（学习清单/复习卡/岗位/文档）：全量重刷
  const dbItems = collectDbAssets();
  const oldDb = db.prepare("SELECT id FROM knowledge_items WHERE source IN ('study','review','weak','job','doc')").all();
  if (oldDb.length || dbItems.length) {
    db.exec("BEGIN");
    try {
      for (const d of oldDb) {
        db.prepare("DELETE FROM knowledge_fts WHERE id=?").run(String(d.id));
        db.prepare("DELETE FROM knowledge_items WHERE id=?").run(String(d.id));
        removed++;
      }
      const vectors = await embedBatch(ed, dbItems);
      dbItems.forEach((it, i) => insertItem(genId(idBase), it, vectors[i], now));
      added += dbItems.length;
      db.exec("COMMIT");
    } catch (e) { db.exec("ROLLBACK"); throw e; }
  }
