// 本地 RAG 知识库：全量资产（面经 md/学习清单/复习卡/薄弱点/岗位/官方文档）→ 切片 → FTS5 trigram 关键词检索
// 检索：FTS5 trigram（中文 3 字窗口，天然支持中文关键词）——轻量实现：零模型、零内存、毫秒级
// （曾用 bge-m3/bge-small WASM embedding 语义检索：800MB 内存 / 效果一般，评估后移除）
...
// ---------- 知识库开关（设置中心可配：rag_enabled "1"/"0"，默认关闭） ----------
// 评估结论：agent 几乎不查本地知识库（trace：search_knowledge 工具历史仅 1 次调用），
// 但 embedding 模型常驻内存成本高（bge-m3 曾占 800MB+）→ 默认关，需要时面板开启
  ...
  const walk = (dir) => {
    for (const name of readdirSync(dir)) {
      ...
      for (const sec of sections) {
        ...
        if (seen.has(content)) continue; // 同内容块去重（同文件重复块 / 跨文件相同 md）
        seen.add(content);
...
async function collectDbAssets() {
  ...
  // 个人项目档案（模拟面试拷打素材；与 indexPersonalProjects 同口径：档案 <200 字符跳过）
  // 全量重建必须重灌 project:* —— 否则面板"重建索引"后项目档案全部丢失且不恢复
  // 修复（2026-08）：buildProjectArchive 已 async——此处必须 await（此前 a 是 Promise，
  // a.content undefined → catch → project 从不重灌——测试回归）
  try {
    for (const p of getPersonalProjects()) {
      const a = await buildProjectArchive(p);
      if (!a || a.content.length < 200) continue;
      items.push({ source: `project:${p.name}`, kind: "project", title: a.title, content: a.content, confidence: 1.0, evidence: p.dir });
    }
  } catch { /* ignore */ }
  return items;
}
...
export async function rebuildKnowledgeBase(_onProgress = null) {
  ...
  return withBuildLock(async () => {
    ...
    const dbItems = await collectDbAssets();
    const items = [...mdItems, ...dbItems];
    const now = Date.now();
    const idBase = now.toString(36);
    // 清库 + 插入在同一个同步事务里（纯 SQL，毫秒级，无外部依赖）
    db.exec("BEGIN");
    try {
      db.exec("DELETE FROM knowledge_items; DELETE FROM knowledge_fts;");
      items.forEach((it) => insertItem(genId(idBase), it, now));
      db.exec("COMMIT");
    ...
    for (const it of mdItems) {
      try {
        ...
        mtimes[it.source] = { m: st.mtimeMs, s: st.size };
      } catch { /* ignore */ }
    }
    db.prepare("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?,?,?)")
      .run(MTIMES_KEY, JSON.stringify(mtimes), now);
    // 记录 DB 资产快照 hash（否则全量重建后首次增量必全量重刷 DB 资产：+N/-N churn）
    const dbHash = simpleHash(dbItems.map((it) => `${it.source}|${it.title}|${it.content}`).join("\n"));
    db.prepare("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?,?,?)")
      .run(DB_HASH_KEY, dbHash, now);
    return { items: items.length, seconds: Math.round((Date.now() - now) / 100) / 10 };
  });
}
...
export async function searchKnowledge(query, topK = 5) {
  ...
  if (!q) return [];
  const K = 50; // 候选池留足：FTS 排名靠后的标题命中需要"提权"空间（如 HTTP 被 https:// URL 噪声淹没）
  ...
  if (terms.length) {
    try {
      ...
      rows.forEach((r, i) => hits.set(String(r.id), { id: String(r.id), score: 1 / (60 + i) }));
    } catch { /* 查询无匹配或语法错误 */ }
  }
  // 短词 LIKE 兜底（title/content 子串；转义 LIKE 通配符防注入/误匹配）
  for (const st of shortTerms) {
    const esc = st.replace(/[\\%_]/g, (c) => "\\" + c);
    try {
      const rows = db.prepare(`SELECT id FROM knowledge_items WHERE title LIKE ? ESCAPE '\\' OR content LIKE ? ESCAPE '\\' LIMIT 10`)
        .all(`%${esc}%`, `%${esc}%`);
      rows.forEach((r, j) => {
        const id = String(r.id);
        const s = 1 / (90 + j);
        const cur = hits.get(id);
        if (!cur || s > cur.score) hits.set(id, { id, score: s });
      });
    } catch { /* ignore */ }
  }

  const ranked = [...hits.values()].sort((a, b) => b.score - a.score).slice(0, K);
  if (!ranked.length) return [];

  // 只回查命中条目（性能工单任务 4：此前全表加载再按 id 查——检索 O(N)；hits 只取 K 条）
  ...
  return ranked
    ...
    .map((r) => {
      ...
      return {
        ...
        vectorScore: 0,
        ftsScore: Math.round(r.score * 1000) / 1000,
      };
    });
}
...
export async function askKnowledge(query) {
  ...
  const context = hits
    .map((h, i) => `【资料${i + 1}】${safeExternalBlock(`${h.title}\n${h.content}`)}`)
    .join("\n\n");
...
export async function incrementalRebuild() {
  ...
  return withBuildLock(async () => {
    ...
    // DB 资产（学习清单/复习卡/岗位/文档/真题/刷题/个人项目档案）：快照 hash 变化才重刷（避免每次全量重嵌）
    const dbItems = await collectDbAssets();
    const dbHash = simpleHash(dbItems.map((it) => `${it.source}|${it.title}|${it.content}`).join("\n"));
    const prevDbHash = readSetting(DB_HASH_KEY, "");
    if (dbHash !== prevDbHash) {
      const oldDb = db.prepare("SELECT id FROM knowledge_items WHERE source IN ('study','review','weak','job','doc','zhenti','zhenti-q','oj') OR source LIKE 'project:%'").all();
      for (const d of oldDb) delIds.push(String(d.id));
      insItems.push(...dbItems);
    }
    ...
    if (delIds.length || insItems.length) {
      ...
      try {
        ...
        for (const it of insItems) {
          insertItem(genId(idBase), it, now);
          added++;
        }
        db.exec("COMMIT");
      } catch (e) { db.exec("ROLLBACK"); throw e; }
    ...
    return { changed, added, removed, seconds: Math.round((Date.now() - t0) / 100) / 10 };
  });
}
