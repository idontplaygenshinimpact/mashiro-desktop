...
/**
 * 全量重建知识库
 * @returns {Promise<{items:number, seconds:number}>}
 */
...
export async function searchKnowledge(query, topK = 5) {
  ...
  const ranked = ftsHits.sort((a, b) => b.score - a.score).slice(0, topK);
...
/**
 * 增量重建：只重嵌 新增/修改 的 md 文件切片 + DB 资产（快照 hash 变化才重刷，避免每次全量重嵌）
 * @returns {Promise<{changed:boolean, added:number, removed:number, seconds:number}>}
 */
