// lib/speech.ts —— 语音输入识别（本地离线 ASR，零 API key）
...
// 注意：推理必须跑在 worker 线程（lib/speech-worker.ts 容器）——同步计算放主进程
//       会阻塞 Electron 整个应用（历史卡顿根因）。
// 全量 TS 升级工单阶段 3：lib/speech.mjs → .ts（lib/speech-worker.ts 与 tests 按 .mjs 路径加载 → 保留同名一行桶）
...
/** 识别结果（面板/worker 用） */
export interface TranscribeResult { ok: boolean; text?: string; error?: string; segments?: number }
/** 分段边界（样本下标） */
export interface VoiceSegment { start: number; end: number }

// ---------- sherpa-onnx paraformer-zh（默认引擎） ----------
/** sherpa-onnx-node 的最小面（CJS require，无类型声明 → 本地收口） */
interface SherpaStream { acceptWaveform: (o: { samples: Float32Array; sampleRate: number }) => unknown; free?: () => void }
interface SherpaRecognizer {
  createStream: () => SherpaStream;
  decode: (s: SherpaStream) => unknown;
  getResult: (s: SherpaStream) => { text?: string } | undefined;
}
/** 加载期失败标记（模块 require/构造/模型缺失 → 上层统一回退 whisper） */
type SherpaLoadError = Error & { sherpaLoadFailed?: boolean };

let recognizerPromise: Promise<SherpaRecognizer> | null = null; // 单例（懒加载 + 缓存；失败重置可重试）

function getSherpaRecognizer(): Promise<SherpaRecognizer> {
  if (!recognizerPromise) {
    recognizerPromise = (async () => {
      ...
      const sherpa = require("sherpa-onnx-node") as { OfflineRecognizer: new (cfg: unknown) => SherpaRecognizer };
      ...
      const err: SherpaLoadError = e instanceof Error ? e : new Error(String(e));
      err.sherpaLoadFailed = true;
      throw err;
    });
  }
...
async function transcribeSherpa(audio: Float32Array): Promise<TranscribeResult> {
...
function getWhisperASR(): Promise<AsrFn> {
  if (!asrPromise) {
    asrPromise = (async () => {
      const { pipeline, env } = await import("@xenova/transformers") as { pipeline: (...args: unknown[]) => Promise<unknown>; env: { remoteHost: string } };
      env.remoteHost = process.env.HF_ENDPOINT || "https://hf-mirror.com/"; // 国内镜像
      return await pipeline("automatic-speech-recognition", ASR_MODEL, { quantized: true }) as AsrFn;
    })().catch((e: unknown) => {
...
async function transcribeWhisper(audio: Float32Array): Promise<TranscribeResult> {
 ...
 */
export function segmentVoice(pcm: Float32Array | null | undefined, sr: number = 16000, maxSec: number = SEG_MAX_SEC): VoiceSegment[] {
  ...
  const wins: Array<{ start: number; end: number; e: number }> = [];
  for (let i = 0; i + win <= total; i += step) {
    let sum = 0;
    for (let j = i; j < i + win; j++) sum += (pcm as Float32Array)[j] * (pcm as Float32Array)[j];
  ...
  // ③ 外扩一窗
  const segs: VoiceSegment[] = islands.map(([a, b]) => ({
...
async function transcribeOne(audio: Float32Array): Promise<TranscribeResult> {
  if (ENGINE === "sherpa") {
    try {
      ...
      if ((e as SherpaLoadError | undefined)?.sherpaLoadFailed) {
...
export async function transcribeAudio(audio: Float32Array): Promise<TranscribeResult> {
  ...
  try {
    ...
    console.error("[speech] 识别失败:", e instanceof Error ? e.message : e);
    return { ok: false, error: `识别失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 120)}` };
  }
}
...
export function fixTerms(text: unknown): string {
  let t = String(text || "");
