...
async function llmFetch(messages, opts = {}, stream = false, onChunk = null) {
  ...
  // 端点列表：主 → 备用（failover）；主端点用通用模型名，备用（官方 API）用官方专用模型
  const endpoints = [{ url: config.baseUrl, key: config.apiKey, model: config.model }];
  if (config.fallbackBaseUrl && config.fallbackApiKey) {
    endpoints.push({ url: config.fallbackBaseUrl, key: config.fallbackApiKey, model: config.officialModel || config.model });
  }

  let lastErr;
  const timeoutMs = opts.timeout || (stream ? DEFAULT_TIMEOUT : DEFAULT_TIMEOUT_NON_STREAM);
