...
async function llmFetch(messages, opts = {}, stream = false, onChunk = null) {
  ...
  const endpoints = (config.providers?.length
    ? config.providers.map((p) => ({ url: p.baseUrl, key: p.apiKey, model: p.model, name: p.name }))
    : []
  );
  // 兜底：config.providers 缺失时退化为旧双端点逻辑（防旧版 config 缓存）
  if (!endpoints.length) {
    endpoints.push({ url: config.baseUrl, key: config.apiKey, model: config.model, name: "main" });
    if (config.fallbackBaseUrl && config.fallbackApiKey) {
      endpoints.push({ url: config.fallbackBaseUrl, key: config.fallbackApiKey, model: config.officialModel || config.model, name: "fallback" });
    }
  }
