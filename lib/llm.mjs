...
/** @typedef {{ role?: string, content?: string, tool_calls?: Array<{ id: string, type: string, function: { name: string, arguments: string } }> }} LLMMessage */
 ...
 */
async function llmFetch(messages, opts = {}, stream = false, onChunk = null, usedEp = null) {
  ...
  const tier = ROLE_TIER[opts.role];
  if (tier) {
      ...
      try {
        ...
        if (stream) {
          ...
          if (toolCalls.size > 0) {
            ...
            markUsed(ep);
            return /** @type {LLMResponse} */ ({ choices: [{ message: { role: "assistant", content: full || null, tool_calls: calls } }] });
          }
...
export async function llmChat(messages, opts = {}) {
  ...
  const usedEp = {}; // 架构 P1-2：实际命中端点（llmFetch 写入）——审计真实 failover 目标
  try {
    const data = /** @type {LLMResponse} */ (await llmFetch(messages, opts, false, null, usedEp));
    ...
    try {
      const { traceLLM } = await import("./trace.mjs");
      traceLLM({ role: opts.role || "agent", model: usedEp.model || config.model, durationMs: Date.now() - start, ok: false, error: e.message, endpoint: usedEp.url || null });
    } catch { /* ignore */ }
...
export async function llmChatStream(messages, opts = {}, onChunk) {
  ...
  const usedEp = {}; // 架构 P1-2：实际命中端点（llmFetch 写入）——审计真实 failover 目标
  try {
    const full = /** @type {string} */ (await llmFetch(messages, opts, true, onChunk, usedEp));
    try {
      const { traceLLM } = await import("./trace.mjs");
      traceLLM({
        role: opts.role || "agent",
    ...
    try {
      const { traceLLM } = await import("./trace.mjs");
      traceLLM({ role: opts.role || "agent", model: usedEp.model || config.model, stream: true, durationMs: Date.now() - start, ok: false, error: e.message, endpoint: usedEp.url || null });
    } catch { /* ignore */ }
    // 架构 P1-6：流式失败也发 llm_done hook
    try {
      const { emitHook } = await import("./hooks.ts");
      emitHook("llm_done", { role: opts.role || "agent", ok: false, stream: true, error: String(e.message || "").slice(0, 200), durationMs: Date.now() - start }).catch(() => {});
    } catch { /* ignore */ }
    throw e;
  }
}
