...
// ---------- 导入（由 scripts/import-ai-career.mjs 调用） ----------
/** 批量导入题目（幂等：INSERT OR REPLACE） */
export function importChallengesData(list) {
  if (!Array.isArray(list) || !list.length) return { ok: false, error: "空数据" };
  const ins = db.prepare(`INSERT OR REPLACE INTO challenges
  ...
  for (const c of list) {
    ...
    ins.run(
      ...
      String(c.skeleton || ""), String(c.testCode || ""), "ai-career", now
    );
    n++;
  }
...
/**
 * 沙箱判题：用户代码 + 测试代码 + `await __test__(导出名)` 在一个 vm 上下文执行
...
export async function runChallengeCode({ userCode, testCode, skeleton, timeoutMs = SANDBOX_TIMEOUT_MS }) {
  ...
  const sandbox = {
    ...
    __sleep__: (ms) => new Promise((r) => setTimeout(r, ms)),
    __assert__: (cond, label) => {
  ...
  const script = `${String(userCode || "")}\n\n${String(testCode || "")}\n\nawait __test__(${exportArgs});`;
  const start = Date.now();
  try {
    await Promise.race([
      (async () => {
        vm.createContext(sandbox);
        await vm.runInContext(`(async () => { "use strict"; ${script} })()`, sandbox, { timeout: timeoutMs });
      })(),
      new Promise((_, reject) => setTimeout(() => reject(new Error(`执行超时（${timeoutMs / 1000} 秒），请检查是否有死循环。`)), timeoutMs)),
    ]);
    return {
