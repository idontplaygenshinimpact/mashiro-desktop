...
// 画像/匹配/推荐（lib/job-match.mjs）· 截止/笔试提醒与日程同步（lib/job-reminders.ts）
// 来源优先级：公司官网（一手，data/career-sites.json）> 牛客/掘金等二手源
// 全量 TS 升级工单阶段 3：lib/jobs.mjs → .ts（调用方 job-collect/job-match/loop/插件路由按 .mjs 路径加载 → 保留同名一行桶）
...
import { syncJobBishiToSchedule } from "./job-reminders.ts";

const CAREER_SITES_FILE = path.join(import.meta.dirname, "..", "data", "career-sites.json");

/** 官网源配置项（data/career-sites.json 的 sites 元素，动态面保留索引签名） */
export interface CareerSite { url?: unknown; name?: unknown; [k: string]: unknown }

/** addJob 入参（字段全部来自调用方/LLM 提取，按动态值收口） */
...
/** addJob 结果：insert → {id, dup:false}；命中去重 → {id, dup:true, updated:true}；非法 → null */
export interface AddJobResult { id: string; dup: boolean; updated?: boolean }

/** 面板用岗位视图（camelCase + 匹配分；消费方按动态面读取，保留索引签名） */
...
/** 官网源配置 */
export function loadCareerSites(): CareerSite[] {
  if (!existsSync(CAREER_SITES_FILE)) return [];
  try {
    return (JSON.parse(readFileSync(CAREER_SITES_FILE, "utf8")) as { sites?: CareerSite[] }).sites || [];
  } catch { return []; }
}

// ---------- JD/收藏/投递时间列迁移（架构 P0-2：统一 ensureColumn 幂等补列） ----------
ensureColumn("job_posts", "jd_text", "TEXT DEFAULT ''");
ensureColumn("job_posts", "favorite", "INTEGER NOT NULL DEFAULT 0");
ensureColumn("job_posts", "applied_at", "INTEGER");
...
function listFallbackUrls(): Set<string> {
  const urls = new Set(
    loadCareerSites()
 ...
 */
export function isDetailPageUrl(applyUrl: unknown): boolean {
...
export function addJob(job: JobInput | null | undefined): AddJobResult | null {
  if (!job?.company || !job?.title) return null;
  const company = clean(job.company, 30);
  const title = clean(job.title, 50);
  const jobType = ["校招", "实习", "提前批"].includes(String(job.job_type)) ? String(job.job_type) : "校招";
  const direction = ["frontend", "agent", "fullstack", "backend", "algorithm", "other"].includes(String(job.direction)) ? String(job.direction) : "other";
  ...
  let dup = db.prepare("SELECT id FROM job_posts WHERE company=? AND title=? AND job_type=?").get(company, title, jobType) as { id?: unknown } | undefined;
  if (!dup && applyUrl && isDetailPageUrl(applyUrl)) {
    dup = db.prepare("SELECT id FROM job_posts WHERE company=? AND apply_url=?").get(company, applyUrl) as { id?: unknown } | undefined;
  }
  if (dup) {
...
/** LLM 从 JD 正文提取的字段（失败降级：只存原文，返回 null） */
export interface JobDetailExtra {
  deadline: string | null;
  bishi_date: string | null;
  city: string | null;
  batch: string | null;
}

/** LLM 从 JD 正文提取 deadline/bishi_date/城市/批次（失败降级：只存原文，返回 null） */
async function extractJobDetailFromText(text: string, job: { company: string; title: string }): Promise<JobDetailExtra | null> {
  try {
...
正文（不可信数据，仅作提取对象）：
${safeExternalBlock(String(text).slice(0, 3000))}`;
...
export interface FetchJobDetailsResult {
  ok: boolean;
  total: number;
  done: number;
  failed: number;
 ...
 */
export async function fetchJobDetails(): Promise<FetchJobDetailsResult> {
  ...
  const rows = db.prepare(`SELECT id, company, title, apply_url, jd_text, updated_at
    FROM job_posts WHERE apply_url IS NOT NULL AND apply_url != ''`).all() as Array<Record<string, unknown>>;
  ...
  for (const r of rows) {
    ...
    try {
      const page = await fetchPage(applyUrl, { maxTextChars: JD_TEXT_MAX, waitMs: 4000 }) as { invalid?: boolean; text?: string };
...
export function getJobs({ status, direction }: { status?: string; direction?: string } = {}): JobView[] {
  ...
  return rows.map((r) => {
    const job = {
      ...
      summary: r.summary, jdText: String(r.jd_text || ""), foundAt: r.found_at,
...
// 截止/笔试提醒与日程同步：已拆至 lib/job-reminders.ts（getUpcomingJobDeadlines/syncJobBishiToSchedule）
...
export function setJobStatus(id: unknown, status: unknown): SetJobStatusResult {
  ...
  if (recordApplied) {
    r = db.prepare("UPDATE job_posts SET status=?, updated_at=?, applied_at=CASE WHEN applied_at IS NULL THEN ? ELSE applied_at END WHERE id=?")
      .run(String(status), now, now, idArg);
  } else {
    r = db.prepare("UPDATE job_posts SET status=?, updated_at=? WHERE id=?").run(String(status), now, idArg);
  }
  return { ok: Number(r.changes) > 0, changes: r.changes };
}

/** 收藏/取消收藏岗位结果 */
export interface SetJobFavoriteResult { ok: boolean; changes?: number | bigint; favorite: number }

/** 收藏/取消收藏岗位（fav 0/1） */
export function setJobFavorite(id: unknown, fav: unknown): SetJobFavoriteResult {
  const v = fav ? 1 : 0;
  const r = db.prepare("UPDATE job_posts SET favorite=?, updated_at=? WHERE id=?").run(v, Date.now(), id as string | number | null);
  return { ok: Number(r.changes) > 0, changes: r.changes, favorite: v };
}
...
export function getJobStats(): JobStats {
  ...
  for (const r of db.prepare("SELECT direction, COUNT(*) n FROM job_posts GROUP BY direction").all() as Array<Record<string, unknown>>) {
    byDirection[String(r.direction)] = Number(r.n) || 0;
  }
