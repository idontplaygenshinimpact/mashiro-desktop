...
// 全量 TS 升级工单阶段 3：lib/db.mjs → .ts（42 个调用方按 .mjs 路径加载 → 保留同名一行桶）
...
const colNames = (table: string): string[] => {
  try { return (db.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name?: unknown }>).map((c) => String(c.name)); } catch { return []; }
};

/** 版本化迁移条目 */
interface Migration { version: number; name: string; up: () => void }
...
export function runMigrations(): number {
  ...
  for (const m of MIGRATIONS) {
    ...
    try {
      ...
      console.error(`[db] schema 迁移 v${m.version} 失败: ${(e instanceof Error ? e.message : String(e)).slice(0, 200)}——版本未推进，下次启动重试`);
      throw e;
    }
  }
 ...
 * @returns 是否实际补列
 */
export function ensureColumn(table: string, column: string, ddl: string): boolean {
...
// ---------- 初始化 ----------
ensureSchema();
// 复习首刷不计 fail 工单任务 1：card_reviews 加 is_first 列（首刷标记——错题本/薄弱点排除首刷）
try { ensureColumn("card_reviews", "is_first", "INTEGER NOT NULL DEFAULT 0"); } catch { /* 表不可用忽略 */ }
// 架构 P0-2：版本化迁移（成功才推进 user_version；失败 console.error + 抛出让调用方可见）
try {
  runMigrations();
} catch (e) {
  console.error(`[db] schema 迁移未完成（v${schemaVersion()}）——部分功能可能异常: ${(e instanceof Error ? e.message : String(e)).slice(0, 150)}`);
}
