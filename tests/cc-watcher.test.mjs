...
test("追加行 → 增量事件；重复 tick 幂等（不重复出）", () => {
  ...
  assert.deepEqual(types, ["agent:assistant_reply", "agent:tool_use"], "只出新行事件");
...
test("新文件出现 → agent:session_started（含目录）", () => {
  const d = tmpDir();
...
test("多文件各自独立增量", () => {
  ...
  w.tick();
  assert.equal(events.filter((e) => e.type === "agent:assistant_reply").length, 1, "只有 x 出新事件");
  assert.equal(events[0].payload.replyLen, 2, "x2");
});

// ---------- 会话结束 ----------
test("停止增长超过 idleTimeout → agent:session_finished（时长+工具数；须本进程内见过增长）", async () => {
  const d = tmpDir();
  ...
  w.tick(); // 首扫：只建静默偏移，没有增长
  // 关键回归（2026-09-11）：首扫后**没有增长**、仅超时 —— 不得发 finished。
  // 旧实现只看超时，于是把静止的历史会话误报成"刚结束"：本机 decision_ledger 里 128 条
  // autonomy:cc:session_finished 大部分是这么来的（~/.claude/projects 只有一个 7 月的 jsonl）。
  await new Promise((r) => setTimeout(r, 80));
  w.tick();
  assert.equal(events.filter((e) => e.type === "agent:session_finished").length, 0, "静止的历史会话不判结束");
  // 真增长后停止 → 才允许判结束
  appendFileSync(f, line("assistant", { content: [{ type: "text", text: "r2" }] }) + "\n", "utf8");
  w.tick();
  await new Promise((r) => setTimeout(r, 80));
  w.tick();
  const fin = events.find((e) => e.type === "agent:session_finished");
  assert.ok(fin, "有增长后超时未增长 → session_finished");
  assert.equal(fin.payload.sessionId, "s3");
  assert.equal(typeof fin.payload.toolCount, "number");
  // 幂等：再次 tick 不重复 finished
  w.tick();
  assert.equal(events.filter((e) => e.type === "agent:session_finished").length, 1);
});
