...
test("S1 Tab 组件存在且按 tab 分发（驾驶舱/知识库 + 数据入口）", () => {
  for (const f of ["tabs/Dashboard.jsx", "tabs/Kb.jsx", "api.js"]) {
    assert.ok(existsSync(R("desktop/renderer/panel-react/src/" + f)), `src/${f} 存在`);
  }
  const main = readFileSync(R("desktop/renderer/panel-react/src/main.jsx"), "utf8");
  assert.ok(/const TABS = \{[\s\S]*?interview:[\s\S]*?dashboard: DashboardPanel[\s\S]*?kb: KbPanel/.test(main), "TABS 注册表含 interview/dashboard/kb");
...
test("React 版 Tab 走同一数据源（HTTP 路由）+ 不 import 业务层", () => {
  ...
  for (const [name, src] of [["Dashboard", dash], ["Kb", kb]]) {
