...
test("S1/S2 Tab 组件存在且按 tab 分发（驾驶舱/知识库/清单/爬取 + 数据入口）", () => {
  for (const f of ["tabs/Dashboard.jsx", "tabs/Kb.jsx", "tabs/Study.jsx", "tabs/Crawl.jsx", "tabs/Jobs.jsx", "tabs/Chat.jsx", "api.js"]) {
    assert.ok(existsSync(R("desktop/renderer/panel-react/src/" + f)), `src/${f} 存在`);
  }
  const main = readFileSync(R("desktop/renderer/panel-react/src/main.jsx"), "utf8");
  assert.ok(/const TABS = \{[\s\S]*?interview:[\s\S]*?dashboard: DashboardPanel[\s\S]*?kb: KbPanel[\s\S]*?study: StudyPanel[\s\S]*?crawl: CrawlPanel[\s\S]*?jobs: JobsPanel[\s\S]*?chat: ChatPanel/.test(main), "TABS 注册表含 interview/dashboard/kb/study/crawl/jobs/chat");
...
test("React 版 Tab 走同一数据源（HTTP 路由 / IPC 桥）+ 不 import 业务层", () => {
  ...
  const chat = readFileSync(R("desktop/renderer/panel-react/src/tabs/Chat.jsx"), "utf8");
  ...
  assert.ok(chat.includes("window.kanban.chatStream") && chat.includes("window.kanban.chatSessions"), "对话与原 panel-chat.js 同 IPC 桥（流式/会话）");
  for (const [name, src] of [["Dashboard", dash], ["Kb", kb], ["Study", study], ["Crawl", crawl], ["Jobs", jobs], ["Chat", chat]]) {
  ...
  assert.ok(chat.includes("useReducer") && chat.includes("React 特性"), "对话用 useReducer 状态机并标注");
});