// web-search.mjs 单测：Bing 搜索结果解析 / 去重 / 失败降级（mock fetch-page + 注入 fetchFn seam）
...
test("searchWeb 解析 Bing 搜索结果：title+url+snippet + URL 去重 + 导航过滤", async () => {
  ...
  const r = await searchWeb("React 19 新特性");
...
test("searchWeb limit 截断到指定条数", async () => {
  ...
  setMockPages([{ links, text: "" }]);
  const r = await searchWeb("x", { limit: 3 });
...
test("searchWeb 无效页 / 空 links → 返回 []", async () => {
  setMockPages([{ invalid: true, links: [] }]);
  assert.deepEqual(await searchWeb("x"), []);
  setMockPages([{ links: [] }]);
  assert.deepEqual(await searchWeb("x"), []);
});
