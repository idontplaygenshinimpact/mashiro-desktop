...
// 与 desktop/tts-edge.ts（主进程预设播放）配合：预设命中零延迟，未命中走实时合成
// 全量 TS 升级工单阶段 4（渲染层）：实现迁至 speech-queue.ts，speech-queue.mjs 保留同名薄桶
//   （esbuild 入口 / scripts/_tts-play-sim.mjs / tests/speech-queue.test.mjs 按 .mjs 路径引用 → 零改动）
...
/** 播放队列选项：prepare 准备音频（返回句柄/null=跳过），play 播放并 resolve 表示播完。
 * 句柄类型 A 由调用方决定（真实合成返回文件路径字符串；预设命中可返回其它句柄）——队列只负责搬运 */
export interface SpeechQueueOpts<A = unknown> {
  prepare?: (text: string) => Promise<A | null>;
  play: (audio: A, text: string) => Promise<void>;
  /** 每日播报预算（超出静默跳过，不阻塞对话） */
  budget?: number;
}
 ...
 * 只有遇到终止符（。！？\n）才切出完整句；代码块/URL 行不播。
 */
export function splitSentences(text: unknown): SplitResult {
...
export function flushRest(rest: unknown, minLen = 2): string {
 ...
 */
export function createSpeechQueue<A = unknown>({ prepare, play, budget = 200 }: SpeechQueueOpts<A>): SpeechQueue {
  let queue: string[] = [];
  let playing = false;
  let spokenToday = 0;
  let stopped = false;
  let prefetch: { text: string; promise: Promise<A | null> } | null = null; // {text, promise}

  function startPrefetch(): void {
    const prep = prepare; // 局部收窄（strict：可选属性直调会被判 possibly undefined）
  ...
  async function pump(): Promise<void> {
    ...
    while (queue.length > 0 && !stopped) {
      ...
      if (text === undefined) break; // queue 非空时 shift 必有值——显式收窄（strict）
      if (spokenToday >= budget) break; // 预算耗尽：静默跳过，不阻塞对话
      try {
        let audio: A | null = null;
