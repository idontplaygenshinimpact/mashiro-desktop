...
// 全量 TS 升级工单阶段 4（桌面）：实现迁至 foreground.ts，foreground.mjs 保留同名薄桶
// （main.mjs 按 .mjs 路径加载 → 调用方零改动；koffi 自带 index.d.ts，FFI 签名可直接类型化）
import koffi from "koffi";

/** 前台窗口类别（desktop=桌面壳 / fullscreen=全屏应用 / normal=普通窗口） */
export type ForegroundKind = "desktop" | "fullscreen" | "normal";
...
let cachedScreen: { w: number; h: number } = { w: 0, h: 0 };
 ...
 * 检测前台窗口状态
 */
export function detectForegroundSync(): ForegroundKind {
  try {
