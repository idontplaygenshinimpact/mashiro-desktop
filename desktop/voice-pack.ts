...
// 全量 TS 升级工单阶段 4（桌面）：实现迁至 voice-pack.ts，voice-pack.mjs 保留同名薄桶
// （main.mjs / tts-edge.mjs 按 .mjs 路径加载 → 调用方零改动）
...
/** 命中结果（file=音频绝对路径，scene=场景名） */
export interface VoicePackHit { file: string; scene: string }

/** 播放结果（ok=false 时带 debounced/busy/error 说明原因） */
...
/** 语音包台词表条目（lines.json / long.json：{ <scene>: { files: string[] } }） */
export interface VoiceLinesEntry { files?: string[] }
export type VoiceLines = Record<string, VoiceLinesEntry>;

/** 播放进程可挂 killedByUs 标记（被新播放主动掐断 → exit 不记异常日志） */
type VoiceChild = ChildProcess & { killedByUs?: boolean };
...
let linesCache: VoiceLines | null = null;
function loadLines(): VoiceLines {
  if (linesCache) return linesCache;
  let parsed: VoiceLines;
  try {
    parsed = JSON.parse(readFileSync(path.join(VOICE_DIR, "lines.json"), "utf8"));
  } catch {
    parsed = {};
  }
  linesCache = parsed;
...
const SCENE_KEYWORDS: Array<{ scene: string; kws: string[] }> = [
...
export function pickSceneFile(scene: string): string | null {
 ...
 * @param text 待匹配文本
 * @param opts ack=false 时不返回通用应答（调用方自行兜底，如实时 TTS） */
export function matchVoicePack(text: unknown, opts: { ack?: boolean } = {}): VoicePackHit | null {
  const t = String(text || "");
...
export function playScene(scene: string): VoicePlayResult | null {
  const file = pickSceneFile(scene);
  if (!file) return null;
...
// 未配置长句 → 回退短句场景（降级不静音）
let longCache: VoiceLines | null = null;
function loadLongLines(): VoiceLines {
  if (longCache) return longCache;
  let parsed: VoiceLines;
  try {
    parsed = JSON.parse(readFileSync(path.join(VOICE_DIR, "long", "long.json"), "utf8"));
  } catch {
    parsed = {};
  }
  longCache = parsed;
...
export function playLongScene(scene: string): VoicePlayResult | null {
...
export function playClickShort(): VoicePlayResult | null {
  const lines = loadLines();
  const pool: string[] = [];
...
export function playClickLong(): VoicePlayResult | null {
  const lines = loadLongLines();
  const pool: string[] = [];
...
let ffplayPath: string | false | null = null; // null=未探测；string=路径；false=不可用

/** 解析 ffplay 可执行路径：FFPLAY_PATH 覆盖 → where 探测（缓存）→ 不可用（无本机路径兜底，防泄露） */
export function resolveFfplay(): string | false {
...
let activeVoicePlayer: VoiceChild | null = null; // 当前语音播放进程（互斥）
...
function isLongFile(file: unknown): boolean {
  return /[\\/]long[\\/]/.test(String(file || ""));
}

function clearActive(child: VoiceChild | null): void {
...
export function playVoicePack(file: string): VoicePlayResult {
  const now = Date.now();
  ...
  if (ff) {
    try {
      const child: VoiceChild = spawn(ff, ["-autoexit", "-nodisp", "-loglevel", "quiet", file], { windowsHide: true, detached: true, stdio: "ignore" });
      ...
      console.log(`[voice] ffplay spawn 异常: ${eMsg(e)}`);
    }
  }
...
function soundPlayerFallback(file: string, isLong = false): VoicePlayResult {
  try {
    const ps = `(New-Object Media.SoundPlayer -ArgumentList '${String(file).replace(/'/g, "''")}').PlaySync()`;
    const child: VoiceChild = spawn("powershell", ["-NoProfile", "-NonInteractive", "-WindowStyle", "Hidden", "-Command", ps], {
      windowsHide: true, detached: true, stdio: "ignore",
    });
    activeVoicePlayer = child;
    activeIsLong = isLong;
    child.on("error", (err: Error) => { console.log(`[voice] SoundPlayer 播放失败: ${err.message}`); clearActive(child); });
    child.on("exit", (code) => {
      clearActive(child);
      if (child.killedByUs) return; // 被新播放主动掐断，不算播放异常
    ...
    console.log(`[voice] SoundPlayer 不可用: ${eMsg(e)}`);
