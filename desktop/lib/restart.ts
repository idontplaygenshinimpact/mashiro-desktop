// desktop/lib/restart.ts —— 桌宠重启设施（纵向拆分：从 desktop/main.mjs 迁出）
// 渲染层产物防呆（app.bundle.js 过期检测 + esbuild 自动重建）+ 暴力清理 widget 进程（重启用）
// 无 electron 依赖（fs/child_process），可单测
// 全量 TS 升级工单阶段 4（桌面）：desktop/lib/restart.mjs → .ts
//   调用方 2 处：desktop/main.mjs（Electron 主进程，动态 import 多个 .ts 为证支持类型擦除）、
//   tests/desktop-utils.test.mjs（--experimental-strip-types 运行）→ 直接改路径，不建桶
 ...
 * @param rendererDir desktop/renderer 目录
 * @param srcFiles 源码文件名（app.js/index.html/style.css）
 */
export function rendererBundleStale(rendererDir: string, srcFiles: string[]): boolean {
...
export async function rebuildRendererBundle(root: string): Promise<boolean> {
  try {
    ...
    console.log("[renderer] 自动重建 bundle 失败（不阻塞重启）:", (e as Error)?.message || e);
    return false;
  }
}
 ...
 */
export function killAllWidgetProcesses(): void {
  try {
    spawnSync(
      "powershell",
      ["-NoProfile", "-Command",
