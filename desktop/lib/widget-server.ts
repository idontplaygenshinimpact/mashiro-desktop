...
// 全量 TS 升级工单阶段 4（桌面）：实现迁至 widget-server.ts，widget-server.mjs 保留同名薄桶
// （main.mjs 按 .mjs 路径加载 → 调用方零改动）
import { spawn, spawnSync } from "node:child_process";
import type { ChildProcess, SpawnOptions } from "node:child_process";

/** 探测用 fetch（带 Bearer token）返回的最小形状（只读 ok/status） */
export interface HealthLikeResponse { ok: boolean; status?: number }

/** createWidgetServer 依赖（宿主注入；无 electron 依赖，可直接单测） */
export interface WidgetServerDeps {
  widgetFetch: (url: string, init?: { signal?: AbortSignal }) => Promise<HealthLikeResponse>;
  healthUrl: string;
  /** 项目根目录（spawn widget.mjs 的 cwd；打包时为 app.asar 路径） */
  root: string;
  /** 打包模式：ELECTRON_RUN_AS_NODE 运行（可读 asar） */
  isPackaged?: boolean;
}
...
export function safeSpawn(cmd: string, args: readonly string[], opts: SpawnOptions = {}): ChildProcess | null {
  try {
    const child = spawn(cmd, args as string[], { windowsHide: true, detached: true, stdio: "ignore", ...opts });
    child.on("error", (err: Error) => console.log(`[kanban] spawn ${cmd} 失败: ${err.message}`));
    try { child.unref(); } catch { /* ignore */ }
    return child;
  } catch (err) {
    console.log(`[kanban] spawn ${cmd} 异常: ${err instanceof Error ? err.message : String(err)}`);
    return null;
  }
}
 ...
 * 创建 widget 服务守护器
 */
export function createWidgetServer({ widgetFetch, healthUrl: healthUrlValue, root, isPackaged = false }: WidgetServerDeps): WidgetServer {
  // 本实例拉起的 widget 子进程 pid 集合（退出清理只杀自己拉起的，避免误杀其他 node 实例/开发进程）
  const spawnedPids = new Set<number>();
  let widgetProc: ChildProcess | null = null;
  ...
  function ensure(): void {
    ...
    widgetFetch(healthUrlValue, { signal: AbortSignal.timeout(5000) })
      ...
      .catch(() => {
        ...
        widgetProc = child;
        if (child) {
          // spawn 成功时 pid 必有值（同步失败会走 catch / 返回 null）——显式收窄，避免 Set<number> 混入 undefined
          const pid = child.pid;
          if (pid !== undefined) spawnedPids.add(pid);
          child.on("exit", (code) => {
            if (pid !== undefined) spawnedPids.delete(pid);
          ...
          child.on("error", (err: Error) => {
            console.log(`[kanban] widget.mjs 启动失败: ${(err as { message?: string })?.message || err}`);
            if (pid !== undefined) spawnedPids.delete(pid);
            if (widgetProc === child) widgetProc = null;
          });
